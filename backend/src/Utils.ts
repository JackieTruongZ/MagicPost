interface AddressProvince {
  _id: string;
  name: string;
  slug: string;
  type: string;
  name_with_type: string;
  code: string;
  isDeleted: boolean;
}

interface AddressDistrict {
  _id: string;
  name: string;
  slug: string;
  type: string;
  name_with_type: string;
  path: string;
  path_with_type: string;
  code: string;
  parent_code: string;
  isDeleted: boolean;
}

export const generateNameOfTransHub = (
  province: string,
  cityDistrict: string,
  name: string,
) => {
  if (
    indexingProvince(province) == '404' ||
    indexingCityDistrict(cityDistrict) == '404'
  ) {
    return '404';
  }
  return `${name}_${indexingProvince(
    province,
  )}_${indexingCityDistrict(cityDistrict)}`;
};

export const indexingProvince = (
  strProvince: string,
): string => {
  function findCodeByName(
    name: string,
  ): string | undefined {
    const province: AddressProvince = PROVINCE_ENUM.find(
      (p: AddressProvince) =>
        p.name.toLowerCase() ===
        name.toLowerCase(),
    );
    if (province === undefined) {
      console.log(province);
      return '404';
    }
    return province.code;
  }
  return findCodeByName(strProvince).toString();
};

export const findProvinceById = (
  provinceId: string,
): string => {
  function findCodeByName(
    proviceId: string,
  ): string | undefined {
    const province: AddressProvince = PROVINCE_ENUM.find(
      (p: AddressProvince) => p.code === proviceId,
    );
    if (province === undefined) {
      console.log(province);
      return '404';
    }
    return province.name;
  }
  return findCodeByName(provinceId).toString();
};

export const indexingCityDistrict = (
  strCityDistrict: string,
): string => {
  function findCodeByName(name: string): string {
    const cityDistrict: AddressDistrict =
      CITY_DISTRICT_ENUM.find(
        (p: AddressDistrict) =>
          p.name.toLowerCase() ===
          name.toLowerCase(),
      );
    if (cityDistrict === undefined) {
      console.log(cityDistrict);
      return '404';
    }
    return cityDistrict.code;
  }
  return findCodeByName(
    strCityDistrict,
  ).toString();
};

export const PROVINCE_ENUM: AddressProvince[] = [
  {
    _id: '60eaaa6f1173335842c35663',
    name: 'An Giang',
    slug: 'an-giang',
    type: 'tinh',
    name_with_type: 'Tỉnh An Giang',
    code: '89',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3565b',
    name: 'Bà Rịa - Vũng Tàu',
    slug: 'ba-ria---vung-tau',
    type: 'tinh',
    name_with_type: 'Tỉnh Bà Rịa - Vũng Tàu',
    code: '77',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35668',
    name: 'Bạc Liêu',
    slug: 'bac-lieu',
    type: 'tinh',
    name_with_type: 'Tỉnh Bạc Liêu',
    code: '95',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35639',
    name: 'Bắc Giang',
    slug: 'bac-giang',
    type: 'tinh',
    name_with_type: 'Tỉnh Bắc Giang',
    code: '24',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3562e',
    name: 'Bắc Kạn',
    slug: 'bac-kan',
    type: 'tinh',
    name_with_type: 'Tỉnh Bắc Kạn',
    code: '06',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3563c',
    name: 'Bắc Ninh',
    slug: 'bac-ninh',
    type: 'tinh',
    name_with_type: 'Tỉnh Bắc Ninh',
    code: '27',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3565f',
    name: 'Bến Tre',
    slug: 'ben-tre',
    type: 'tinh',
    name_with_type: 'Tỉnh Bến Tre',
    code: '83',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35659',
    name: 'Bình Dương',
    slug: 'binh-duong',
    type: 'tinh',
    name_with_type: 'Tỉnh Bình Dương',
    code: '74',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3564d',
    name: 'Bình Định',
    slug: 'binh-dinh',
    type: 'tinh',
    name_with_type: 'Tỉnh Bình Định',
    code: '52',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35657',
    name: 'Bình Phước',
    slug: 'binh-phuoc',
    type: 'tinh',
    name_with_type: 'Tỉnh Bình Phước',
    code: '70',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35651',
    name: 'Bình Thuận',
    slug: 'binh-thuan',
    type: 'tinh',
    name_with_type: 'Tỉnh Bình Thuận',
    code: '60',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35669',
    name: 'Cà Mau',
    slug: 'ca-mau',
    type: 'tinh',
    name_with_type: 'Tỉnh Cà Mau',
    code: '96',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3562d',
    name: 'Cao Bằng',
    slug: 'cao-bang',
    type: 'tinh',
    name_with_type: 'Tỉnh Cao Bằng',
    code: '04',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35665',
    name: 'Cần Thơ',
    slug: 'can-tho',
    type: 'thanh-pho',
    name_with_type: 'Thành phố Cần Thơ',
    code: '92',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3564a',
    name: 'Đà Nẵng',
    slug: 'da-nang',
    type: 'thanh-pho',
    name_with_type: 'Thành phố Đà Nẵng',
    code: '48',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35654',
    name: 'Đắk Lắk',
    slug: 'dak-lak',
    type: 'tinh',
    name_with_type: 'Tỉnh Đắk Lắk',
    code: '66',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35655',
    name: 'Đắk Nông',
    slug: 'dak-nong',
    type: 'tinh',
    name_with_type: 'Tỉnh Đắk Nông',
    code: '67',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35631',
    name: 'Điện Biên',
    slug: 'dien-bien',
    type: 'tinh',
    name_with_type: 'Tỉnh Điện Biên',
    code: '11',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3565a',
    name: 'Đồng Nai',
    slug: 'dong-nai',
    type: 'tinh',
    name_with_type: 'Tỉnh Đồng Nai',
    code: '75',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35662',
    name: 'Đồng Tháp',
    slug: 'dong-thap',
    type: 'tinh',
    name_with_type: 'Tỉnh Đồng Tháp',
    code: '87',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35653',
    name: 'Gia Lai',
    slug: 'gia-lai',
    type: 'tinh',
    name_with_type: 'Tỉnh Gia Lai',
    code: '64',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3562c',
    name: 'Hà Giang',
    slug: 'ha-giang',
    type: 'tinh',
    name_with_type: 'Tỉnh Hà Giang',
    code: '02',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35641',
    name: 'Hà Nam',
    slug: 'ha-nam',
    type: 'tinh',
    name_with_type: 'Tỉnh Hà Nam',
    code: '35',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3562b',
    name: 'Hà Nội',
    slug: 'ha-noi',
    type: 'thanh-pho',
    name_with_type: 'Thành phố Hà Nội',
    code: '01',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35646',
    name: 'Hà Tĩnh',
    slug: 'ha-tinh',
    type: 'tinh',
    name_with_type: 'Tỉnh Hà Tĩnh',
    code: '42',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3563d',
    name: 'Hải Dương',
    slug: 'hai-duong',
    type: 'tinh',
    name_with_type: 'Tỉnh Hải Dương',
    code: '30',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3563e',
    name: 'Hải Phòng',
    slug: 'hai-phong',
    type: 'thanh-pho',
    name_with_type: 'Thành phố Hải Phòng',
    code: '31',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35666',
    name: 'Hậu Giang',
    slug: 'hau-giang',
    type: 'tinh',
    name_with_type: 'Tỉnh Hậu Giang',
    code: '93',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35635',
    name: 'Hoà Bình',
    slug: 'hoa-binh',
    type: 'tinh',
    name_with_type: 'Tỉnh Hoà Bình',
    code: '17',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3565c',
    name: 'Hồ Chí Minh',
    slug: 'ho-chi-minh',
    type: 'thanh-pho',
    name_with_type: 'Thành phố Hồ Chí Minh',
    code: '79',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3563f',
    name: 'Hưng Yên',
    slug: 'hung-yen',
    type: 'tinh',
    name_with_type: 'Tỉnh Hưng Yên',
    code: '33',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3564f',
    name: 'Khánh Hòa',
    slug: 'khanh-hoa',
    type: 'tinh',
    name_with_type: 'Tỉnh Khánh Hòa',
    code: '56',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35664',
    name: 'Kiên Giang',
    slug: 'kien-giang',
    type: 'tinh',
    name_with_type: 'Tỉnh Kiên Giang',
    code: '91',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35652',
    name: 'Kon Tum',
    slug: 'kon-tum',
    type: 'tinh',
    name_with_type: 'Tỉnh Kon Tum',
    code: '62',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35632',
    name: 'Lai Châu',
    slug: 'lai-chau',
    type: 'tinh',
    name_with_type: 'Tỉnh Lai Châu',
    code: '12',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35637',
    name: 'Lạng Sơn',
    slug: 'lang-son',
    type: 'tinh',
    name_with_type: 'Tỉnh Lạng Sơn',
    code: '20',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35630',
    name: 'Lào Cai',
    slug: 'lao-cai',
    type: 'tinh',
    name_with_type: 'Tỉnh Lào Cai',
    code: '10',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35656',
    name: 'Lâm Đồng',
    slug: 'lam-dong',
    type: 'tinh',
    name_with_type: 'Tỉnh Lâm Đồng',
    code: '68',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3565d',
    name: 'Long An',
    slug: 'long-an',
    type: 'tinh',
    name_with_type: 'Tỉnh Long An',
    code: '80',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35642',
    name: 'Nam Định',
    slug: 'nam-dinh',
    type: 'tinh',
    name_with_type: 'Tỉnh Nam Định',
    code: '36',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35645',
    name: 'Nghệ An',
    slug: 'nghe-an',
    type: 'tinh',
    name_with_type: 'Tỉnh Nghệ An',
    code: '40',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35643',
    name: 'Ninh Bình',
    slug: 'ninh-binh',
    type: 'tinh',
    name_with_type: 'Tỉnh Ninh Bình',
    code: '37',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35650',
    name: 'Ninh Thuận',
    slug: 'ninh-thuan',
    type: 'tinh',
    name_with_type: 'Tỉnh Ninh Thuận',
    code: '58',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3563a',
    name: 'Phú Thọ',
    slug: 'phu-tho',
    type: 'tinh',
    name_with_type: 'Tỉnh Phú Thọ',
    code: '25',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3564e',
    name: 'Phú Yên',
    slug: 'phu-yen',
    type: 'tinh',
    name_with_type: 'Tỉnh Phú Yên',
    code: '54',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35647',
    name: 'Quảng Bình',
    slug: 'quang-binh',
    type: 'tinh',
    name_with_type: 'Tỉnh Quảng Bình',
    code: '44',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3564b',
    name: 'Quảng Nam',
    slug: 'quang-nam',
    type: 'tinh',
    name_with_type: 'Tỉnh Quảng Nam',
    code: '49',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3564c',
    name: 'Quảng Ngãi',
    slug: 'quang-ngai',
    type: 'tinh',
    name_with_type: 'Tỉnh Quảng Ngãi',
    code: '51',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35638',
    name: 'Quảng Ninh',
    slug: 'quang-ninh',
    type: 'tinh',
    name_with_type: 'Tỉnh Quảng Ninh',
    code: '22',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35648',
    name: 'Quảng Trị',
    slug: 'quang-tri',
    type: 'tinh',
    name_with_type: 'Tỉnh Quảng Trị',
    code: '45',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35667',
    name: 'Sóc Trăng',
    slug: 'soc-trang',
    type: 'tinh',
    name_with_type: 'Tỉnh Sóc Trăng',
    code: '94',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35633',
    name: 'Sơn La',
    slug: 'son-la',
    type: 'tinh',
    name_with_type: 'Tỉnh Sơn La',
    code: '14',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35658',
    name: 'Tây Ninh',
    slug: 'tay-ninh',
    type: 'tinh',
    name_with_type: 'Tỉnh Tây Ninh',
    code: '72',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35640',
    name: 'Thái Bình',
    slug: 'thai-binh',
    type: 'tinh',
    name_with_type: 'Tỉnh Thái Bình',
    code: '34',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35636',
    name: 'Thái Nguyên',
    slug: 'thai-nguyen',
    type: 'tinh',
    name_with_type: 'Tỉnh Thái Nguyên',
    code: '19',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35644',
    name: 'Thanh Hóa',
    slug: 'thanh-hoa',
    type: 'tinh',
    name_with_type: 'Tỉnh Thanh Hóa',
    code: '38',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35649',
    name: 'Thừa Thiên Huế',
    slug: 'thua-thien-hue',
    type: 'tinh',
    name_with_type: 'Tỉnh Thừa Thiên Huế',
    code: '46',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3565e',
    name: 'Tiền Giang',
    slug: 'tien-giang',
    type: 'tinh',
    name_with_type: 'Tỉnh Tiền Giang',
    code: '82',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35660',
    name: 'Trà Vinh',
    slug: 'tra-vinh',
    type: 'tinh',
    name_with_type: 'Tỉnh Trà Vinh',
    code: '84',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3562f',
    name: 'Tuyên Quang',
    slug: 'tuyen-quang',
    type: 'tinh',
    name_with_type: 'Tỉnh Tuyên Quang',
    code: '08',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35661',
    name: 'Vĩnh Long',
    slug: 'vinh-long',
    type: 'tinh',
    name_with_type: 'Tỉnh Vĩnh Long',
    code: '86',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c3563b',
    name: 'Vĩnh Phúc',
    slug: 'vinh-phuc',
    type: 'tinh',
    name_with_type: 'Tỉnh Vĩnh Phúc',
    code: '26',
    isDeleted: false,
  },
  {
    _id: '60eaaa6f1173335842c35634',
    name: 'Yên Bái',
    slug: 'yen-bai',
    type: 'tinh',
    name_with_type: 'Tỉnh Yên Bái',
    code: '15',
    isDeleted: false,
  },
];

export const CITY_DISTRICT_ENUM: AddressDistrict[] =
  [
    {
      _id: '60eaaa6f1173335842c3536a',
      name: 'Ba Đình',
      type: 'quan',
      slug: 'ba-dinh',
      name_with_type: 'Quận Ba Đình',
      path: 'Ba Đình, Hà Nội',
      path_with_type:
        'Quận Ba Đình, Thành phố Hà Nội',
      code: '001',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3536b',
      name: 'Hoàn Kiếm',
      type: 'quan',
      slug: 'hoan-kiem',
      name_with_type: 'Quận Hoàn Kiếm',
      path: 'Hoàn Kiếm, Hà Nội',
      path_with_type:
        'Quận Hoàn Kiếm, Thành phố Hà Nội',
      code: '002',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3536c',
      name: 'Tây Hồ',
      type: 'quan',
      slug: 'tay-ho',
      name_with_type: 'Quận Tây Hồ',
      path: 'Tây Hồ, Hà Nội',
      path_with_type:
        'Quận Tây Hồ, Thành phố Hà Nội',
      code: '003',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3536d',
      name: 'Long Biên',
      type: 'quan',
      slug: 'long-bien',
      name_with_type: 'Quận Long Biên',
      path: 'Long Biên, Hà Nội',
      path_with_type:
        'Quận Long Biên, Thành phố Hà Nội',
      code: '004',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3536e',
      name: 'Cầu Giấy',
      type: 'quan',
      slug: 'cau-giay',
      name_with_type: 'Quận Cầu Giấy',
      path: 'Cầu Giấy, Hà Nội',
      path_with_type:
        'Quận Cầu Giấy, Thành phố Hà Nội',
      code: '005',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3536f',
      name: 'Đống Đa',
      type: 'quan',
      slug: 'dong-da',
      name_with_type: 'Quận Đống Đa',
      path: 'Đống Đa, Hà Nội',
      path_with_type:
        'Quận Đống Đa, Thành phố Hà Nội',
      code: '006',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35370',
      name: 'Hai Bà Trưng',
      type: 'quan',
      slug: 'hai-ba-trung',
      name_with_type: 'Quận Hai Bà Trưng',
      path: 'Hai Bà Trưng, Hà Nội',
      path_with_type:
        'Quận Hai Bà Trưng, Thành phố Hà Nội',
      code: '007',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35371',
      name: 'Hoàng Mai',
      type: 'quan',
      slug: 'hoang-mai',
      name_with_type: 'Quận Hoàng Mai',
      path: 'Hoàng Mai, Hà Nội',
      path_with_type:
        'Quận Hoàng Mai, Thành phố Hà Nội',
      code: '008',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35372',
      name: 'Thanh Xuân',
      type: 'quan',
      slug: 'thanh-xuan',
      name_with_type: 'Quận Thanh Xuân',
      path: 'Thanh Xuân, Hà Nội',
      path_with_type:
        'Quận Thanh Xuân, Thành phố Hà Nội',
      code: '009',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35373',
      name: 'Sóc Sơn',
      type: 'huyen',
      slug: 'soc-son',
      name_with_type: 'Huyện Sóc Sơn',
      path: 'Sóc Sơn, Hà Nội',
      path_with_type:
        'Huyện Sóc Sơn, Thành phố Hà Nội',
      code: '016',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35374',
      name: 'Đông Anh',
      type: 'huyen',
      slug: 'dong-anh',
      name_with_type: 'Huyện Đông Anh',
      path: 'Đông Anh, Hà Nội',
      path_with_type:
        'Huyện Đông Anh, Thành phố Hà Nội',
      code: '017',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35375',
      name: 'Gia Lâm',
      type: 'huyen',
      slug: 'gia-lam',
      name_with_type: 'Huyện Gia Lâm',
      path: 'Gia Lâm, Hà Nội',
      path_with_type:
        'Huyện Gia Lâm, Thành phố Hà Nội',
      code: '018',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35376',
      name: 'Nam Từ Liêm',
      type: 'quan',
      slug: 'nam-tu-liem',
      name_with_type: 'Quận Nam Từ Liêm',
      path: 'Nam Từ Liêm, Hà Nội',
      path_with_type:
        'Quận Nam Từ Liêm, Thành phố Hà Nội',
      code: '019',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35377',
      name: 'Thanh Trì',
      type: 'huyen',
      slug: 'thanh-tri',
      name_with_type: 'Huyện Thanh Trì',
      path: 'Thanh Trì, Hà Nội',
      path_with_type:
        'Huyện Thanh Trì, Thành phố Hà Nội',
      code: '020',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35378',
      name: 'Bắc Từ Liêm',
      type: 'quan',
      slug: 'bac-tu-liem',
      name_with_type: 'Quận Bắc Từ Liêm',
      path: 'Bắc Từ Liêm, Hà Nội',
      path_with_type:
        'Quận Bắc Từ Liêm, Thành phố Hà Nội',
      code: '021',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35379',
      name: 'Mê Linh',
      type: 'huyen',
      slug: 'me-linh',
      name_with_type: 'Huyện Mê Linh',
      path: 'Mê Linh, Hà Nội',
      path_with_type:
        'Huyện Mê Linh, Thành phố Hà Nội',
      code: '250',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3537a',
      name: 'Hà Đông',
      type: 'quan',
      slug: 'ha-dong',
      name_with_type: 'Quận Hà Đông',
      path: 'Hà Đông, Hà Nội',
      path_with_type:
        'Quận Hà Đông, Thành phố Hà Nội',
      code: '268',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3537b',
      name: 'Sơn Tây',
      type: 'thi-xa',
      slug: 'son-tay',
      name_with_type: 'Thị xã Sơn Tây',
      path: 'Sơn Tây, Hà Nội',
      path_with_type:
        'Thị xã Sơn Tây, Thành phố Hà Nội',
      code: '269',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3537c',
      name: 'Ba Vì',
      type: 'huyen',
      slug: 'ba-vi',
      name_with_type: 'Huyện Ba Vì',
      path: 'Ba Vì, Hà Nội',
      path_with_type:
        'Huyện Ba Vì, Thành phố Hà Nội',
      code: '271',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3537d',
      name: 'Phúc Thọ',
      type: 'huyen',
      slug: 'phuc-tho',
      name_with_type: 'Huyện Phúc Thọ',
      path: 'Phúc Thọ, Hà Nội',
      path_with_type:
        'Huyện Phúc Thọ, Thành phố Hà Nội',
      code: '272',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3537e',
      name: 'Đan Phượng',
      type: 'huyen',
      slug: 'dan-phuong',
      name_with_type: 'Huyện Đan Phượng',
      path: 'Đan Phượng, Hà Nội',
      path_with_type:
        'Huyện Đan Phượng, Thành phố Hà Nội',
      code: '273',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3537f',
      name: 'Hoài Đức',
      type: 'huyen',
      slug: 'hoai-duc',
      name_with_type: 'Huyện Hoài Đức',
      path: 'Hoài Đức, Hà Nội',
      path_with_type:
        'Huyện Hoài Đức, Thành phố Hà Nội',
      code: '274',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35380',
      name: 'Quốc Oai',
      type: 'huyen',
      slug: 'quoc-oai',
      name_with_type: 'Huyện Quốc Oai',
      path: 'Quốc Oai, Hà Nội',
      path_with_type:
        'Huyện Quốc Oai, Thành phố Hà Nội',
      code: '275',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35381',
      name: 'Thạch Thất',
      type: 'huyen',
      slug: 'thach-that',
      name_with_type: 'Huyện Thạch Thất',
      path: 'Thạch Thất, Hà Nội',
      path_with_type:
        'Huyện Thạch Thất, Thành phố Hà Nội',
      code: '276',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35382',
      name: 'Chương Mỹ',
      type: 'huyen',
      slug: 'chuong-my',
      name_with_type: 'Huyện Chương Mỹ',
      path: 'Chương Mỹ, Hà Nội',
      path_with_type:
        'Huyện Chương Mỹ, Thành phố Hà Nội',
      code: '277',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35383',
      name: 'Thanh Oai',
      type: 'huyen',
      slug: 'thanh-oai',
      name_with_type: 'Huyện Thanh Oai',
      path: 'Thanh Oai, Hà Nội',
      path_with_type:
        'Huyện Thanh Oai, Thành phố Hà Nội',
      code: '278',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35384',
      name: 'Thường Tín',
      type: 'huyen',
      slug: 'thuong-tin',
      name_with_type: 'Huyện Thường Tín',
      path: 'Thường Tín, Hà Nội',
      path_with_type:
        'Huyện Thường Tín, Thành phố Hà Nội',
      code: '279',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35385',
      name: 'Phú Xuyên',
      type: 'huyen',
      slug: 'phu-xuyen',
      name_with_type: 'Huyện Phú Xuyên',
      path: 'Phú Xuyên, Hà Nội',
      path_with_type:
        'Huyện Phú Xuyên, Thành phố Hà Nội',
      code: '280',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35386',
      name: 'Ứng Hòa',
      type: 'huyen',
      slug: 'ung-hoa',
      name_with_type: 'Huyện Ứng Hòa',
      path: 'Ứng Hòa, Hà Nội',
      path_with_type:
        'Huyện Ứng Hòa, Thành phố Hà Nội',
      code: '281',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35387',
      name: 'Mỹ Đức',
      type: 'huyen',
      slug: 'my-duc',
      name_with_type: 'Huyện Mỹ Đức',
      path: 'Mỹ Đức, Hà Nội',
      path_with_type:
        'Huyện Mỹ Đức, Thành phố Hà Nội',
      code: '282',
      parent_code: '01',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35388',
      name: 'Hà Giang',
      type: 'thanh-pho',
      slug: 'ha-giang',
      name_with_type: 'Thành phố Hà Giang',
      path: 'Hà Giang, Hà Giang',
      path_with_type:
        'Thành phố Hà Giang, Tỉnh Hà Giang',
      code: '024',
      parent_code: '02',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35389',
      name: 'Đồng Văn',
      type: 'huyen',
      slug: 'dong-van',
      name_with_type: 'Huyện Đồng Văn',
      path: 'Đồng Văn, Hà Giang',
      path_with_type:
        'Huyện Đồng Văn, Tỉnh Hà Giang',
      code: '026',
      parent_code: '02',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3538a',
      name: 'Mèo Vạc',
      type: 'huyen',
      slug: 'meo-vac',
      name_with_type: 'Huyện Mèo Vạc',
      path: 'Mèo Vạc, Hà Giang',
      path_with_type:
        'Huyện Mèo Vạc, Tỉnh Hà Giang',
      code: '027',
      parent_code: '02',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3538b',
      name: 'Yên Minh',
      type: 'huyen',
      slug: 'yen-minh',
      name_with_type: 'Huyện Yên Minh',
      path: 'Yên Minh, Hà Giang',
      path_with_type:
        'Huyện Yên Minh, Tỉnh Hà Giang',
      code: '028',
      parent_code: '02',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3538c',
      name: 'Quản Bạ',
      type: 'huyen',
      slug: 'quan-ba',
      name_with_type: 'Huyện Quản Bạ',
      path: 'Quản Bạ, Hà Giang',
      path_with_type:
        'Huyện Quản Bạ, Tỉnh Hà Giang',
      code: '029',
      parent_code: '02',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3538d',
      name: 'Vị Xuyên',
      type: 'huyen',
      slug: 'vi-xuyen',
      name_with_type: 'Huyện Vị Xuyên',
      path: 'Vị Xuyên, Hà Giang',
      path_with_type:
        'Huyện Vị Xuyên, Tỉnh Hà Giang',
      code: '030',
      parent_code: '02',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3538e',
      name: 'Bắc Mê',
      type: 'huyen',
      slug: 'bac-me',
      name_with_type: 'Huyện Bắc Mê',
      path: 'Bắc Mê, Hà Giang',
      path_with_type:
        'Huyện Bắc Mê, Tỉnh Hà Giang',
      code: '031',
      parent_code: '02',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3538f',
      name: 'Hoàng Su Phì',
      type: 'huyen',
      slug: 'hoang-su-phi',
      name_with_type: 'Huyện Hoàng Su Phì',
      path: 'Hoàng Su Phì, Hà Giang',
      path_with_type:
        'Huyện Hoàng Su Phì, Tỉnh Hà Giang',
      code: '032',
      parent_code: '02',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35390',
      name: 'Xín Mần',
      type: 'huyen',
      slug: 'xin-man',
      name_with_type: 'Huyện Xín Mần',
      path: 'Xín Mần, Hà Giang',
      path_with_type:
        'Huyện Xín Mần, Tỉnh Hà Giang',
      code: '033',
      parent_code: '02',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35391',
      name: 'Bắc Quang',
      type: 'huyen',
      slug: 'bac-quang',
      name_with_type: 'Huyện Bắc Quang',
      path: 'Bắc Quang, Hà Giang',
      path_with_type:
        'Huyện Bắc Quang, Tỉnh Hà Giang',
      code: '034',
      parent_code: '02',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35392',
      name: 'Quang Bình',
      type: 'huyen',
      slug: 'quang-binh',
      name_with_type: 'Huyện Quang Bình',
      path: 'Quang Bình, Hà Giang',
      path_with_type:
        'Huyện Quang Bình, Tỉnh Hà Giang',
      code: '035',
      parent_code: '02',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35393',
      name: 'Cao Bằng',
      type: 'thanh-pho',
      slug: 'cao-bang',
      name_with_type: 'Thành phố Cao Bằng',
      path: 'Cao Bằng, Cao Bằng',
      path_with_type:
        'Thành phố Cao Bằng, Tỉnh Cao Bằng',
      code: '040',
      parent_code: '04',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35394',
      name: 'Bảo Lâm',
      type: 'huyen',
      slug: 'bao-lam',
      name_with_type: 'Huyện Bảo Lâm',
      path: 'Bảo Lâm, Cao Bằng',
      path_with_type:
        'Huyện Bảo Lâm, Tỉnh Cao Bằng',
      code: '042',
      parent_code: '04',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35395',
      name: 'Bảo Lạc',
      type: 'huyen',
      slug: 'bao-lac',
      name_with_type: 'Huyện Bảo Lạc',
      path: 'Bảo Lạc, Cao Bằng',
      path_with_type:
        'Huyện Bảo Lạc, Tỉnh Cao Bằng',
      code: '043',
      parent_code: '04',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35396',
      name: 'Hà Quảng',
      type: 'huyen',
      slug: 'ha-quang',
      name_with_type: 'Huyện Hà Quảng',
      path: 'Hà Quảng, Cao Bằng',
      path_with_type:
        'Huyện Hà Quảng, Tỉnh Cao Bằng',
      code: '045',
      parent_code: '04',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35397',
      name: 'Trùng Khánh',
      type: 'huyen',
      slug: 'trung-khanh',
      name_with_type: 'Huyện Trùng Khánh',
      path: 'Trùng Khánh, Cao Bằng',
      path_with_type:
        'Huyện Trùng Khánh, Tỉnh Cao Bằng',
      code: '047',
      parent_code: '04',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35398',
      name: 'Hạ Lang',
      type: 'huyen',
      slug: 'ha-lang',
      name_with_type: 'Huyện Hạ Lang',
      path: 'Hạ Lang, Cao Bằng',
      path_with_type:
        'Huyện Hạ Lang, Tỉnh Cao Bằng',
      code: '048',
      parent_code: '04',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35399',
      name: 'Quảng Hòa',
      type: 'huyen',
      slug: 'quang-hoa',
      name_with_type: 'Huyện Quảng Hòa',
      path: 'Quảng Hòa, Cao Bằng',
      path_with_type:
        'Huyện Quảng Hòa, Tỉnh Cao Bằng',
      code: '049',
      parent_code: '04',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3539a',
      name: 'Hoà An',
      type: 'huyen',
      slug: 'hoa-an',
      name_with_type: 'Huyện Hoà An',
      path: 'Hoà An, Cao Bằng',
      path_with_type:
        'Huyện Hoà An, Tỉnh Cao Bằng',
      code: '051',
      parent_code: '04',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3539b',
      name: 'Nguyên Bình',
      type: 'huyen',
      slug: 'nguyen-binh',
      name_with_type: 'Huyện Nguyên Bình',
      path: 'Nguyên Bình, Cao Bằng',
      path_with_type:
        'Huyện Nguyên Bình, Tỉnh Cao Bằng',
      code: '052',
      parent_code: '04',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3539c',
      name: 'Thạch An',
      type: 'huyen',
      slug: 'thach-an',
      name_with_type: 'Huyện Thạch An',
      path: 'Thạch An, Cao Bằng',
      path_with_type:
        'Huyện Thạch An, Tỉnh Cao Bằng',
      code: '053',
      parent_code: '04',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3539d',
      name: 'Bắc Kạn',
      type: 'thanh-pho',
      slug: 'bac-kan',
      name_with_type: 'Thành Phố Bắc Kạn',
      path: 'Bắc Kạn, Bắc Kạn',
      path_with_type:
        'Thành Phố Bắc Kạn, Tỉnh Bắc Kạn',
      code: '058',
      parent_code: '06',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3539e',
      name: 'Pác Nặm',
      type: 'huyen',
      slug: 'pac-nam',
      name_with_type: 'Huyện Pác Nặm',
      path: 'Pác Nặm, Bắc Kạn',
      path_with_type:
        'Huyện Pác Nặm, Tỉnh Bắc Kạn',
      code: '060',
      parent_code: '06',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3539f',
      name: 'Ba Bể',
      type: 'huyen',
      slug: 'ba-be',
      name_with_type: 'Huyện Ba Bể',
      path: 'Ba Bể, Bắc Kạn',
      path_with_type: 'Huyện Ba Bể, Tỉnh Bắc Kạn',
      code: '061',
      parent_code: '06',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353a0',
      name: 'Ngân Sơn',
      type: 'huyen',
      slug: 'ngan-son',
      name_with_type: 'Huyện Ngân Sơn',
      path: 'Ngân Sơn, Bắc Kạn',
      path_with_type:
        'Huyện Ngân Sơn, Tỉnh Bắc Kạn',
      code: '062',
      parent_code: '06',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353a1',
      name: 'Bạch Thông',
      type: 'huyen',
      slug: 'bach-thong',
      name_with_type: 'Huyện Bạch Thông',
      path: 'Bạch Thông, Bắc Kạn',
      path_with_type:
        'Huyện Bạch Thông, Tỉnh Bắc Kạn',
      code: '063',
      parent_code: '06',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353a2',
      name: 'Chợ Đồn',
      type: 'huyen',
      slug: 'cho-don',
      name_with_type: 'Huyện Chợ Đồn',
      path: 'Chợ Đồn, Bắc Kạn',
      path_with_type:
        'Huyện Chợ Đồn, Tỉnh Bắc Kạn',
      code: '064',
      parent_code: '06',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353a3',
      name: 'Chợ Mới',
      type: 'huyen',
      slug: 'cho-moi',
      name_with_type: 'Huyện Chợ Mới',
      path: 'Chợ Mới, Bắc Kạn',
      path_with_type:
        'Huyện Chợ Mới, Tỉnh Bắc Kạn',
      code: '065',
      parent_code: '06',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353a4',
      name: 'Na Rì',
      type: 'huyen',
      slug: 'na-ri',
      name_with_type: 'Huyện Na Rì',
      path: 'Na Rì, Bắc Kạn',
      path_with_type: 'Huyện Na Rì, Tỉnh Bắc Kạn',
      code: '066',
      parent_code: '06',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353a5',
      name: 'Tuyên Quang',
      type: 'thanh-pho',
      slug: 'tuyen-quang',
      name_with_type: 'Thành phố Tuyên Quang',
      path: 'Tuyên Quang, Tuyên Quang',
      path_with_type:
        'Thành phố Tuyên Quang, Tỉnh Tuyên Quang',
      code: '070',
      parent_code: '08',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353a6',
      name: 'Lâm Bình',
      type: 'huyen',
      slug: 'lam-binh',
      name_with_type: 'Huyện Lâm Bình',
      path: 'Lâm Bình, Tuyên Quang',
      path_with_type:
        'Huyện Lâm Bình, Tỉnh Tuyên Quang',
      code: '071',
      parent_code: '08',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353a7',
      name: 'Na Hang',
      type: 'huyen',
      slug: 'na-hang',
      name_with_type: 'Huyện Na Hang',
      path: 'Na Hang, Tuyên Quang',
      path_with_type:
        'Huyện Na Hang, Tỉnh Tuyên Quang',
      code: '072',
      parent_code: '08',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353a8',
      name: 'Chiêm Hóa',
      type: 'huyen',
      slug: 'chiem-hoa',
      name_with_type: 'Huyện Chiêm Hóa',
      path: 'Chiêm Hóa, Tuyên Quang',
      path_with_type:
        'Huyện Chiêm Hóa, Tỉnh Tuyên Quang',
      code: '073',
      parent_code: '08',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353a9',
      name: 'Hàm Yên',
      type: 'huyen',
      slug: 'ham-yen',
      name_with_type: 'Huyện Hàm Yên',
      path: 'Hàm Yên, Tuyên Quang',
      path_with_type:
        'Huyện Hàm Yên, Tỉnh Tuyên Quang',
      code: '074',
      parent_code: '08',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353aa',
      name: 'Yên Sơn',
      type: 'huyen',
      slug: 'yen-son',
      name_with_type: 'Huyện Yên Sơn',
      path: 'Yên Sơn, Tuyên Quang',
      path_with_type:
        'Huyện Yên Sơn, Tỉnh Tuyên Quang',
      code: '075',
      parent_code: '08',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353ab',
      name: 'Sơn Dương',
      type: 'huyen',
      slug: 'son-duong',
      name_with_type: 'Huyện Sơn Dương',
      path: 'Sơn Dương, Tuyên Quang',
      path_with_type:
        'Huyện Sơn Dương, Tỉnh Tuyên Quang',
      code: '076',
      parent_code: '08',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353ac',
      name: 'Lào Cai',
      type: 'thanh-pho',
      slug: 'lao-cai',
      name_with_type: 'Thành phố Lào Cai',
      path: 'Lào Cai, Lào Cai',
      path_with_type:
        'Thành phố Lào Cai, Tỉnh Lào Cai',
      code: '080',
      parent_code: '10',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353ad',
      name: 'Bát Xát',
      type: 'huyen',
      slug: 'bat-xat',
      name_with_type: 'Huyện Bát Xát',
      path: 'Bát Xát, Lào Cai',
      path_with_type:
        'Huyện Bát Xát, Tỉnh Lào Cai',
      code: '082',
      parent_code: '10',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353ae',
      name: 'Mường Khương',
      type: 'huyen',
      slug: 'muong-khuong',
      name_with_type: 'Huyện Mường Khương',
      path: 'Mường Khương, Lào Cai',
      path_with_type:
        'Huyện Mường Khương, Tỉnh Lào Cai',
      code: '083',
      parent_code: '10',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353af',
      name: 'Si Ma Cai',
      type: 'huyen',
      slug: 'si-ma-cai',
      name_with_type: 'Huyện Si Ma Cai',
      path: 'Si Ma Cai, Lào Cai',
      path_with_type:
        'Huyện Si Ma Cai, Tỉnh Lào Cai',
      code: '084',
      parent_code: '10',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353b0',
      name: 'Bắc Hà',
      type: 'huyen',
      slug: 'bac-ha',
      name_with_type: 'Huyện Bắc Hà',
      path: 'Bắc Hà, Lào Cai',
      path_with_type:
        'Huyện Bắc Hà, Tỉnh Lào Cai',
      code: '085',
      parent_code: '10',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353b1',
      name: 'Bảo Thắng',
      type: 'huyen',
      slug: 'bao-thang',
      name_with_type: 'Huyện Bảo Thắng',
      path: 'Bảo Thắng, Lào Cai',
      path_with_type:
        'Huyện Bảo Thắng, Tỉnh Lào Cai',
      code: '086',
      parent_code: '10',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353b2',
      name: 'Bảo Yên',
      type: 'huyen',
      slug: 'bao-yen',
      name_with_type: 'Huyện Bảo Yên',
      path: 'Bảo Yên, Lào Cai',
      path_with_type:
        'Huyện Bảo Yên, Tỉnh Lào Cai',
      code: '087',
      parent_code: '10',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353b3',
      name: 'Sa Pa',
      type: 'thi-xa',
      slug: 'sa-pa',
      name_with_type: 'Thị xã Sa Pa',
      path: 'Sa Pa, Lào Cai',
      path_with_type:
        'Thị xã Sa Pa, Tỉnh Lào Cai',
      code: '088',
      parent_code: '10',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353b4',
      name: 'Văn Bàn',
      type: 'huyen',
      slug: 'van-ban',
      name_with_type: 'Huyện Văn Bàn',
      path: 'Văn Bàn, Lào Cai',
      path_with_type:
        'Huyện Văn Bàn, Tỉnh Lào Cai',
      code: '089',
      parent_code: '10',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353b5',
      name: 'Điện Biên Phủ',
      type: 'thanh-pho',
      slug: 'dien-bien-phu',
      name_with_type: 'Thành phố Điện Biên Phủ',
      path: 'Điện Biên Phủ, Điện Biên',
      path_with_type:
        'Thành phố Điện Biên Phủ, Tỉnh Điện Biên',
      code: '094',
      parent_code: '11',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353b6',
      name: 'Mường Lay',
      type: 'thi-xa',
      slug: 'muong-lay',
      name_with_type: 'Thị Xã Mường Lay',
      path: 'Mường Lay, Điện Biên',
      path_with_type:
        'Thị Xã Mường Lay, Tỉnh Điện Biên',
      code: '095',
      parent_code: '11',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353b7',
      name: 'Mường Nhé',
      type: 'huyen',
      slug: 'muong-nhe',
      name_with_type: 'Huyện Mường Nhé',
      path: 'Mường Nhé, Điện Biên',
      path_with_type:
        'Huyện Mường Nhé, Tỉnh Điện Biên',
      code: '096',
      parent_code: '11',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353b8',
      name: 'Mường Chà',
      type: 'huyen',
      slug: 'muong-cha',
      name_with_type: 'Huyện Mường Chà',
      path: 'Mường Chà, Điện Biên',
      path_with_type:
        'Huyện Mường Chà, Tỉnh Điện Biên',
      code: '097',
      parent_code: '11',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353b9',
      name: 'Tủa Chùa',
      type: 'huyen',
      slug: 'tua-chua',
      name_with_type: 'Huyện Tủa Chùa',
      path: 'Tủa Chùa, Điện Biên',
      path_with_type:
        'Huyện Tủa Chùa, Tỉnh Điện Biên',
      code: '098',
      parent_code: '11',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353ba',
      name: 'Tuần Giáo',
      type: 'huyen',
      slug: 'tuan-giao',
      name_with_type: 'Huyện Tuần Giáo',
      path: 'Tuần Giáo, Điện Biên',
      path_with_type:
        'Huyện Tuần Giáo, Tỉnh Điện Biên',
      code: '099',
      parent_code: '11',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353bb',
      name: 'Điện Biên',
      type: 'huyen',
      slug: 'dien-bien',
      name_with_type: 'Huyện Điện Biên',
      path: 'Điện Biên, Điện Biên',
      path_with_type:
        'Huyện Điện Biên, Tỉnh Điện Biên',
      code: '100',
      parent_code: '11',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353bc',
      name: 'Điện Biên Đông',
      type: 'huyen',
      slug: 'dien-bien-dong',
      name_with_type: 'Huyện Điện Biên Đông',
      path: 'Điện Biên Đông, Điện Biên',
      path_with_type:
        'Huyện Điện Biên Đông, Tỉnh Điện Biên',
      code: '101',
      parent_code: '11',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353bd',
      name: 'Mường Ảng',
      type: 'huyen',
      slug: 'muong-ang',
      name_with_type: 'Huyện Mường Ảng',
      path: 'Mường Ảng, Điện Biên',
      path_with_type:
        'Huyện Mường Ảng, Tỉnh Điện Biên',
      code: '102',
      parent_code: '11',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353be',
      name: 'Nậm Pồ',
      type: 'huyen',
      slug: 'nam-po',
      name_with_type: 'Huyện Nậm Pồ',
      path: 'Nậm Pồ, Điện Biên',
      path_with_type:
        'Huyện Nậm Pồ, Tỉnh Điện Biên',
      code: '103',
      parent_code: '11',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353bf',
      name: 'Lai Châu',
      type: 'thanh-pho',
      slug: 'lai-chau',
      name_with_type: 'Thành phố Lai Châu',
      path: 'Lai Châu, Lai Châu',
      path_with_type:
        'Thành phố Lai Châu, Tỉnh Lai Châu',
      code: '105',
      parent_code: '12',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353c0',
      name: 'Tam Đường',
      type: 'huyen',
      slug: 'tam-duong',
      name_with_type: 'Huyện Tam Đường',
      path: 'Tam Đường, Lai Châu',
      path_with_type:
        'Huyện Tam Đường, Tỉnh Lai Châu',
      code: '106',
      parent_code: '12',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353c1',
      name: 'Mường Tè',
      type: 'huyen',
      slug: 'muong-te',
      name_with_type: 'Huyện Mường Tè',
      path: 'Mường Tè, Lai Châu',
      path_with_type:
        'Huyện Mường Tè, Tỉnh Lai Châu',
      code: '107',
      parent_code: '12',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353c2',
      name: 'Sìn Hồ',
      type: 'huyen',
      slug: 'sin-ho',
      name_with_type: 'Huyện Sìn Hồ',
      path: 'Sìn Hồ, Lai Châu',
      path_with_type:
        'Huyện Sìn Hồ, Tỉnh Lai Châu',
      code: '108',
      parent_code: '12',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353c3',
      name: 'Phong Thổ',
      type: 'huyen',
      slug: 'phong-tho',
      name_with_type: 'Huyện Phong Thổ',
      path: 'Phong Thổ, Lai Châu',
      path_with_type:
        'Huyện Phong Thổ, Tỉnh Lai Châu',
      code: '109',
      parent_code: '12',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353c4',
      name: 'Than Uyên',
      type: 'huyen',
      slug: 'than-uyen',
      name_with_type: 'Huyện Than Uyên',
      path: 'Than Uyên, Lai Châu',
      path_with_type:
        'Huyện Than Uyên, Tỉnh Lai Châu',
      code: '110',
      parent_code: '12',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353c5',
      name: 'Tân Uyên',
      type: 'huyen',
      slug: 'tan-uyen',
      name_with_type: 'Huyện Tân Uyên',
      path: 'Tân Uyên, Lai Châu',
      path_with_type:
        'Huyện Tân Uyên, Tỉnh Lai Châu',
      code: '111',
      parent_code: '12',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353c6',
      name: 'Nậm Nhùn',
      type: 'huyen',
      slug: 'nam-nhun',
      name_with_type: 'Huyện Nậm Nhùn',
      path: 'Nậm Nhùn, Lai Châu',
      path_with_type:
        'Huyện Nậm Nhùn, Tỉnh Lai Châu',
      code: '112',
      parent_code: '12',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353c7',
      name: 'Sơn La',
      type: 'thanh-pho',
      slug: 'son-la',
      name_with_type: 'Thành phố Sơn La',
      path: 'Sơn La, Sơn La',
      path_with_type:
        'Thành phố Sơn La, Tỉnh Sơn La',
      code: '116',
      parent_code: '14',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353c8',
      name: 'Quỳnh Nhai',
      type: 'huyen',
      slug: 'quynh-nhai',
      name_with_type: 'Huyện Quỳnh Nhai',
      path: 'Quỳnh Nhai, Sơn La',
      path_with_type:
        'Huyện Quỳnh Nhai, Tỉnh Sơn La',
      code: '118',
      parent_code: '14',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353c9',
      name: 'Thuận Châu',
      type: 'huyen',
      slug: 'thuan-chau',
      name_with_type: 'Huyện Thuận Châu',
      path: 'Thuận Châu, Sơn La',
      path_with_type:
        'Huyện Thuận Châu, Tỉnh Sơn La',
      code: '119',
      parent_code: '14',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353ca',
      name: 'Mường La',
      type: 'huyen',
      slug: 'muong-la',
      name_with_type: 'Huyện Mường La',
      path: 'Mường La, Sơn La',
      path_with_type:
        'Huyện Mường La, Tỉnh Sơn La',
      code: '120',
      parent_code: '14',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353cb',
      name: 'Bắc Yên',
      type: 'huyen',
      slug: 'bac-yen',
      name_with_type: 'Huyện Bắc Yên',
      path: 'Bắc Yên, Sơn La',
      path_with_type:
        'Huyện Bắc Yên, Tỉnh Sơn La',
      code: '121',
      parent_code: '14',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353cc',
      name: 'Phù Yên',
      type: 'huyen',
      slug: 'phu-yen',
      name_with_type: 'Huyện Phù Yên',
      path: 'Phù Yên, Sơn La',
      path_with_type:
        'Huyện Phù Yên, Tỉnh Sơn La',
      code: '122',
      parent_code: '14',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353cd',
      name: 'Mộc Châu',
      type: 'huyen',
      slug: 'moc-chau',
      name_with_type: 'Huyện Mộc Châu',
      path: 'Mộc Châu, Sơn La',
      path_with_type:
        'Huyện Mộc Châu, Tỉnh Sơn La',
      code: '123',
      parent_code: '14',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353ce',
      name: 'Yên Châu',
      type: 'huyen',
      slug: 'yen-chau',
      name_with_type: 'Huyện Yên Châu',
      path: 'Yên Châu, Sơn La',
      path_with_type:
        'Huyện Yên Châu, Tỉnh Sơn La',
      code: '124',
      parent_code: '14',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353cf',
      name: 'Mai Sơn',
      type: 'huyen',
      slug: 'mai-son',
      name_with_type: 'Huyện Mai Sơn',
      path: 'Mai Sơn, Sơn La',
      path_with_type:
        'Huyện Mai Sơn, Tỉnh Sơn La',
      code: '125',
      parent_code: '14',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353d0',
      name: 'Sông Mã',
      type: 'huyen',
      slug: 'song-ma',
      name_with_type: 'Huyện Sông Mã',
      path: 'Sông Mã, Sơn La',
      path_with_type:
        'Huyện Sông Mã, Tỉnh Sơn La',
      code: '126',
      parent_code: '14',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353d1',
      name: 'Sốp Cộp',
      type: 'huyen',
      slug: 'sop-cop',
      name_with_type: 'Huyện Sốp Cộp',
      path: 'Sốp Cộp, Sơn La',
      path_with_type:
        'Huyện Sốp Cộp, Tỉnh Sơn La',
      code: '127',
      parent_code: '14',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353d2',
      name: 'Vân Hồ',
      type: 'huyen',
      slug: 'van-ho',
      name_with_type: 'Huyện Vân Hồ',
      path: 'Vân Hồ, Sơn La',
      path_with_type: 'Huyện Vân Hồ, Tỉnh Sơn La',
      code: '128',
      parent_code: '14',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353d3',
      name: 'Yên Bái',
      type: 'thanh-pho',
      slug: 'yen-bai',
      name_with_type: 'Thành phố Yên Bái',
      path: 'Yên Bái, Yên Bái',
      path_with_type:
        'Thành phố Yên Bái, Tỉnh Yên Bái',
      code: '132',
      parent_code: '15',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353d4',
      name: 'Nghĩa Lộ',
      type: 'thi-xa',
      slug: 'nghia-lo',
      name_with_type: 'Thị xã Nghĩa Lộ',
      path: 'Nghĩa Lộ, Yên Bái',
      path_with_type:
        'Thị xã Nghĩa Lộ, Tỉnh Yên Bái',
      code: '133',
      parent_code: '15',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353d5',
      name: 'Lục Yên',
      type: 'huyen',
      slug: 'luc-yen',
      name_with_type: 'Huyện Lục Yên',
      path: 'Lục Yên, Yên Bái',
      path_with_type:
        'Huyện Lục Yên, Tỉnh Yên Bái',
      code: '135',
      parent_code: '15',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353d6',
      name: 'Văn Yên',
      type: 'huyen',
      slug: 'van-yen',
      name_with_type: 'Huyện Văn Yên',
      path: 'Văn Yên, Yên Bái',
      path_with_type:
        'Huyện Văn Yên, Tỉnh Yên Bái',
      code: '136',
      parent_code: '15',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353d7',
      name: 'Mù Căng Chải',
      type: 'huyen',
      slug: 'mu-cang-chai',
      name_with_type: 'Huyện Mù Căng Chải',
      path: 'Mù Căng Chải, Yên Bái',
      path_with_type:
        'Huyện Mù Căng Chải, Tỉnh Yên Bái',
      code: '137',
      parent_code: '15',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353d8',
      name: 'Trấn Yên',
      type: 'huyen',
      slug: 'tran-yen',
      name_with_type: 'Huyện Trấn Yên',
      path: 'Trấn Yên, Yên Bái',
      path_with_type:
        'Huyện Trấn Yên, Tỉnh Yên Bái',
      code: '138',
      parent_code: '15',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353d9',
      name: 'Trạm Tấu',
      type: 'huyen',
      slug: 'tram-tau',
      name_with_type: 'Huyện Trạm Tấu',
      path: 'Trạm Tấu, Yên Bái',
      path_with_type:
        'Huyện Trạm Tấu, Tỉnh Yên Bái',
      code: '139',
      parent_code: '15',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353da',
      name: 'Văn Chấn',
      type: 'huyen',
      slug: 'van-chan',
      name_with_type: 'Huyện Văn Chấn',
      path: 'Văn Chấn, Yên Bái',
      path_with_type:
        'Huyện Văn Chấn, Tỉnh Yên Bái',
      code: '140',
      parent_code: '15',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353db',
      name: 'Yên Bình',
      type: 'huyen',
      slug: 'yen-binh',
      name_with_type: 'Huyện Yên Bình',
      path: 'Yên Bình, Yên Bái',
      path_with_type:
        'Huyện Yên Bình, Tỉnh Yên Bái',
      code: '141',
      parent_code: '15',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353dc',
      name: 'Hòa Bình',
      type: 'thanh-pho',
      slug: 'hoa-binh',
      name_with_type: 'Thành phố Hòa Bình',
      path: 'Hòa Bình, Hoà Bình',
      path_with_type:
        'Thành phố Hòa Bình, Tỉnh Hoà Bình',
      code: '148',
      parent_code: '17',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353dd',
      name: 'Đà Bắc',
      type: 'huyen',
      slug: 'da-bac',
      name_with_type: 'Huyện Đà Bắc',
      path: 'Đà Bắc, Hoà Bình',
      path_with_type:
        'Huyện Đà Bắc, Tỉnh Hoà Bình',
      code: '150',
      parent_code: '17',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353de',
      name: 'Lương Sơn',
      type: 'huyen',
      slug: 'luong-son',
      name_with_type: 'Huyện Lương Sơn',
      path: 'Lương Sơn, Hoà Bình',
      path_with_type:
        'Huyện Lương Sơn, Tỉnh Hoà Bình',
      code: '152',
      parent_code: '17',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353df',
      name: 'Kim Bôi',
      type: 'huyen',
      slug: 'kim-boi',
      name_with_type: 'Huyện Kim Bôi',
      path: 'Kim Bôi, Hoà Bình',
      path_with_type:
        'Huyện Kim Bôi, Tỉnh Hoà Bình',
      code: '153',
      parent_code: '17',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353e0',
      name: 'Cao Phong',
      type: 'huyen',
      slug: 'cao-phong',
      name_with_type: 'Huyện Cao Phong',
      path: 'Cao Phong, Hoà Bình',
      path_with_type:
        'Huyện Cao Phong, Tỉnh Hoà Bình',
      code: '154',
      parent_code: '17',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353e1',
      name: 'Tân Lạc',
      type: 'huyen',
      slug: 'tan-lac',
      name_with_type: 'Huyện Tân Lạc',
      path: 'Tân Lạc, Hoà Bình',
      path_with_type:
        'Huyện Tân Lạc, Tỉnh Hoà Bình',
      code: '155',
      parent_code: '17',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353e2',
      name: 'Mai Châu',
      type: 'huyen',
      slug: 'mai-chau',
      name_with_type: 'Huyện Mai Châu',
      path: 'Mai Châu, Hoà Bình',
      path_with_type:
        'Huyện Mai Châu, Tỉnh Hoà Bình',
      code: '156',
      parent_code: '17',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353e3',
      name: 'Lạc Sơn',
      type: 'huyen',
      slug: 'lac-son',
      name_with_type: 'Huyện Lạc Sơn',
      path: 'Lạc Sơn, Hoà Bình',
      path_with_type:
        'Huyện Lạc Sơn, Tỉnh Hoà Bình',
      code: '157',
      parent_code: '17',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353e4',
      name: 'Yên Thủy',
      type: 'huyen',
      slug: 'yen-thuy',
      name_with_type: 'Huyện Yên Thủy',
      path: 'Yên Thủy, Hoà Bình',
      path_with_type:
        'Huyện Yên Thủy, Tỉnh Hoà Bình',
      code: '158',
      parent_code: '17',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353e5',
      name: 'Lạc Thủy',
      type: 'huyen',
      slug: 'lac-thuy',
      name_with_type: 'Huyện Lạc Thủy',
      path: 'Lạc Thủy, Hoà Bình',
      path_with_type:
        'Huyện Lạc Thủy, Tỉnh Hoà Bình',
      code: '159',
      parent_code: '17',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353e6',
      name: 'Thái Nguyên',
      type: 'thanh-pho',
      slug: 'thai-nguyen',
      name_with_type: 'Thành phố Thái Nguyên',
      path: 'Thái Nguyên, Thái Nguyên',
      path_with_type:
        'Thành phố Thái Nguyên, Tỉnh Thái Nguyên',
      code: '164',
      parent_code: '19',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353e7',
      name: 'Sông Công',
      type: 'thanh-pho',
      slug: 'song-cong',
      name_with_type: 'Thành phố Sông Công',
      path: 'Sông Công, Thái Nguyên',
      path_with_type:
        'Thành phố Sông Công, Tỉnh Thái Nguyên',
      code: '165',
      parent_code: '19',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353e8',
      name: 'Định Hóa',
      type: 'huyen',
      slug: 'dinh-hoa',
      name_with_type: 'Huyện Định Hóa',
      path: 'Định Hóa, Thái Nguyên',
      path_with_type:
        'Huyện Định Hóa, Tỉnh Thái Nguyên',
      code: '167',
      parent_code: '19',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353e9',
      name: 'Phú Lương',
      type: 'huyen',
      slug: 'phu-luong',
      name_with_type: 'Huyện Phú Lương',
      path: 'Phú Lương, Thái Nguyên',
      path_with_type:
        'Huyện Phú Lương, Tỉnh Thái Nguyên',
      code: '168',
      parent_code: '19',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353ea',
      name: 'Đồng Hỷ',
      type: 'huyen',
      slug: 'dong-hy',
      name_with_type: 'Huyện Đồng Hỷ',
      path: 'Đồng Hỷ, Thái Nguyên',
      path_with_type:
        'Huyện Đồng Hỷ, Tỉnh Thái Nguyên',
      code: '169',
      parent_code: '19',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353eb',
      name: 'Võ Nhai',
      type: 'huyen',
      slug: 'vo-nhai',
      name_with_type: 'Huyện Võ Nhai',
      path: 'Võ Nhai, Thái Nguyên',
      path_with_type:
        'Huyện Võ Nhai, Tỉnh Thái Nguyên',
      code: '170',
      parent_code: '19',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353ec',
      name: 'Đại Từ',
      type: 'huyen',
      slug: 'dai-tu',
      name_with_type: 'Huyện Đại Từ',
      path: 'Đại Từ, Thái Nguyên',
      path_with_type:
        'Huyện Đại Từ, Tỉnh Thái Nguyên',
      code: '171',
      parent_code: '19',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353ed',
      name: 'Phổ Yên',
      type: 'thi-xa',
      slug: 'pho-yen',
      name_with_type: 'Thị xã Phổ Yên',
      path: 'Phổ Yên, Thái Nguyên',
      path_with_type:
        'Thị xã Phổ Yên, Tỉnh Thái Nguyên',
      code: '172',
      parent_code: '19',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353ee',
      name: 'Phú Bình',
      type: 'huyen',
      slug: 'phu-binh',
      name_with_type: 'Huyện Phú Bình',
      path: 'Phú Bình, Thái Nguyên',
      path_with_type:
        'Huyện Phú Bình, Tỉnh Thái Nguyên',
      code: '173',
      parent_code: '19',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353ef',
      name: 'Lạng Sơn',
      type: 'thanh-pho',
      slug: 'lang-son',
      name_with_type: 'Thành phố Lạng Sơn',
      path: 'Lạng Sơn, Lạng Sơn',
      path_with_type:
        'Thành phố Lạng Sơn, Tỉnh Lạng Sơn',
      code: '178',
      parent_code: '20',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353f0',
      name: 'Tràng Định',
      type: 'huyen',
      slug: 'trang-dinh',
      name_with_type: 'Huyện Tràng Định',
      path: 'Tràng Định, Lạng Sơn',
      path_with_type:
        'Huyện Tràng Định, Tỉnh Lạng Sơn',
      code: '180',
      parent_code: '20',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353f1',
      name: 'Bình Gia',
      type: 'huyen',
      slug: 'binh-gia',
      name_with_type: 'Huyện Bình Gia',
      path: 'Bình Gia, Lạng Sơn',
      path_with_type:
        'Huyện Bình Gia, Tỉnh Lạng Sơn',
      code: '181',
      parent_code: '20',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353f2',
      name: 'Văn Lãng',
      type: 'huyen',
      slug: 'van-lang',
      name_with_type: 'Huyện Văn Lãng',
      path: 'Văn Lãng, Lạng Sơn',
      path_with_type:
        'Huyện Văn Lãng, Tỉnh Lạng Sơn',
      code: '182',
      parent_code: '20',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353f3',
      name: 'Cao Lộc',
      type: 'huyen',
      slug: 'cao-loc',
      name_with_type: 'Huyện Cao Lộc',
      path: 'Cao Lộc, Lạng Sơn',
      path_with_type:
        'Huyện Cao Lộc, Tỉnh Lạng Sơn',
      code: '183',
      parent_code: '20',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353f4',
      name: 'Văn Quan',
      type: 'huyen',
      slug: 'van-quan',
      name_with_type: 'Huyện Văn Quan',
      path: 'Văn Quan, Lạng Sơn',
      path_with_type:
        'Huyện Văn Quan, Tỉnh Lạng Sơn',
      code: '184',
      parent_code: '20',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353f5',
      name: 'Bắc Sơn',
      type: 'huyen',
      slug: 'bac-son',
      name_with_type: 'Huyện Bắc Sơn',
      path: 'Bắc Sơn, Lạng Sơn',
      path_with_type:
        'Huyện Bắc Sơn, Tỉnh Lạng Sơn',
      code: '185',
      parent_code: '20',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353f6',
      name: 'Hữu Lũng',
      type: 'huyen',
      slug: 'huu-lung',
      name_with_type: 'Huyện Hữu Lũng',
      path: 'Hữu Lũng, Lạng Sơn',
      path_with_type:
        'Huyện Hữu Lũng, Tỉnh Lạng Sơn',
      code: '186',
      parent_code: '20',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353f7',
      name: 'Chi Lăng',
      type: 'huyen',
      slug: 'chi-lang',
      name_with_type: 'Huyện Chi Lăng',
      path: 'Chi Lăng, Lạng Sơn',
      path_with_type:
        'Huyện Chi Lăng, Tỉnh Lạng Sơn',
      code: '187',
      parent_code: '20',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353f8',
      name: 'Lộc Bình',
      type: 'huyen',
      slug: 'loc-binh',
      name_with_type: 'Huyện Lộc Bình',
      path: 'Lộc Bình, Lạng Sơn',
      path_with_type:
        'Huyện Lộc Bình, Tỉnh Lạng Sơn',
      code: '188',
      parent_code: '20',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353f9',
      name: 'Đình Lập',
      type: 'huyen',
      slug: 'dinh-lap',
      name_with_type: 'Huyện Đình Lập',
      path: 'Đình Lập, Lạng Sơn',
      path_with_type:
        'Huyện Đình Lập, Tỉnh Lạng Sơn',
      code: '189',
      parent_code: '20',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353fa',
      name: 'Hạ Long',
      type: 'thanh-pho',
      slug: 'ha-long',
      name_with_type: 'Thành phố Hạ Long',
      path: 'Hạ Long, Quảng Ninh',
      path_with_type:
        'Thành phố Hạ Long, Tỉnh Quảng Ninh',
      code: '193',
      parent_code: '22',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353fb',
      name: 'Móng Cái',
      type: 'thanh-pho',
      slug: 'mong-cai',
      name_with_type: 'Thành phố Móng Cái',
      path: 'Móng Cái, Quảng Ninh',
      path_with_type:
        'Thành phố Móng Cái, Tỉnh Quảng Ninh',
      code: '194',
      parent_code: '22',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353fc',
      name: 'Cẩm Phả',
      type: 'thanh-pho',
      slug: 'cam-pha',
      name_with_type: 'Thành phố Cẩm Phả',
      path: 'Cẩm Phả, Quảng Ninh',
      path_with_type:
        'Thành phố Cẩm Phả, Tỉnh Quảng Ninh',
      code: '195',
      parent_code: '22',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353fd',
      name: 'Uông Bí',
      type: 'thanh-pho',
      slug: 'uong-bi',
      name_with_type: 'Thành phố Uông Bí',
      path: 'Uông Bí, Quảng Ninh',
      path_with_type:
        'Thành phố Uông Bí, Tỉnh Quảng Ninh',
      code: '196',
      parent_code: '22',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353fe',
      name: 'Bình Liêu',
      type: 'huyen',
      slug: 'binh-lieu',
      name_with_type: 'Huyện Bình Liêu',
      path: 'Bình Liêu, Quảng Ninh',
      path_with_type:
        'Huyện Bình Liêu, Tỉnh Quảng Ninh',
      code: '198',
      parent_code: '22',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c353ff',
      name: 'Tiên Yên',
      type: 'huyen',
      slug: 'tien-yen',
      name_with_type: 'Huyện Tiên Yên',
      path: 'Tiên Yên, Quảng Ninh',
      path_with_type:
        'Huyện Tiên Yên, Tỉnh Quảng Ninh',
      code: '199',
      parent_code: '22',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35400',
      name: 'Đầm Hà',
      type: 'huyen',
      slug: 'dam-ha',
      name_with_type: 'Huyện Đầm Hà',
      path: 'Đầm Hà, Quảng Ninh',
      path_with_type:
        'Huyện Đầm Hà, Tỉnh Quảng Ninh',
      code: '200',
      parent_code: '22',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35401',
      name: 'Hải Hà',
      type: 'huyen',
      slug: 'hai-ha',
      name_with_type: 'Huyện Hải Hà',
      path: 'Hải Hà, Quảng Ninh',
      path_with_type:
        'Huyện Hải Hà, Tỉnh Quảng Ninh',
      code: '201',
      parent_code: '22',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35402',
      name: 'Ba Chẽ',
      type: 'huyen',
      slug: 'ba-che',
      name_with_type: 'Huyện Ba Chẽ',
      path: 'Ba Chẽ, Quảng Ninh',
      path_with_type:
        'Huyện Ba Chẽ, Tỉnh Quảng Ninh',
      code: '202',
      parent_code: '22',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35403',
      name: 'Vân Đồn',
      type: 'huyen',
      slug: 'van-don',
      name_with_type: 'Huyện Vân Đồn',
      path: 'Vân Đồn, Quảng Ninh',
      path_with_type:
        'Huyện Vân Đồn, Tỉnh Quảng Ninh',
      code: '203',
      parent_code: '22',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35404',
      name: 'Đông Triều',
      type: 'thi-xa',
      slug: 'dong-trieu',
      name_with_type: 'Thị xã Đông Triều',
      path: 'Đông Triều, Quảng Ninh',
      path_with_type:
        'Thị xã Đông Triều, Tỉnh Quảng Ninh',
      code: '205',
      parent_code: '22',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35405',
      name: 'Quảng Yên',
      type: 'thi-xa',
      slug: 'quang-yen',
      name_with_type: 'Thị xã Quảng Yên',
      path: 'Quảng Yên, Quảng Ninh',
      path_with_type:
        'Thị xã Quảng Yên, Tỉnh Quảng Ninh',
      code: '206',
      parent_code: '22',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35406',
      name: 'Cô Tô',
      type: 'huyen',
      slug: 'co-to',
      name_with_type: 'Huyện Cô Tô',
      path: 'Cô Tô, Quảng Ninh',
      path_with_type:
        'Huyện Cô Tô, Tỉnh Quảng Ninh',
      code: '207',
      parent_code: '22',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35407',
      name: 'Bắc Giang',
      type: 'thanh-pho',
      slug: 'bac-giang',
      name_with_type: 'Thành phố Bắc Giang',
      path: 'Bắc Giang, Bắc Giang',
      path_with_type:
        'Thành phố Bắc Giang, Tỉnh Bắc Giang',
      code: '213',
      parent_code: '24',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35408',
      name: 'Yên Thế',
      type: 'huyen',
      slug: 'yen-the',
      name_with_type: 'Huyện Yên Thế',
      path: 'Yên Thế, Bắc Giang',
      path_with_type:
        'Huyện Yên Thế, Tỉnh Bắc Giang',
      code: '215',
      parent_code: '24',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35409',
      name: 'Tân Yên',
      type: 'huyen',
      slug: 'tan-yen',
      name_with_type: 'Huyện Tân Yên',
      path: 'Tân Yên, Bắc Giang',
      path_with_type:
        'Huyện Tân Yên, Tỉnh Bắc Giang',
      code: '216',
      parent_code: '24',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3540a',
      name: 'Lạng Giang',
      type: 'huyen',
      slug: 'lang-giang',
      name_with_type: 'Huyện Lạng Giang',
      path: 'Lạng Giang, Bắc Giang',
      path_with_type:
        'Huyện Lạng Giang, Tỉnh Bắc Giang',
      code: '217',
      parent_code: '24',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3540b',
      name: 'Lục Nam',
      type: 'huyen',
      slug: 'luc-nam',
      name_with_type: 'Huyện Lục Nam',
      path: 'Lục Nam, Bắc Giang',
      path_with_type:
        'Huyện Lục Nam, Tỉnh Bắc Giang',
      code: '218',
      parent_code: '24',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3540c',
      name: 'Lục Ngạn',
      type: 'huyen',
      slug: 'luc-ngan',
      name_with_type: 'Huyện Lục Ngạn',
      path: 'Lục Ngạn, Bắc Giang',
      path_with_type:
        'Huyện Lục Ngạn, Tỉnh Bắc Giang',
      code: '219',
      parent_code: '24',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3540d',
      name: 'Sơn Động',
      type: 'huyen',
      slug: 'son-dong',
      name_with_type: 'Huyện Sơn Động',
      path: 'Sơn Động, Bắc Giang',
      path_with_type:
        'Huyện Sơn Động, Tỉnh Bắc Giang',
      code: '220',
      parent_code: '24',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3540e',
      name: 'Yên Dũng',
      type: 'huyen',
      slug: 'yen-dung',
      name_with_type: 'Huyện Yên Dũng',
      path: 'Yên Dũng, Bắc Giang',
      path_with_type:
        'Huyện Yên Dũng, Tỉnh Bắc Giang',
      code: '221',
      parent_code: '24',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3540f',
      name: 'Việt Yên',
      type: 'huyen',
      slug: 'viet-yen',
      name_with_type: 'Huyện Việt Yên',
      path: 'Việt Yên, Bắc Giang',
      path_with_type:
        'Huyện Việt Yên, Tỉnh Bắc Giang',
      code: '222',
      parent_code: '24',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35410',
      name: 'Hiệp Hòa',
      type: 'huyen',
      slug: 'hiep-hoa',
      name_with_type: 'Huyện Hiệp Hòa',
      path: 'Hiệp Hòa, Bắc Giang',
      path_with_type:
        'Huyện Hiệp Hòa, Tỉnh Bắc Giang',
      code: '223',
      parent_code: '24',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35411',
      name: 'Việt Trì',
      type: 'thanh-pho',
      slug: 'viet-tri',
      name_with_type: 'Thành phố Việt Trì',
      path: 'Việt Trì, Phú Thọ',
      path_with_type:
        'Thành phố Việt Trì, Tỉnh Phú Thọ',
      code: '227',
      parent_code: '25',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35412',
      name: 'Phú Thọ',
      type: 'thi-xa',
      slug: 'phu-tho',
      name_with_type: 'Thị xã Phú Thọ',
      path: 'Phú Thọ, Phú Thọ',
      path_with_type:
        'Thị xã Phú Thọ, Tỉnh Phú Thọ',
      code: '228',
      parent_code: '25',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35413',
      name: 'Đoan Hùng',
      type: 'huyen',
      slug: 'doan-hung',
      name_with_type: 'Huyện Đoan Hùng',
      path: 'Đoan Hùng, Phú Thọ',
      path_with_type:
        'Huyện Đoan Hùng, Tỉnh Phú Thọ',
      code: '230',
      parent_code: '25',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35414',
      name: 'Hạ Hoà',
      type: 'huyen',
      slug: 'ha-hoa',
      name_with_type: 'Huyện Hạ Hoà',
      path: 'Hạ Hoà, Phú Thọ',
      path_with_type:
        'Huyện Hạ Hoà, Tỉnh Phú Thọ',
      code: '231',
      parent_code: '25',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35415',
      name: 'Thanh Ba',
      type: 'huyen',
      slug: 'thanh-ba',
      name_with_type: 'Huyện Thanh Ba',
      path: 'Thanh Ba, Phú Thọ',
      path_with_type:
        'Huyện Thanh Ba, Tỉnh Phú Thọ',
      code: '232',
      parent_code: '25',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35416',
      name: 'Phù Ninh',
      type: 'huyen',
      slug: 'phu-ninh',
      name_with_type: 'Huyện Phù Ninh',
      path: 'Phù Ninh, Phú Thọ',
      path_with_type:
        'Huyện Phù Ninh, Tỉnh Phú Thọ',
      code: '233',
      parent_code: '25',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35417',
      name: 'Yên Lập',
      type: 'huyen',
      slug: 'yen-lap',
      name_with_type: 'Huyện Yên Lập',
      path: 'Yên Lập, Phú Thọ',
      path_with_type:
        'Huyện Yên Lập, Tỉnh Phú Thọ',
      code: '234',
      parent_code: '25',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35418',
      name: 'Cẩm Khê',
      type: 'huyen',
      slug: 'cam-khe',
      name_with_type: 'Huyện Cẩm Khê',
      path: 'Cẩm Khê, Phú Thọ',
      path_with_type:
        'Huyện Cẩm Khê, Tỉnh Phú Thọ',
      code: '235',
      parent_code: '25',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35419',
      name: 'Tam Nông',
      type: 'huyen',
      slug: 'tam-nong',
      name_with_type: 'Huyện Tam Nông',
      path: 'Tam Nông, Phú Thọ',
      path_with_type:
        'Huyện Tam Nông, Tỉnh Phú Thọ',
      code: '236',
      parent_code: '25',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3541a',
      name: 'Lâm Thao',
      type: 'huyen',
      slug: 'lam-thao',
      name_with_type: 'Huyện Lâm Thao',
      path: 'Lâm Thao, Phú Thọ',
      path_with_type:
        'Huyện Lâm Thao, Tỉnh Phú Thọ',
      code: '237',
      parent_code: '25',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3541b',
      name: 'Thanh Sơn',
      type: 'huyen',
      slug: 'thanh-son',
      name_with_type: 'Huyện Thanh Sơn',
      path: 'Thanh Sơn, Phú Thọ',
      path_with_type:
        'Huyện Thanh Sơn, Tỉnh Phú Thọ',
      code: '238',
      parent_code: '25',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3541c',
      name: 'Thanh Thuỷ',
      type: 'huyen',
      slug: 'thanh-thuy',
      name_with_type: 'Huyện Thanh Thuỷ',
      path: 'Thanh Thuỷ, Phú Thọ',
      path_with_type:
        'Huyện Thanh Thuỷ, Tỉnh Phú Thọ',
      code: '239',
      parent_code: '25',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3541d',
      name: 'Tân Sơn',
      type: 'huyen',
      slug: 'tan-son',
      name_with_type: 'Huyện Tân Sơn',
      path: 'Tân Sơn, Phú Thọ',
      path_with_type:
        'Huyện Tân Sơn, Tỉnh Phú Thọ',
      code: '240',
      parent_code: '25',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3541e',
      name: 'Vĩnh Yên',
      type: 'thanh-pho',
      slug: 'vinh-yen',
      name_with_type: 'Thành phố Vĩnh Yên',
      path: 'Vĩnh Yên, Vĩnh Phúc',
      path_with_type:
        'Thành phố Vĩnh Yên, Tỉnh Vĩnh Phúc',
      code: '243',
      parent_code: '26',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3541f',
      name: 'Phúc Yên',
      type: 'thanh-pho',
      slug: 'phuc-yen',
      name_with_type: 'Thành phố Phúc Yên',
      path: 'Phúc Yên, Vĩnh Phúc',
      path_with_type:
        'Thành phố Phúc Yên, Tỉnh Vĩnh Phúc',
      code: '244',
      parent_code: '26',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35420',
      name: 'Lập Thạch',
      type: 'huyen',
      slug: 'lap-thach',
      name_with_type: 'Huyện Lập Thạch',
      path: 'Lập Thạch, Vĩnh Phúc',
      path_with_type:
        'Huyện Lập Thạch, Tỉnh Vĩnh Phúc',
      code: '246',
      parent_code: '26',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35421',
      name: 'Tam Dương',
      type: 'huyen',
      slug: 'tam-duong',
      name_with_type: 'Huyện Tam Dương',
      path: 'Tam Dương, Vĩnh Phúc',
      path_with_type:
        'Huyện Tam Dương, Tỉnh Vĩnh Phúc',
      code: '247',
      parent_code: '26',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35422',
      name: 'Tam Đảo',
      type: 'huyen',
      slug: 'tam-dao',
      name_with_type: 'Huyện Tam Đảo',
      path: 'Tam Đảo, Vĩnh Phúc',
      path_with_type:
        'Huyện Tam Đảo, Tỉnh Vĩnh Phúc',
      code: '248',
      parent_code: '26',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35423',
      name: 'Bình Xuyên',
      type: 'huyen',
      slug: 'binh-xuyen',
      name_with_type: 'Huyện Bình Xuyên',
      path: 'Bình Xuyên, Vĩnh Phúc',
      path_with_type:
        'Huyện Bình Xuyên, Tỉnh Vĩnh Phúc',
      code: '249',
      parent_code: '26',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35424',
      name: 'Yên Lạc',
      type: 'huyen',
      slug: 'yen-lac',
      name_with_type: 'Huyện Yên Lạc',
      path: 'Yên Lạc, Vĩnh Phúc',
      path_with_type:
        'Huyện Yên Lạc, Tỉnh Vĩnh Phúc',
      code: '251',
      parent_code: '26',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35425',
      name: 'Vĩnh Tường',
      type: 'huyen',
      slug: 'vinh-tuong',
      name_with_type: 'Huyện Vĩnh Tường',
      path: 'Vĩnh Tường, Vĩnh Phúc',
      path_with_type:
        'Huyện Vĩnh Tường, Tỉnh Vĩnh Phúc',
      code: '252',
      parent_code: '26',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35426',
      name: 'Sông Lô',
      type: 'huyen',
      slug: 'song-lo',
      name_with_type: 'Huyện Sông Lô',
      path: 'Sông Lô, Vĩnh Phúc',
      path_with_type:
        'Huyện Sông Lô, Tỉnh Vĩnh Phúc',
      code: '253',
      parent_code: '26',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35427',
      name: 'Bắc Ninh',
      type: 'thanh-pho',
      slug: 'bac-ninh',
      name_with_type: 'Thành phố Bắc Ninh',
      path: 'Bắc Ninh, Bắc Ninh',
      path_with_type:
        'Thành phố Bắc Ninh, Tỉnh Bắc Ninh',
      code: '256',
      parent_code: '27',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35428',
      name: 'Yên Phong',
      type: 'huyen',
      slug: 'yen-phong',
      name_with_type: 'Huyện Yên Phong',
      path: 'Yên Phong, Bắc Ninh',
      path_with_type:
        'Huyện Yên Phong, Tỉnh Bắc Ninh',
      code: '258',
      parent_code: '27',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35429',
      name: 'Quế Võ',
      type: 'huyen',
      slug: 'que-vo',
      name_with_type: 'Huyện Quế Võ',
      path: 'Quế Võ, Bắc Ninh',
      path_with_type:
        'Huyện Quế Võ, Tỉnh Bắc Ninh',
      code: '259',
      parent_code: '27',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3542a',
      name: 'Tiên Du',
      type: 'huyen',
      slug: 'tien-du',
      name_with_type: 'Huyện Tiên Du',
      path: 'Tiên Du, Bắc Ninh',
      path_with_type:
        'Huyện Tiên Du, Tỉnh Bắc Ninh',
      code: '260',
      parent_code: '27',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3542b',
      name: 'Từ Sơn',
      type: 'thi-xa',
      slug: 'tu-son',
      name_with_type: 'Thị xã Từ Sơn',
      path: 'Từ Sơn, Bắc Ninh',
      path_with_type:
        'Thị xã Từ Sơn, Tỉnh Bắc Ninh',
      code: '261',
      parent_code: '27',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3542c',
      name: 'Thuận Thành',
      type: 'huyen',
      slug: 'thuan-thanh',
      name_with_type: 'Huyện Thuận Thành',
      path: 'Thuận Thành, Bắc Ninh',
      path_with_type:
        'Huyện Thuận Thành, Tỉnh Bắc Ninh',
      code: '262',
      parent_code: '27',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3542d',
      name: 'Gia Bình',
      type: 'huyen',
      slug: 'gia-binh',
      name_with_type: 'Huyện Gia Bình',
      path: 'Gia Bình, Bắc Ninh',
      path_with_type:
        'Huyện Gia Bình, Tỉnh Bắc Ninh',
      code: '263',
      parent_code: '27',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3542e',
      name: 'Lương Tài',
      type: 'huyen',
      slug: 'luong-tai',
      name_with_type: 'Huyện Lương Tài',
      path: 'Lương Tài, Bắc Ninh',
      path_with_type:
        'Huyện Lương Tài, Tỉnh Bắc Ninh',
      code: '264',
      parent_code: '27',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3542f',
      name: 'Hải Dương',
      type: 'thanh-pho',
      slug: 'hai-duong',
      name_with_type: 'Thành phố Hải Dương',
      path: 'Hải Dương, Hải Dương',
      path_with_type:
        'Thành phố Hải Dương, Tỉnh Hải Dương',
      code: '288',
      parent_code: '30',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35430',
      name: 'Chí Linh',
      type: 'thanh-pho',
      slug: 'chi-linh',
      name_with_type: 'Thành phố Chí Linh',
      path: 'Chí Linh, Hải Dương',
      path_with_type:
        'Thành phố Chí Linh, Tỉnh Hải Dương',
      code: '290',
      parent_code: '30',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35431',
      name: 'Nam Sách',
      type: 'huyen',
      slug: 'nam-sach',
      name_with_type: 'Huyện Nam Sách',
      path: 'Nam Sách, Hải Dương',
      path_with_type:
        'Huyện Nam Sách, Tỉnh Hải Dương',
      code: '291',
      parent_code: '30',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35432',
      name: 'Kinh Môn',
      type: 'thi-xa',
      slug: 'kinh-mon',
      name_with_type: 'Thị xã Kinh Môn',
      path: 'Kinh Môn, Hải Dương',
      path_with_type:
        'Thị xã Kinh Môn, Tỉnh Hải Dương',
      code: '292',
      parent_code: '30',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35433',
      name: 'Kim Thành',
      type: 'huyen',
      slug: 'kim-thanh',
      name_with_type: 'Huyện Kim Thành',
      path: 'Kim Thành, Hải Dương',
      path_with_type:
        'Huyện Kim Thành, Tỉnh Hải Dương',
      code: '293',
      parent_code: '30',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35434',
      name: 'Thanh Hà',
      type: 'huyen',
      slug: 'thanh-ha',
      name_with_type: 'Huyện Thanh Hà',
      path: 'Thanh Hà, Hải Dương',
      path_with_type:
        'Huyện Thanh Hà, Tỉnh Hải Dương',
      code: '294',
      parent_code: '30',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35435',
      name: 'Cẩm Giàng',
      type: 'huyen',
      slug: 'cam-giang',
      name_with_type: 'Huyện Cẩm Giàng',
      path: 'Cẩm Giàng, Hải Dương',
      path_with_type:
        'Huyện Cẩm Giàng, Tỉnh Hải Dương',
      code: '295',
      parent_code: '30',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35436',
      name: 'Bình Giang',
      type: 'huyen',
      slug: 'binh-giang',
      name_with_type: 'Huyện Bình Giang',
      path: 'Bình Giang, Hải Dương',
      path_with_type:
        'Huyện Bình Giang, Tỉnh Hải Dương',
      code: '296',
      parent_code: '30',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35437',
      name: 'Gia Lộc',
      type: 'huyen',
      slug: 'gia-loc',
      name_with_type: 'Huyện Gia Lộc',
      path: 'Gia Lộc, Hải Dương',
      path_with_type:
        'Huyện Gia Lộc, Tỉnh Hải Dương',
      code: '297',
      parent_code: '30',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35438',
      name: 'Tứ Kỳ',
      type: 'huyen',
      slug: 'tu-ky',
      name_with_type: 'Huyện Tứ Kỳ',
      path: 'Tứ Kỳ, Hải Dương',
      path_with_type:
        'Huyện Tứ Kỳ, Tỉnh Hải Dương',
      code: '298',
      parent_code: '30',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35439',
      name: 'Ninh Giang',
      type: 'huyen',
      slug: 'ninh-giang',
      name_with_type: 'Huyện Ninh Giang',
      path: 'Ninh Giang, Hải Dương',
      path_with_type:
        'Huyện Ninh Giang, Tỉnh Hải Dương',
      code: '299',
      parent_code: '30',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3543a',
      name: 'Thanh Miện',
      type: 'huyen',
      slug: 'thanh-mien',
      name_with_type: 'Huyện Thanh Miện',
      path: 'Thanh Miện, Hải Dương',
      path_with_type:
        'Huyện Thanh Miện, Tỉnh Hải Dương',
      code: '300',
      parent_code: '30',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3543b',
      name: 'Hồng Bàng',
      type: 'quan',
      slug: 'hong-bang',
      name_with_type: 'Quận Hồng Bàng',
      path: 'Hồng Bàng, Hải Phòng',
      path_with_type:
        'Quận Hồng Bàng, Thành phố Hải Phòng',
      code: '303',
      parent_code: '31',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3543c',
      name: 'Ngô Quyền',
      type: 'quan',
      slug: 'ngo-quyen',
      name_with_type: 'Quận Ngô Quyền',
      path: 'Ngô Quyền, Hải Phòng',
      path_with_type:
        'Quận Ngô Quyền, Thành phố Hải Phòng',
      code: '304',
      parent_code: '31',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3543d',
      name: 'Lê Chân',
      type: 'quan',
      slug: 'le-chan',
      name_with_type: 'Quận Lê Chân',
      path: 'Lê Chân, Hải Phòng',
      path_with_type:
        'Quận Lê Chân, Thành phố Hải Phòng',
      code: '305',
      parent_code: '31',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3543e',
      name: 'Hải An',
      type: 'quan',
      slug: 'hai-an',
      name_with_type: 'Quận Hải An',
      path: 'Hải An, Hải Phòng',
      path_with_type:
        'Quận Hải An, Thành phố Hải Phòng',
      code: '306',
      parent_code: '31',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3543f',
      name: 'Kiến An',
      type: 'quan',
      slug: 'kien-an',
      name_with_type: 'Quận Kiến An',
      path: 'Kiến An, Hải Phòng',
      path_with_type:
        'Quận Kiến An, Thành phố Hải Phòng',
      code: '307',
      parent_code: '31',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35440',
      name: 'Đồ Sơn',
      type: 'quan',
      slug: 'do-son',
      name_with_type: 'Quận Đồ Sơn',
      path: 'Đồ Sơn, Hải Phòng',
      path_with_type:
        'Quận Đồ Sơn, Thành phố Hải Phòng',
      code: '308',
      parent_code: '31',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35441',
      name: 'Dương Kinh',
      type: 'quan',
      slug: 'duong-kinh',
      name_with_type: 'Quận Dương Kinh',
      path: 'Dương Kinh, Hải Phòng',
      path_with_type:
        'Quận Dương Kinh, Thành phố Hải Phòng',
      code: '309',
      parent_code: '31',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35442',
      name: 'Thuỷ Nguyên',
      type: 'huyen',
      slug: 'thuy-nguyen',
      name_with_type: 'Huyện Thuỷ Nguyên',
      path: 'Thuỷ Nguyên, Hải Phòng',
      path_with_type:
        'Huyện Thuỷ Nguyên, Thành phố Hải Phòng',
      code: '311',
      parent_code: '31',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35443',
      name: 'An Dương',
      type: 'huyen',
      slug: 'an-duong',
      name_with_type: 'Huyện An Dương',
      path: 'An Dương, Hải Phòng',
      path_with_type:
        'Huyện An Dương, Thành phố Hải Phòng',
      code: '312',
      parent_code: '31',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35444',
      name: 'An Lão',
      type: 'huyen',
      slug: 'an-lao',
      name_with_type: 'Huyện An Lão',
      path: 'An Lão, Hải Phòng',
      path_with_type:
        'Huyện An Lão, Thành phố Hải Phòng',
      code: '313',
      parent_code: '31',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35445',
      name: 'Kiến Thuỵ',
      type: 'huyen',
      slug: 'kien-thuy',
      name_with_type: 'Huyện Kiến Thuỵ',
      path: 'Kiến Thuỵ, Hải Phòng',
      path_with_type:
        'Huyện Kiến Thuỵ, Thành phố Hải Phòng',
      code: '314',
      parent_code: '31',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35446',
      name: 'Tiên Lãng',
      type: 'huyen',
      slug: 'tien-lang',
      name_with_type: 'Huyện Tiên Lãng',
      path: 'Tiên Lãng, Hải Phòng',
      path_with_type:
        'Huyện Tiên Lãng, Thành phố Hải Phòng',
      code: '315',
      parent_code: '31',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35447',
      name: 'Vĩnh Bảo',
      type: 'huyen',
      slug: 'vinh-bao',
      name_with_type: 'Huyện Vĩnh Bảo',
      path: 'Vĩnh Bảo, Hải Phòng',
      path_with_type:
        'Huyện Vĩnh Bảo, Thành phố Hải Phòng',
      code: '316',
      parent_code: '31',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35448',
      name: 'Cát Hải',
      type: 'huyen',
      slug: 'cat-hai',
      name_with_type: 'Huyện Cát Hải',
      path: 'Cát Hải, Hải Phòng',
      path_with_type:
        'Huyện Cát Hải, Thành phố Hải Phòng',
      code: '317',
      parent_code: '31',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35449',
      name: 'Bạch Long Vĩ',
      type: 'huyen',
      slug: 'bach-long-vi',
      name_with_type: 'Huyện Bạch Long Vĩ',
      path: 'Bạch Long Vĩ, Hải Phòng',
      path_with_type:
        'Huyện Bạch Long Vĩ, Thành phố Hải Phòng',
      code: '318',
      parent_code: '31',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3544a',
      name: 'Hưng Yên',
      type: 'thanh-pho',
      slug: 'hung-yen',
      name_with_type: 'Thành phố Hưng Yên',
      path: 'Hưng Yên, Hưng Yên',
      path_with_type:
        'Thành phố Hưng Yên, Tỉnh Hưng Yên',
      code: '323',
      parent_code: '33',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3544b',
      name: 'Văn Lâm',
      type: 'huyen',
      slug: 'van-lam',
      name_with_type: 'Huyện Văn Lâm',
      path: 'Văn Lâm, Hưng Yên',
      path_with_type:
        'Huyện Văn Lâm, Tỉnh Hưng Yên',
      code: '325',
      parent_code: '33',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3544c',
      name: 'Văn Giang',
      type: 'huyen',
      slug: 'van-giang',
      name_with_type: 'Huyện Văn Giang',
      path: 'Văn Giang, Hưng Yên',
      path_with_type:
        'Huyện Văn Giang, Tỉnh Hưng Yên',
      code: '326',
      parent_code: '33',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3544d',
      name: 'Yên Mỹ',
      type: 'huyen',
      slug: 'yen-my',
      name_with_type: 'Huyện Yên Mỹ',
      path: 'Yên Mỹ, Hưng Yên',
      path_with_type:
        'Huyện Yên Mỹ, Tỉnh Hưng Yên',
      code: '327',
      parent_code: '33',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3544e',
      name: 'Mỹ Hào',
      type: 'thi-xa',
      slug: 'my-hao',
      name_with_type: 'Thị xã Mỹ Hào',
      path: 'Mỹ Hào, Hưng Yên',
      path_with_type:
        'Thị xã Mỹ Hào, Tỉnh Hưng Yên',
      code: '328',
      parent_code: '33',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3544f',
      name: 'Ân Thi',
      type: 'huyen',
      slug: 'an-thi',
      name_with_type: 'Huyện Ân Thi',
      path: 'Ân Thi, Hưng Yên',
      path_with_type:
        'Huyện Ân Thi, Tỉnh Hưng Yên',
      code: '329',
      parent_code: '33',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35450',
      name: 'Khoái Châu',
      type: 'huyen',
      slug: 'khoai-chau',
      name_with_type: 'Huyện Khoái Châu',
      path: 'Khoái Châu, Hưng Yên',
      path_with_type:
        'Huyện Khoái Châu, Tỉnh Hưng Yên',
      code: '330',
      parent_code: '33',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35451',
      name: 'Kim Động',
      type: 'huyen',
      slug: 'kim-dong',
      name_with_type: 'Huyện Kim Động',
      path: 'Kim Động, Hưng Yên',
      path_with_type:
        'Huyện Kim Động, Tỉnh Hưng Yên',
      code: '331',
      parent_code: '33',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35452',
      name: 'Tiên Lữ',
      type: 'huyen',
      slug: 'tien-lu',
      name_with_type: 'Huyện Tiên Lữ',
      path: 'Tiên Lữ, Hưng Yên',
      path_with_type:
        'Huyện Tiên Lữ, Tỉnh Hưng Yên',
      code: '332',
      parent_code: '33',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35453',
      name: 'Phù Cừ',
      type: 'huyen',
      slug: 'phu-cu',
      name_with_type: 'Huyện Phù Cừ',
      path: 'Phù Cừ, Hưng Yên',
      path_with_type:
        'Huyện Phù Cừ, Tỉnh Hưng Yên',
      code: '333',
      parent_code: '33',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35454',
      name: 'Thái Bình',
      type: 'thanh-pho',
      slug: 'thai-binh',
      name_with_type: 'Thành phố Thái Bình',
      path: 'Thái Bình, Thái Bình',
      path_with_type:
        'Thành phố Thái Bình, Tỉnh Thái Bình',
      code: '336',
      parent_code: '34',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35455',
      name: 'Quỳnh Phụ',
      type: 'huyen',
      slug: 'quynh-phu',
      name_with_type: 'Huyện Quỳnh Phụ',
      path: 'Quỳnh Phụ, Thái Bình',
      path_with_type:
        'Huyện Quỳnh Phụ, Tỉnh Thái Bình',
      code: '338',
      parent_code: '34',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35456',
      name: 'Hưng Hà',
      type: 'huyen',
      slug: 'hung-ha',
      name_with_type: 'Huyện Hưng Hà',
      path: 'Hưng Hà, Thái Bình',
      path_with_type:
        'Huyện Hưng Hà, Tỉnh Thái Bình',
      code: '339',
      parent_code: '34',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35457',
      name: 'Đông Hưng',
      type: 'huyen',
      slug: 'dong-hung',
      name_with_type: 'Huyện Đông Hưng',
      path: 'Đông Hưng, Thái Bình',
      path_with_type:
        'Huyện Đông Hưng, Tỉnh Thái Bình',
      code: '340',
      parent_code: '34',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35458',
      name: 'Thái Thụy',
      type: 'huyen',
      slug: 'thai-thuy',
      name_with_type: 'Huyện Thái Thụy',
      path: 'Thái Thụy, Thái Bình',
      path_with_type:
        'Huyện Thái Thụy, Tỉnh Thái Bình',
      code: '341',
      parent_code: '34',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35459',
      name: 'Tiền Hải',
      type: 'huyen',
      slug: 'tien-hai',
      name_with_type: 'Huyện Tiền Hải',
      path: 'Tiền Hải, Thái Bình',
      path_with_type:
        'Huyện Tiền Hải, Tỉnh Thái Bình',
      code: '342',
      parent_code: '34',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3545a',
      name: 'Kiến Xương',
      type: 'huyen',
      slug: 'kien-xuong',
      name_with_type: 'Huyện Kiến Xương',
      path: 'Kiến Xương, Thái Bình',
      path_with_type:
        'Huyện Kiến Xương, Tỉnh Thái Bình',
      code: '343',
      parent_code: '34',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3545b',
      name: 'Vũ Thư',
      type: 'huyen',
      slug: 'vu-thu',
      name_with_type: 'Huyện Vũ Thư',
      path: 'Vũ Thư, Thái Bình',
      path_with_type:
        'Huyện Vũ Thư, Tỉnh Thái Bình',
      code: '344',
      parent_code: '34',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3545c',
      name: 'Phủ Lý',
      type: 'thanh-pho',
      slug: 'phu-ly',
      name_with_type: 'Thành phố Phủ Lý',
      path: 'Phủ Lý, Hà Nam',
      path_with_type:
        'Thành phố Phủ Lý, Tỉnh Hà Nam',
      code: '347',
      parent_code: '35',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3545d',
      name: 'Duy Tiên',
      type: 'thi-xa',
      slug: 'duy-tien',
      name_with_type: 'Thị xã Duy Tiên',
      path: 'Duy Tiên, Hà Nam',
      path_with_type:
        'Thị xã Duy Tiên, Tỉnh Hà Nam',
      code: '349',
      parent_code: '35',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3545e',
      name: 'Kim Bảng',
      type: 'huyen',
      slug: 'kim-bang',
      name_with_type: 'Huyện Kim Bảng',
      path: 'Kim Bảng, Hà Nam',
      path_with_type:
        'Huyện Kim Bảng, Tỉnh Hà Nam',
      code: '350',
      parent_code: '35',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3545f',
      name: 'Thanh Liêm',
      type: 'huyen',
      slug: 'thanh-liem',
      name_with_type: 'Huyện Thanh Liêm',
      path: 'Thanh Liêm, Hà Nam',
      path_with_type:
        'Huyện Thanh Liêm, Tỉnh Hà Nam',
      code: '351',
      parent_code: '35',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35460',
      name: 'Bình Lục',
      type: 'huyen',
      slug: 'binh-luc',
      name_with_type: 'Huyện Bình Lục',
      path: 'Bình Lục, Hà Nam',
      path_with_type:
        'Huyện Bình Lục, Tỉnh Hà Nam',
      code: '352',
      parent_code: '35',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35461',
      name: 'Lý Nhân',
      type: 'huyen',
      slug: 'ly-nhan',
      name_with_type: 'Huyện Lý Nhân',
      path: 'Lý Nhân, Hà Nam',
      path_with_type:
        'Huyện Lý Nhân, Tỉnh Hà Nam',
      code: '353',
      parent_code: '35',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35462',
      name: 'Nam Định',
      type: 'thanh-pho',
      slug: 'nam-dinh',
      name_with_type: 'Thành phố Nam Định',
      path: 'Nam Định, Nam Định',
      path_with_type:
        'Thành phố Nam Định, Tỉnh Nam Định',
      code: '356',
      parent_code: '36',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35463',
      name: 'Mỹ Lộc',
      type: 'huyen',
      slug: 'my-loc',
      name_with_type: 'Huyện Mỹ Lộc',
      path: 'Mỹ Lộc, Nam Định',
      path_with_type:
        'Huyện Mỹ Lộc, Tỉnh Nam Định',
      code: '358',
      parent_code: '36',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35464',
      name: 'Vụ Bản',
      type: 'huyen',
      slug: 'vu-ban',
      name_with_type: 'Huyện Vụ Bản',
      path: 'Vụ Bản, Nam Định',
      path_with_type:
        'Huyện Vụ Bản, Tỉnh Nam Định',
      code: '359',
      parent_code: '36',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35465',
      name: 'Ý Yên',
      type: 'huyen',
      slug: 'y-yen',
      name_with_type: 'Huyện Ý Yên',
      path: 'Ý Yên, Nam Định',
      path_with_type:
        'Huyện Ý Yên, Tỉnh Nam Định',
      code: '360',
      parent_code: '36',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35466',
      name: 'Nghĩa Hưng',
      type: 'huyen',
      slug: 'nghia-hung',
      name_with_type: 'Huyện Nghĩa Hưng',
      path: 'Nghĩa Hưng, Nam Định',
      path_with_type:
        'Huyện Nghĩa Hưng, Tỉnh Nam Định',
      code: '361',
      parent_code: '36',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35467',
      name: 'Nam Trực',
      type: 'huyen',
      slug: 'nam-truc',
      name_with_type: 'Huyện Nam Trực',
      path: 'Nam Trực, Nam Định',
      path_with_type:
        'Huyện Nam Trực, Tỉnh Nam Định',
      code: '362',
      parent_code: '36',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35468',
      name: 'Trực Ninh',
      type: 'huyen',
      slug: 'truc-ninh',
      name_with_type: 'Huyện Trực Ninh',
      path: 'Trực Ninh, Nam Định',
      path_with_type:
        'Huyện Trực Ninh, Tỉnh Nam Định',
      code: '363',
      parent_code: '36',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35469',
      name: 'Xuân Trường',
      type: 'huyen',
      slug: 'xuan-truong',
      name_with_type: 'Huyện Xuân Trường',
      path: 'Xuân Trường, Nam Định',
      path_with_type:
        'Huyện Xuân Trường, Tỉnh Nam Định',
      code: '364',
      parent_code: '36',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3546a',
      name: 'Giao Thủy',
      type: 'huyen',
      slug: 'giao-thuy',
      name_with_type: 'Huyện Giao Thủy',
      path: 'Giao Thủy, Nam Định',
      path_with_type:
        'Huyện Giao Thủy, Tỉnh Nam Định',
      code: '365',
      parent_code: '36',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3546b',
      name: 'Hải Hậu',
      type: 'huyen',
      slug: 'hai-hau',
      name_with_type: 'Huyện Hải Hậu',
      path: 'Hải Hậu, Nam Định',
      path_with_type:
        'Huyện Hải Hậu, Tỉnh Nam Định',
      code: '366',
      parent_code: '36',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3546c',
      name: 'Ninh Bình',
      type: 'thanh-pho',
      slug: 'ninh-binh',
      name_with_type: 'Thành phố Ninh Bình',
      path: 'Ninh Bình, Ninh Bình',
      path_with_type:
        'Thành phố Ninh Bình, Tỉnh Ninh Bình',
      code: '369',
      parent_code: '37',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3546d',
      name: 'Tam Điệp',
      type: 'thanh-pho',
      slug: 'tam-diep',
      name_with_type: 'Thành phố Tam Điệp',
      path: 'Tam Điệp, Ninh Bình',
      path_with_type:
        'Thành phố Tam Điệp, Tỉnh Ninh Bình',
      code: '370',
      parent_code: '37',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3546e',
      name: 'Nho Quan',
      type: 'huyen',
      slug: 'nho-quan',
      name_with_type: 'Huyện Nho Quan',
      path: 'Nho Quan, Ninh Bình',
      path_with_type:
        'Huyện Nho Quan, Tỉnh Ninh Bình',
      code: '372',
      parent_code: '37',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3546f',
      name: 'Gia Viễn',
      type: 'huyen',
      slug: 'gia-vien',
      name_with_type: 'Huyện Gia Viễn',
      path: 'Gia Viễn, Ninh Bình',
      path_with_type:
        'Huyện Gia Viễn, Tỉnh Ninh Bình',
      code: '373',
      parent_code: '37',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35470',
      name: 'Hoa Lư',
      type: 'huyen',
      slug: 'hoa-lu',
      name_with_type: 'Huyện Hoa Lư',
      path: 'Hoa Lư, Ninh Bình',
      path_with_type:
        'Huyện Hoa Lư, Tỉnh Ninh Bình',
      code: '374',
      parent_code: '37',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35471',
      name: 'Yên Khánh',
      type: 'huyen',
      slug: 'yen-khanh',
      name_with_type: 'Huyện Yên Khánh',
      path: 'Yên Khánh, Ninh Bình',
      path_with_type:
        'Huyện Yên Khánh, Tỉnh Ninh Bình',
      code: '375',
      parent_code: '37',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35472',
      name: 'Kim Sơn',
      type: 'huyen',
      slug: 'kim-son',
      name_with_type: 'Huyện Kim Sơn',
      path: 'Kim Sơn, Ninh Bình',
      path_with_type:
        'Huyện Kim Sơn, Tỉnh Ninh Bình',
      code: '376',
      parent_code: '37',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35473',
      name: 'Yên Mô',
      type: 'huyen',
      slug: 'yen-mo',
      name_with_type: 'Huyện Yên Mô',
      path: 'Yên Mô, Ninh Bình',
      path_with_type:
        'Huyện Yên Mô, Tỉnh Ninh Bình',
      code: '377',
      parent_code: '37',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35474',
      name: 'Thanh Hóa',
      type: 'thanh-pho',
      slug: 'thanh-hoa',
      name_with_type: 'Thành phố Thanh Hóa',
      path: 'Thanh Hóa, Thanh Hóa',
      path_with_type:
        'Thành phố Thanh Hóa, Tỉnh Thanh Hóa',
      code: '380',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35475',
      name: 'Bỉm Sơn',
      type: 'thi-xa',
      slug: 'bim-son',
      name_with_type: 'Thị xã Bỉm Sơn',
      path: 'Bỉm Sơn, Thanh Hóa',
      path_with_type:
        'Thị xã Bỉm Sơn, Tỉnh Thanh Hóa',
      code: '381',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35476',
      name: 'Sầm Sơn',
      type: 'thanh-pho',
      slug: 'sam-son',
      name_with_type: 'Thành phố Sầm Sơn',
      path: 'Sầm Sơn, Thanh Hóa',
      path_with_type:
        'Thành phố Sầm Sơn, Tỉnh Thanh Hóa',
      code: '382',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35477',
      name: 'Mường Lát',
      type: 'huyen',
      slug: 'muong-lat',
      name_with_type: 'Huyện Mường Lát',
      path: 'Mường Lát, Thanh Hóa',
      path_with_type:
        'Huyện Mường Lát, Tỉnh Thanh Hóa',
      code: '384',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35478',
      name: 'Quan Hóa',
      type: 'huyen',
      slug: 'quan-hoa',
      name_with_type: 'Huyện Quan Hóa',
      path: 'Quan Hóa, Thanh Hóa',
      path_with_type:
        'Huyện Quan Hóa, Tỉnh Thanh Hóa',
      code: '385',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35479',
      name: 'Bá Thước',
      type: 'huyen',
      slug: 'ba-thuoc',
      name_with_type: 'Huyện Bá Thước',
      path: 'Bá Thước, Thanh Hóa',
      path_with_type:
        'Huyện Bá Thước, Tỉnh Thanh Hóa',
      code: '386',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3547a',
      name: 'Quan Sơn',
      type: 'huyen',
      slug: 'quan-son',
      name_with_type: 'Huyện Quan Sơn',
      path: 'Quan Sơn, Thanh Hóa',
      path_with_type:
        'Huyện Quan Sơn, Tỉnh Thanh Hóa',
      code: '387',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3547b',
      name: 'Lang Chánh',
      type: 'huyen',
      slug: 'lang-chanh',
      name_with_type: 'Huyện Lang Chánh',
      path: 'Lang Chánh, Thanh Hóa',
      path_with_type:
        'Huyện Lang Chánh, Tỉnh Thanh Hóa',
      code: '388',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3547c',
      name: 'Ngọc Lặc',
      type: 'huyen',
      slug: 'ngoc-lac',
      name_with_type: 'Huyện Ngọc Lặc',
      path: 'Ngọc Lặc, Thanh Hóa',
      path_with_type:
        'Huyện Ngọc Lặc, Tỉnh Thanh Hóa',
      code: '389',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3547d',
      name: 'Cẩm Thủy',
      type: 'huyen',
      slug: 'cam-thuy',
      name_with_type: 'Huyện Cẩm Thủy',
      path: 'Cẩm Thủy, Thanh Hóa',
      path_with_type:
        'Huyện Cẩm Thủy, Tỉnh Thanh Hóa',
      code: '390',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3547e',
      name: 'Thạch Thành',
      type: 'huyen',
      slug: 'thach-thanh',
      name_with_type: 'Huyện Thạch Thành',
      path: 'Thạch Thành, Thanh Hóa',
      path_with_type:
        'Huyện Thạch Thành, Tỉnh Thanh Hóa',
      code: '391',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3547f',
      name: 'Hà Trung',
      type: 'huyen',
      slug: 'ha-trung',
      name_with_type: 'Huyện Hà Trung',
      path: 'Hà Trung, Thanh Hóa',
      path_with_type:
        'Huyện Hà Trung, Tỉnh Thanh Hóa',
      code: '392',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35480',
      name: 'Vĩnh Lộc',
      type: 'huyen',
      slug: 'vinh-loc',
      name_with_type: 'Huyện Vĩnh Lộc',
      path: 'Vĩnh Lộc, Thanh Hóa',
      path_with_type:
        'Huyện Vĩnh Lộc, Tỉnh Thanh Hóa',
      code: '393',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35481',
      name: 'Yên Định',
      type: 'huyen',
      slug: 'yen-dinh',
      name_with_type: 'Huyện Yên Định',
      path: 'Yên Định, Thanh Hóa',
      path_with_type:
        'Huyện Yên Định, Tỉnh Thanh Hóa',
      code: '394',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35482',
      name: 'Thọ Xuân',
      type: 'huyen',
      slug: 'tho-xuan',
      name_with_type: 'Huyện Thọ Xuân',
      path: 'Thọ Xuân, Thanh Hóa',
      path_with_type:
        'Huyện Thọ Xuân, Tỉnh Thanh Hóa',
      code: '395',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35483',
      name: 'Thường Xuân',
      type: 'huyen',
      slug: 'thuong-xuan',
      name_with_type: 'Huyện Thường Xuân',
      path: 'Thường Xuân, Thanh Hóa',
      path_with_type:
        'Huyện Thường Xuân, Tỉnh Thanh Hóa',
      code: '396',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35484',
      name: 'Triệu Sơn',
      type: 'huyen',
      slug: 'trieu-son',
      name_with_type: 'Huyện Triệu Sơn',
      path: 'Triệu Sơn, Thanh Hóa',
      path_with_type:
        'Huyện Triệu Sơn, Tỉnh Thanh Hóa',
      code: '397',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35485',
      name: 'Thiệu Hóa',
      type: 'huyen',
      slug: 'thieu-hoa',
      name_with_type: 'Huyện Thiệu Hóa',
      path: 'Thiệu Hóa, Thanh Hóa',
      path_with_type:
        'Huyện Thiệu Hóa, Tỉnh Thanh Hóa',
      code: '398',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35486',
      name: 'Hoằng Hóa',
      type: 'huyen',
      slug: 'hoang-hoa',
      name_with_type: 'Huyện Hoằng Hóa',
      path: 'Hoằng Hóa, Thanh Hóa',
      path_with_type:
        'Huyện Hoằng Hóa, Tỉnh Thanh Hóa',
      code: '399',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35487',
      name: 'Hậu Lộc',
      type: 'huyen',
      slug: 'hau-loc',
      name_with_type: 'Huyện Hậu Lộc',
      path: 'Hậu Lộc, Thanh Hóa',
      path_with_type:
        'Huyện Hậu Lộc, Tỉnh Thanh Hóa',
      code: '400',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35488',
      name: 'Nga Sơn',
      type: 'huyen',
      slug: 'nga-son',
      name_with_type: 'Huyện Nga Sơn',
      path: 'Nga Sơn, Thanh Hóa',
      path_with_type:
        'Huyện Nga Sơn, Tỉnh Thanh Hóa',
      code: '401',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35489',
      name: 'Như Xuân',
      type: 'huyen',
      slug: 'nhu-xuan',
      name_with_type: 'Huyện Như Xuân',
      path: 'Như Xuân, Thanh Hóa',
      path_with_type:
        'Huyện Như Xuân, Tỉnh Thanh Hóa',
      code: '402',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3548a',
      name: 'Như Thanh',
      type: 'huyen',
      slug: 'nhu-thanh',
      name_with_type: 'Huyện Như Thanh',
      path: 'Như Thanh, Thanh Hóa',
      path_with_type:
        'Huyện Như Thanh, Tỉnh Thanh Hóa',
      code: '403',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3548b',
      name: 'Nông Cống',
      type: 'huyen',
      slug: 'nong-cong',
      name_with_type: 'Huyện Nông Cống',
      path: 'Nông Cống, Thanh Hóa',
      path_with_type:
        'Huyện Nông Cống, Tỉnh Thanh Hóa',
      code: '404',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3548c',
      name: 'Đông Sơn',
      type: 'huyen',
      slug: 'dong-son',
      name_with_type: 'Huyện Đông Sơn',
      path: 'Đông Sơn, Thanh Hóa',
      path_with_type:
        'Huyện Đông Sơn, Tỉnh Thanh Hóa',
      code: '405',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3548d',
      name: 'Quảng Xương',
      type: 'huyen',
      slug: 'quang-xuong',
      name_with_type: 'Huyện Quảng Xương',
      path: 'Quảng Xương, Thanh Hóa',
      path_with_type:
        'Huyện Quảng Xương, Tỉnh Thanh Hóa',
      code: '406',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3548e',
      name: 'Nghi Sơn',
      type: 'thi-xa',
      slug: 'nghi-son',
      name_with_type: 'Thị xã Nghi Sơn',
      path: 'Nghi Sơn, Thanh Hóa',
      path_with_type:
        'Thị xã Nghi Sơn, Tỉnh Thanh Hóa',
      code: '407',
      parent_code: '38',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3548f',
      name: 'Vinh',
      type: 'thanh-pho',
      slug: 'vinh',
      name_with_type: 'Thành phố Vinh',
      path: 'Vinh, Nghệ An',
      path_with_type:
        'Thành phố Vinh, Tỉnh Nghệ An',
      code: '412',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35490',
      name: 'Cửa Lò',
      type: 'thi-xa',
      slug: 'cua-lo',
      name_with_type: 'Thị xã Cửa Lò',
      path: 'Cửa Lò, Nghệ An',
      path_with_type:
        'Thị xã Cửa Lò, Tỉnh Nghệ An',
      code: '413',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35491',
      name: 'Thái Hoà',
      type: 'thi-xa',
      slug: 'thai-hoa',
      name_with_type: 'Thị xã Thái Hoà',
      path: 'Thái Hoà, Nghệ An',
      path_with_type:
        'Thị xã Thái Hoà, Tỉnh Nghệ An',
      code: '414',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35492',
      name: 'Quế Phong',
      type: 'huyen',
      slug: 'que-phong',
      name_with_type: 'Huyện Quế Phong',
      path: 'Quế Phong, Nghệ An',
      path_with_type:
        'Huyện Quế Phong, Tỉnh Nghệ An',
      code: '415',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35493',
      name: 'Quỳ Châu',
      type: 'huyen',
      slug: 'quy-chau',
      name_with_type: 'Huyện Quỳ Châu',
      path: 'Quỳ Châu, Nghệ An',
      path_with_type:
        'Huyện Quỳ Châu, Tỉnh Nghệ An',
      code: '416',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35494',
      name: 'Kỳ Sơn',
      type: 'huyen',
      slug: 'ky-son',
      name_with_type: 'Huyện Kỳ Sơn',
      path: 'Kỳ Sơn, Nghệ An',
      path_with_type:
        'Huyện Kỳ Sơn, Tỉnh Nghệ An',
      code: '417',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35495',
      name: 'Tương Dương',
      type: 'huyen',
      slug: 'tuong-duong',
      name_with_type: 'Huyện Tương Dương',
      path: 'Tương Dương, Nghệ An',
      path_with_type:
        'Huyện Tương Dương, Tỉnh Nghệ An',
      code: '418',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35496',
      name: 'Nghĩa Đàn',
      type: 'huyen',
      slug: 'nghia-dan',
      name_with_type: 'Huyện Nghĩa Đàn',
      path: 'Nghĩa Đàn, Nghệ An',
      path_with_type:
        'Huyện Nghĩa Đàn, Tỉnh Nghệ An',
      code: '419',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35497',
      name: 'Quỳ Hợp',
      type: 'huyen',
      slug: 'quy-hop',
      name_with_type: 'Huyện Quỳ Hợp',
      path: 'Quỳ Hợp, Nghệ An',
      path_with_type:
        'Huyện Quỳ Hợp, Tỉnh Nghệ An',
      code: '420',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35498',
      name: 'Quỳnh Lưu',
      type: 'huyen',
      slug: 'quynh-luu',
      name_with_type: 'Huyện Quỳnh Lưu',
      path: 'Quỳnh Lưu, Nghệ An',
      path_with_type:
        'Huyện Quỳnh Lưu, Tỉnh Nghệ An',
      code: '421',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35499',
      name: 'Con Cuông',
      type: 'huyen',
      slug: 'con-cuong',
      name_with_type: 'Huyện Con Cuông',
      path: 'Con Cuông, Nghệ An',
      path_with_type:
        'Huyện Con Cuông, Tỉnh Nghệ An',
      code: '422',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3549a',
      name: 'Tân Kỳ',
      type: 'huyen',
      slug: 'tan-ky',
      name_with_type: 'Huyện Tân Kỳ',
      path: 'Tân Kỳ, Nghệ An',
      path_with_type:
        'Huyện Tân Kỳ, Tỉnh Nghệ An',
      code: '423',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3549b',
      name: 'Anh Sơn',
      type: 'huyen',
      slug: 'anh-son',
      name_with_type: 'Huyện Anh Sơn',
      path: 'Anh Sơn, Nghệ An',
      path_with_type:
        'Huyện Anh Sơn, Tỉnh Nghệ An',
      code: '424',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3549c',
      name: 'Diễn Châu',
      type: 'huyen',
      slug: 'dien-chau',
      name_with_type: 'Huyện Diễn Châu',
      path: 'Diễn Châu, Nghệ An',
      path_with_type:
        'Huyện Diễn Châu, Tỉnh Nghệ An',
      code: '425',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3549d',
      name: 'Yên Thành',
      type: 'huyen',
      slug: 'yen-thanh',
      name_with_type: 'Huyện Yên Thành',
      path: 'Yên Thành, Nghệ An',
      path_with_type:
        'Huyện Yên Thành, Tỉnh Nghệ An',
      code: '426',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3549e',
      name: 'Đô Lương',
      type: 'huyen',
      slug: 'do-luong',
      name_with_type: 'Huyện Đô Lương',
      path: 'Đô Lương, Nghệ An',
      path_with_type:
        'Huyện Đô Lương, Tỉnh Nghệ An',
      code: '427',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3549f',
      name: 'Thanh Chương',
      type: 'huyen',
      slug: 'thanh-chuong',
      name_with_type: 'Huyện Thanh Chương',
      path: 'Thanh Chương, Nghệ An',
      path_with_type:
        'Huyện Thanh Chương, Tỉnh Nghệ An',
      code: '428',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354a0',
      name: 'Nghi Lộc',
      type: 'huyen',
      slug: 'nghi-loc',
      name_with_type: 'Huyện Nghi Lộc',
      path: 'Nghi Lộc, Nghệ An',
      path_with_type:
        'Huyện Nghi Lộc, Tỉnh Nghệ An',
      code: '429',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354a1',
      name: 'Nam Đàn',
      type: 'huyen',
      slug: 'nam-dan',
      name_with_type: 'Huyện Nam Đàn',
      path: 'Nam Đàn, Nghệ An',
      path_with_type:
        'Huyện Nam Đàn, Tỉnh Nghệ An',
      code: '430',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354a2',
      name: 'Hưng Nguyên',
      type: 'huyen',
      slug: 'hung-nguyen',
      name_with_type: 'Huyện Hưng Nguyên',
      path: 'Hưng Nguyên, Nghệ An',
      path_with_type:
        'Huyện Hưng Nguyên, Tỉnh Nghệ An',
      code: '431',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354a3',
      name: 'Hoàng Mai',
      type: 'thi-xa',
      slug: 'hoang-mai',
      name_with_type: 'Thị xã Hoàng Mai',
      path: 'Hoàng Mai, Nghệ An',
      path_with_type:
        'Thị xã Hoàng Mai, Tỉnh Nghệ An',
      code: '432',
      parent_code: '40',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354a4',
      name: 'Hà Tĩnh',
      type: 'thanh-pho',
      slug: 'ha-tinh',
      name_with_type: 'Thành phố Hà Tĩnh',
      path: 'Hà Tĩnh, Hà Tĩnh',
      path_with_type:
        'Thành phố Hà Tĩnh, Tỉnh Hà Tĩnh',
      code: '436',
      parent_code: '42',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354a5',
      name: 'Hồng Lĩnh',
      type: 'thi-xa',
      slug: 'hong-linh',
      name_with_type: 'Thị xã Hồng Lĩnh',
      path: 'Hồng Lĩnh, Hà Tĩnh',
      path_with_type:
        'Thị xã Hồng Lĩnh, Tỉnh Hà Tĩnh',
      code: '437',
      parent_code: '42',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354a6',
      name: 'Hương Sơn',
      type: 'huyen',
      slug: 'huong-son',
      name_with_type: 'Huyện Hương Sơn',
      path: 'Hương Sơn, Hà Tĩnh',
      path_with_type:
        'Huyện Hương Sơn, Tỉnh Hà Tĩnh',
      code: '439',
      parent_code: '42',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354a7',
      name: 'Đức Thọ',
      type: 'huyen',
      slug: 'duc-tho',
      name_with_type: 'Huyện Đức Thọ',
      path: 'Đức Thọ, Hà Tĩnh',
      path_with_type:
        'Huyện Đức Thọ, Tỉnh Hà Tĩnh',
      code: '440',
      parent_code: '42',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354a8',
      name: 'Vũ Quang',
      type: 'huyen',
      slug: 'vu-quang',
      name_with_type: 'Huyện Vũ Quang',
      path: 'Vũ Quang, Hà Tĩnh',
      path_with_type:
        'Huyện Vũ Quang, Tỉnh Hà Tĩnh',
      code: '441',
      parent_code: '42',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354a9',
      name: 'Nghi Xuân',
      type: 'huyen',
      slug: 'nghi-xuan',
      name_with_type: 'Huyện Nghi Xuân',
      path: 'Nghi Xuân, Hà Tĩnh',
      path_with_type:
        'Huyện Nghi Xuân, Tỉnh Hà Tĩnh',
      code: '442',
      parent_code: '42',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354aa',
      name: 'Can Lộc',
      type: 'huyen',
      slug: 'can-loc',
      name_with_type: 'Huyện Can Lộc',
      path: 'Can Lộc, Hà Tĩnh',
      path_with_type:
        'Huyện Can Lộc, Tỉnh Hà Tĩnh',
      code: '443',
      parent_code: '42',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354ab',
      name: 'Hương Khê',
      type: 'huyen',
      slug: 'huong-khe',
      name_with_type: 'Huyện Hương Khê',
      path: 'Hương Khê, Hà Tĩnh',
      path_with_type:
        'Huyện Hương Khê, Tỉnh Hà Tĩnh',
      code: '444',
      parent_code: '42',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354ac',
      name: 'Thạch Hà',
      type: 'huyen',
      slug: 'thach-ha',
      name_with_type: 'Huyện Thạch Hà',
      path: 'Thạch Hà, Hà Tĩnh',
      path_with_type:
        'Huyện Thạch Hà, Tỉnh Hà Tĩnh',
      code: '445',
      parent_code: '42',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354ad',
      name: 'Cẩm Xuyên',
      type: 'huyen',
      slug: 'cam-xuyen',
      name_with_type: 'Huyện Cẩm Xuyên',
      path: 'Cẩm Xuyên, Hà Tĩnh',
      path_with_type:
        'Huyện Cẩm Xuyên, Tỉnh Hà Tĩnh',
      code: '446',
      parent_code: '42',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354ae',
      name: 'Kỳ Anh',
      type: 'huyen',
      slug: 'ky-anh',
      name_with_type: 'Huyện Kỳ Anh',
      path: 'Kỳ Anh, Hà Tĩnh',
      path_with_type:
        'Huyện Kỳ Anh, Tỉnh Hà Tĩnh',
      code: '447',
      parent_code: '42',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354af',
      name: 'Lộc Hà',
      type: 'huyen',
      slug: 'loc-ha',
      name_with_type: 'Huyện Lộc Hà',
      path: 'Lộc Hà, Hà Tĩnh',
      path_with_type:
        'Huyện Lộc Hà, Tỉnh Hà Tĩnh',
      code: '448',
      parent_code: '42',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354b0',
      name: 'Kỳ Anh',
      type: 'thi-xa',
      slug: 'ky-anh',
      name_with_type: 'Thị xã Kỳ Anh',
      path: 'Kỳ Anh, Hà Tĩnh',
      path_with_type:
        'Thị xã Kỳ Anh, Tỉnh Hà Tĩnh',
      code: '449',
      parent_code: '42',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354b1',
      name: 'Đồng Hới',
      type: 'thanh-pho',
      slug: 'dong-hoi',
      name_with_type: 'Thành Phố Đồng Hới',
      path: 'Đồng Hới, Quảng Bình',
      path_with_type:
        'Thành Phố Đồng Hới, Tỉnh Quảng Bình',
      code: '450',
      parent_code: '44',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354b2',
      name: 'Minh Hóa',
      type: 'huyen',
      slug: 'minh-hoa',
      name_with_type: 'Huyện Minh Hóa',
      path: 'Minh Hóa, Quảng Bình',
      path_with_type:
        'Huyện Minh Hóa, Tỉnh Quảng Bình',
      code: '452',
      parent_code: '44',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354b3',
      name: 'Tuyên Hóa',
      type: 'huyen',
      slug: 'tuyen-hoa',
      name_with_type: 'Huyện Tuyên Hóa',
      path: 'Tuyên Hóa, Quảng Bình',
      path_with_type:
        'Huyện Tuyên Hóa, Tỉnh Quảng Bình',
      code: '453',
      parent_code: '44',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354b4',
      name: 'Quảng Trạch',
      type: 'huyen',
      slug: 'quang-trach',
      name_with_type: 'Huyện Quảng Trạch',
      path: 'Quảng Trạch, Quảng Bình',
      path_with_type:
        'Huyện Quảng Trạch, Tỉnh Quảng Bình',
      code: '454',
      parent_code: '44',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354b5',
      name: 'Bố Trạch',
      type: 'huyen',
      slug: 'bo-trach',
      name_with_type: 'Huyện Bố Trạch',
      path: 'Bố Trạch, Quảng Bình',
      path_with_type:
        'Huyện Bố Trạch, Tỉnh Quảng Bình',
      code: '455',
      parent_code: '44',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354b6',
      name: 'Quảng Ninh',
      type: 'huyen',
      slug: 'quang-ninh',
      name_with_type: 'Huyện Quảng Ninh',
      path: 'Quảng Ninh, Quảng Bình',
      path_with_type:
        'Huyện Quảng Ninh, Tỉnh Quảng Bình',
      code: '456',
      parent_code: '44',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354b7',
      name: 'Lệ Thủy',
      type: 'huyen',
      slug: 'le-thuy',
      name_with_type: 'Huyện Lệ Thủy',
      path: 'Lệ Thủy, Quảng Bình',
      path_with_type:
        'Huyện Lệ Thủy, Tỉnh Quảng Bình',
      code: '457',
      parent_code: '44',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354b8',
      name: 'Ba Đồn',
      type: 'thi-xa',
      slug: 'ba-don',
      name_with_type: 'Thị xã Ba Đồn',
      path: 'Ba Đồn, Quảng Bình',
      path_with_type:
        'Thị xã Ba Đồn, Tỉnh Quảng Bình',
      code: '458',
      parent_code: '44',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354b9',
      name: 'Đông Hà',
      type: 'thanh-pho',
      slug: 'dong-ha',
      name_with_type: 'Thành phố Đông Hà',
      path: 'Đông Hà, Quảng Trị',
      path_with_type:
        'Thành phố Đông Hà, Tỉnh Quảng Trị',
      code: '461',
      parent_code: '45',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354ba',
      name: 'Quảng Trị',
      type: 'thi-xa',
      slug: 'quang-tri',
      name_with_type: 'Thị xã Quảng Trị',
      path: 'Quảng Trị, Quảng Trị',
      path_with_type:
        'Thị xã Quảng Trị, Tỉnh Quảng Trị',
      code: '462',
      parent_code: '45',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354bb',
      name: 'Vĩnh Linh',
      type: 'huyen',
      slug: 'vinh-linh',
      name_with_type: 'Huyện Vĩnh Linh',
      path: 'Vĩnh Linh, Quảng Trị',
      path_with_type:
        'Huyện Vĩnh Linh, Tỉnh Quảng Trị',
      code: '464',
      parent_code: '45',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354bc',
      name: 'Hướng Hóa',
      type: 'huyen',
      slug: 'huong-hoa',
      name_with_type: 'Huyện Hướng Hóa',
      path: 'Hướng Hóa, Quảng Trị',
      path_with_type:
        'Huyện Hướng Hóa, Tỉnh Quảng Trị',
      code: '465',
      parent_code: '45',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354bd',
      name: 'Gio Linh',
      type: 'huyen',
      slug: 'gio-linh',
      name_with_type: 'Huyện Gio Linh',
      path: 'Gio Linh, Quảng Trị',
      path_with_type:
        'Huyện Gio Linh, Tỉnh Quảng Trị',
      code: '466',
      parent_code: '45',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354be',
      name: 'Đa Krông',
      type: 'huyen',
      slug: 'da-krong',
      name_with_type: 'Huyện Đa Krông',
      path: 'Đa Krông, Quảng Trị',
      path_with_type:
        'Huyện Đa Krông, Tỉnh Quảng Trị',
      code: '467',
      parent_code: '45',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354bf',
      name: 'Cam Lộ',
      type: 'huyen',
      slug: 'cam-lo',
      name_with_type: 'Huyện Cam Lộ',
      path: 'Cam Lộ, Quảng Trị',
      path_with_type:
        'Huyện Cam Lộ, Tỉnh Quảng Trị',
      code: '468',
      parent_code: '45',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354c0',
      name: 'Triệu Phong',
      type: 'huyen',
      slug: 'trieu-phong',
      name_with_type: 'Huyện Triệu Phong',
      path: 'Triệu Phong, Quảng Trị',
      path_with_type:
        'Huyện Triệu Phong, Tỉnh Quảng Trị',
      code: '469',
      parent_code: '45',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354c1',
      name: 'Hải Lăng',
      type: 'huyen',
      slug: 'hai-lang',
      name_with_type: 'Huyện Hải Lăng',
      path: 'Hải Lăng, Quảng Trị',
      path_with_type:
        'Huyện Hải Lăng, Tỉnh Quảng Trị',
      code: '470',
      parent_code: '45',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354c2',
      name: 'Cồn Cỏ',
      type: 'huyen',
      slug: 'con-co',
      name_with_type: 'Huyện Cồn Cỏ',
      path: 'Cồn Cỏ, Quảng Trị',
      path_with_type:
        'Huyện Cồn Cỏ, Tỉnh Quảng Trị',
      code: '471',
      parent_code: '45',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354c3',
      name: 'Huế',
      type: 'thanh-pho',
      slug: 'hue',
      name_with_type: 'Thành phố Huế',
      path: 'Huế, Thừa Thiên Huế',
      path_with_type:
        'Thành phố Huế, Tỉnh Thừa Thiên Huế',
      code: '474',
      parent_code: '46',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354c4',
      name: 'Phong Điền',
      type: 'huyen',
      slug: 'phong-dien',
      name_with_type: 'Huyện Phong Điền',
      path: 'Phong Điền, Thừa Thiên Huế',
      path_with_type:
        'Huyện Phong Điền, Tỉnh Thừa Thiên Huế',
      code: '476',
      parent_code: '46',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354c5',
      name: 'Quảng Điền',
      type: 'huyen',
      slug: 'quang-dien',
      name_with_type: 'Huyện Quảng Điền',
      path: 'Quảng Điền, Thừa Thiên Huế',
      path_with_type:
        'Huyện Quảng Điền, Tỉnh Thừa Thiên Huế',
      code: '477',
      parent_code: '46',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354c6',
      name: 'Phú Vang',
      type: 'huyen',
      slug: 'phu-vang',
      name_with_type: 'Huyện Phú Vang',
      path: 'Phú Vang, Thừa Thiên Huế',
      path_with_type:
        'Huyện Phú Vang, Tỉnh Thừa Thiên Huế',
      code: '478',
      parent_code: '46',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354c7',
      name: 'Hương Thủy',
      type: 'thi-xa',
      slug: 'huong-thuy',
      name_with_type: 'Thị xã Hương Thủy',
      path: 'Hương Thủy, Thừa Thiên Huế',
      path_with_type:
        'Thị xã Hương Thủy, Tỉnh Thừa Thiên Huế',
      code: '479',
      parent_code: '46',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354c8',
      name: 'Hương Trà',
      type: 'thi-xa',
      slug: 'huong-tra',
      name_with_type: 'Thị xã Hương Trà',
      path: 'Hương Trà, Thừa Thiên Huế',
      path_with_type:
        'Thị xã Hương Trà, Tỉnh Thừa Thiên Huế',
      code: '480',
      parent_code: '46',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354c9',
      name: 'A Lưới',
      type: 'huyen',
      slug: 'a-luoi',
      name_with_type: 'Huyện A Lưới',
      path: 'A Lưới, Thừa Thiên Huế',
      path_with_type:
        'Huyện A Lưới, Tỉnh Thừa Thiên Huế',
      code: '481',
      parent_code: '46',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354ca',
      name: 'Phú Lộc',
      type: 'huyen',
      slug: 'phu-loc',
      name_with_type: 'Huyện Phú Lộc',
      path: 'Phú Lộc, Thừa Thiên Huế',
      path_with_type:
        'Huyện Phú Lộc, Tỉnh Thừa Thiên Huế',
      code: '482',
      parent_code: '46',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354cb',
      name: 'Nam Đông',
      type: 'huyen',
      slug: 'nam-dong',
      name_with_type: 'Huyện Nam Đông',
      path: 'Nam Đông, Thừa Thiên Huế',
      path_with_type:
        'Huyện Nam Đông, Tỉnh Thừa Thiên Huế',
      code: '483',
      parent_code: '46',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354cc',
      name: 'Liên Chiểu',
      type: 'quan',
      slug: 'lien-chieu',
      name_with_type: 'Quận Liên Chiểu',
      path: 'Liên Chiểu, Đà Nẵng',
      path_with_type:
        'Quận Liên Chiểu, Thành phố Đà Nẵng',
      code: '490',
      parent_code: '48',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354cd',
      name: 'Thanh Khê',
      type: 'quan',
      slug: 'thanh-khe',
      name_with_type: 'Quận Thanh Khê',
      path: 'Thanh Khê, Đà Nẵng',
      path_with_type:
        'Quận Thanh Khê, Thành phố Đà Nẵng',
      code: '491',
      parent_code: '48',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354ce',
      name: 'Hải Châu',
      type: 'quan',
      slug: 'hai-chau',
      name_with_type: 'Quận Hải Châu',
      path: 'Hải Châu, Đà Nẵng',
      path_with_type:
        'Quận Hải Châu, Thành phố Đà Nẵng',
      code: '492',
      parent_code: '48',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354cf',
      name: 'Sơn Trà',
      type: 'quan',
      slug: 'son-tra',
      name_with_type: 'Quận Sơn Trà',
      path: 'Sơn Trà, Đà Nẵng',
      path_with_type:
        'Quận Sơn Trà, Thành phố Đà Nẵng',
      code: '493',
      parent_code: '48',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354d0',
      name: 'Ngũ Hành Sơn',
      type: 'quan',
      slug: 'ngu-hanh-son',
      name_with_type: 'Quận Ngũ Hành Sơn',
      path: 'Ngũ Hành Sơn, Đà Nẵng',
      path_with_type:
        'Quận Ngũ Hành Sơn, Thành phố Đà Nẵng',
      code: '494',
      parent_code: '48',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354d1',
      name: 'Cẩm Lệ',
      type: 'quan',
      slug: 'cam-le',
      name_with_type: 'Quận Cẩm Lệ',
      path: 'Cẩm Lệ, Đà Nẵng',
      path_with_type:
        'Quận Cẩm Lệ, Thành phố Đà Nẵng',
      code: '495',
      parent_code: '48',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354d2',
      name: 'Hòa Vang',
      type: 'huyen',
      slug: 'hoa-vang',
      name_with_type: 'Huyện Hòa Vang',
      path: 'Hòa Vang, Đà Nẵng',
      path_with_type:
        'Huyện Hòa Vang, Thành phố Đà Nẵng',
      code: '497',
      parent_code: '48',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354d3',
      name: 'Hoàng Sa',
      type: 'huyen',
      slug: 'hoang-sa',
      name_with_type: 'Huyện Hoàng Sa',
      path: 'Hoàng Sa, Đà Nẵng',
      path_with_type:
        'Huyện Hoàng Sa, Thành phố Đà Nẵng',
      code: '498',
      parent_code: '48',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354d4',
      name: 'Tam Kỳ',
      type: 'thanh-pho',
      slug: 'tam-ky',
      name_with_type: 'Thành phố Tam Kỳ',
      path: 'Tam Kỳ, Quảng Nam',
      path_with_type:
        'Thành phố Tam Kỳ, Tỉnh Quảng Nam',
      code: '502',
      parent_code: '49',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354d5',
      name: 'Hội An',
      type: 'thanh-pho',
      slug: 'hoi-an',
      name_with_type: 'Thành phố Hội An',
      path: 'Hội An, Quảng Nam',
      path_with_type:
        'Thành phố Hội An, Tỉnh Quảng Nam',
      code: '503',
      parent_code: '49',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354d6',
      name: 'Tây Giang',
      type: 'huyen',
      slug: 'tay-giang',
      name_with_type: 'Huyện Tây Giang',
      path: 'Tây Giang, Quảng Nam',
      path_with_type:
        'Huyện Tây Giang, Tỉnh Quảng Nam',
      code: '504',
      parent_code: '49',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354d7',
      name: 'Đông Giang',
      type: 'huyen',
      slug: 'dong-giang',
      name_with_type: 'Huyện Đông Giang',
      path: 'Đông Giang, Quảng Nam',
      path_with_type:
        'Huyện Đông Giang, Tỉnh Quảng Nam',
      code: '505',
      parent_code: '49',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354d8',
      name: 'Đại Lộc',
      type: 'huyen',
      slug: 'dai-loc',
      name_with_type: 'Huyện Đại Lộc',
      path: 'Đại Lộc, Quảng Nam',
      path_with_type:
        'Huyện Đại Lộc, Tỉnh Quảng Nam',
      code: '506',
      parent_code: '49',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354d9',
      name: 'Điện Bàn',
      type: 'thi-xa',
      slug: 'dien-ban',
      name_with_type: 'Thị xã Điện Bàn',
      path: 'Điện Bàn, Quảng Nam',
      path_with_type:
        'Thị xã Điện Bàn, Tỉnh Quảng Nam',
      code: '507',
      parent_code: '49',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354da',
      name: 'Duy Xuyên',
      type: 'huyen',
      slug: 'duy-xuyen',
      name_with_type: 'Huyện Duy Xuyên',
      path: 'Duy Xuyên, Quảng Nam',
      path_with_type:
        'Huyện Duy Xuyên, Tỉnh Quảng Nam',
      code: '508',
      parent_code: '49',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354db',
      name: 'Quế Sơn',
      type: 'huyen',
      slug: 'que-son',
      name_with_type: 'Huyện Quế Sơn',
      path: 'Quế Sơn, Quảng Nam',
      path_with_type:
        'Huyện Quế Sơn, Tỉnh Quảng Nam',
      code: '509',
      parent_code: '49',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354dc',
      name: 'Nam Giang',
      type: 'huyen',
      slug: 'nam-giang',
      name_with_type: 'Huyện Nam Giang',
      path: 'Nam Giang, Quảng Nam',
      path_with_type:
        'Huyện Nam Giang, Tỉnh Quảng Nam',
      code: '510',
      parent_code: '49',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354dd',
      name: 'Phước Sơn',
      type: 'huyen',
      slug: 'phuoc-son',
      name_with_type: 'Huyện Phước Sơn',
      path: 'Phước Sơn, Quảng Nam',
      path_with_type:
        'Huyện Phước Sơn, Tỉnh Quảng Nam',
      code: '511',
      parent_code: '49',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354de',
      name: 'Hiệp Đức',
      type: 'huyen',
      slug: 'hiep-duc',
      name_with_type: 'Huyện Hiệp Đức',
      path: 'Hiệp Đức, Quảng Nam',
      path_with_type:
        'Huyện Hiệp Đức, Tỉnh Quảng Nam',
      code: '512',
      parent_code: '49',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354df',
      name: 'Thăng Bình',
      type: 'huyen',
      slug: 'thang-binh',
      name_with_type: 'Huyện Thăng Bình',
      path: 'Thăng Bình, Quảng Nam',
      path_with_type:
        'Huyện Thăng Bình, Tỉnh Quảng Nam',
      code: '513',
      parent_code: '49',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354e0',
      name: 'Tiên Phước',
      type: 'huyen',
      slug: 'tien-phuoc',
      name_with_type: 'Huyện Tiên Phước',
      path: 'Tiên Phước, Quảng Nam',
      path_with_type:
        'Huyện Tiên Phước, Tỉnh Quảng Nam',
      code: '514',
      parent_code: '49',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354e1',
      name: 'Bắc Trà My',
      type: 'huyen',
      slug: 'bac-tra-my',
      name_with_type: 'Huyện Bắc Trà My',
      path: 'Bắc Trà My, Quảng Nam',
      path_with_type:
        'Huyện Bắc Trà My, Tỉnh Quảng Nam',
      code: '515',
      parent_code: '49',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354e2',
      name: 'Nam Trà My',
      type: 'huyen',
      slug: 'nam-tra-my',
      name_with_type: 'Huyện Nam Trà My',
      path: 'Nam Trà My, Quảng Nam',
      path_with_type:
        'Huyện Nam Trà My, Tỉnh Quảng Nam',
      code: '516',
      parent_code: '49',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354e3',
      name: 'Núi Thành',
      type: 'huyen',
      slug: 'nui-thanh',
      name_with_type: 'Huyện Núi Thành',
      path: 'Núi Thành, Quảng Nam',
      path_with_type:
        'Huyện Núi Thành, Tỉnh Quảng Nam',
      code: '517',
      parent_code: '49',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354e4',
      name: 'Phú Ninh',
      type: 'huyen',
      slug: 'phu-ninh',
      name_with_type: 'Huyện Phú Ninh',
      path: 'Phú Ninh, Quảng Nam',
      path_with_type:
        'Huyện Phú Ninh, Tỉnh Quảng Nam',
      code: '518',
      parent_code: '49',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354e5',
      name: 'Nông Sơn',
      type: 'huyen',
      slug: 'nong-son',
      name_with_type: 'Huyện Nông Sơn',
      path: 'Nông Sơn, Quảng Nam',
      path_with_type:
        'Huyện Nông Sơn, Tỉnh Quảng Nam',
      code: '519',
      parent_code: '49',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354e6',
      name: 'Quảng Ngãi',
      type: 'thanh-pho',
      slug: 'quang-ngai',
      name_with_type: 'Thành phố Quảng Ngãi',
      path: 'Quảng Ngãi, Quảng Ngãi',
      path_with_type:
        'Thành phố Quảng Ngãi, Tỉnh Quảng Ngãi',
      code: '522',
      parent_code: '51',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354e7',
      name: 'Bình Sơn',
      type: 'huyen',
      slug: 'binh-son',
      name_with_type: 'Huyện Bình Sơn',
      path: 'Bình Sơn, Quảng Ngãi',
      path_with_type:
        'Huyện Bình Sơn, Tỉnh Quảng Ngãi',
      code: '524',
      parent_code: '51',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354e8',
      name: 'Trà Bồng',
      type: 'huyen',
      slug: 'tra-bong',
      name_with_type: 'Huyện Trà Bồng',
      path: 'Trà Bồng, Quảng Ngãi',
      path_with_type:
        'Huyện Trà Bồng, Tỉnh Quảng Ngãi',
      code: '525',
      parent_code: '51',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354e9',
      name: 'Sơn Tịnh',
      type: 'huyen',
      slug: 'son-tinh',
      name_with_type: 'Huyện Sơn Tịnh',
      path: 'Sơn Tịnh, Quảng Ngãi',
      path_with_type:
        'Huyện Sơn Tịnh, Tỉnh Quảng Ngãi',
      code: '527',
      parent_code: '51',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354ea',
      name: 'Tư Nghĩa',
      type: 'huyen',
      slug: 'tu-nghia',
      name_with_type: 'Huyện Tư Nghĩa',
      path: 'Tư Nghĩa, Quảng Ngãi',
      path_with_type:
        'Huyện Tư Nghĩa, Tỉnh Quảng Ngãi',
      code: '528',
      parent_code: '51',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354eb',
      name: 'Sơn Hà',
      type: 'huyen',
      slug: 'son-ha',
      name_with_type: 'Huyện Sơn Hà',
      path: 'Sơn Hà, Quảng Ngãi',
      path_with_type:
        'Huyện Sơn Hà, Tỉnh Quảng Ngãi',
      code: '529',
      parent_code: '51',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354ec',
      name: 'Sơn Tây',
      type: 'huyen',
      slug: 'son-tay',
      name_with_type: 'Huyện Sơn Tây',
      path: 'Sơn Tây, Quảng Ngãi',
      path_with_type:
        'Huyện Sơn Tây, Tỉnh Quảng Ngãi',
      code: '530',
      parent_code: '51',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354ed',
      name: 'Minh Long',
      type: 'huyen',
      slug: 'minh-long',
      name_with_type: 'Huyện Minh Long',
      path: 'Minh Long, Quảng Ngãi',
      path_with_type:
        'Huyện Minh Long, Tỉnh Quảng Ngãi',
      code: '531',
      parent_code: '51',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354ee',
      name: 'Nghĩa Hành',
      type: 'huyen',
      slug: 'nghia-hanh',
      name_with_type: 'Huyện Nghĩa Hành',
      path: 'Nghĩa Hành, Quảng Ngãi',
      path_with_type:
        'Huyện Nghĩa Hành, Tỉnh Quảng Ngãi',
      code: '532',
      parent_code: '51',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354ef',
      name: 'Mộ Đức',
      type: 'huyen',
      slug: 'mo-duc',
      name_with_type: 'Huyện Mộ Đức',
      path: 'Mộ Đức, Quảng Ngãi',
      path_with_type:
        'Huyện Mộ Đức, Tỉnh Quảng Ngãi',
      code: '533',
      parent_code: '51',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354f0',
      name: 'Đức Phổ',
      type: 'thi-xa',
      slug: 'duc-pho',
      name_with_type: 'Thị xã Đức Phổ',
      path: 'Đức Phổ, Quảng Ngãi',
      path_with_type:
        'Thị xã Đức Phổ, Tỉnh Quảng Ngãi',
      code: '534',
      parent_code: '51',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354f1',
      name: 'Ba Tơ',
      type: 'huyen',
      slug: 'ba-to',
      name_with_type: 'Huyện Ba Tơ',
      path: 'Ba Tơ, Quảng Ngãi',
      path_with_type:
        'Huyện Ba Tơ, Tỉnh Quảng Ngãi',
      code: '535',
      parent_code: '51',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354f2',
      name: 'Lý Sơn',
      type: 'huyen',
      slug: 'ly-son',
      name_with_type: 'Huyện Lý Sơn',
      path: 'Lý Sơn, Quảng Ngãi',
      path_with_type:
        'Huyện Lý Sơn, Tỉnh Quảng Ngãi',
      code: '536',
      parent_code: '51',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354f3',
      name: 'Quy Nhơn',
      type: 'thanh-pho',
      slug: 'quy-nhon',
      name_with_type: 'Thành phố Quy Nhơn',
      path: 'Quy Nhơn, Bình Định',
      path_with_type:
        'Thành phố Quy Nhơn, Tỉnh Bình Định',
      code: '540',
      parent_code: '52',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354f4',
      name: 'An Lão',
      type: 'huyen',
      slug: 'an-lao',
      name_with_type: 'Huyện An Lão',
      path: 'An Lão, Bình Định',
      path_with_type:
        'Huyện An Lão, Tỉnh Bình Định',
      code: '542',
      parent_code: '52',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354f5',
      name: 'Hoài Nhơn',
      type: 'thi-xa',
      slug: 'hoai-nhon',
      name_with_type: 'Thị xã Hoài Nhơn',
      path: 'Hoài Nhơn, Bình Định',
      path_with_type:
        'Thị xã Hoài Nhơn, Tỉnh Bình Định',
      code: '543',
      parent_code: '52',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354f6',
      name: 'Hoài Ân',
      type: 'huyen',
      slug: 'hoai-an',
      name_with_type: 'Huyện Hoài Ân',
      path: 'Hoài Ân, Bình Định',
      path_with_type:
        'Huyện Hoài Ân, Tỉnh Bình Định',
      code: '544',
      parent_code: '52',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354f7',
      name: 'Phù Mỹ',
      type: 'huyen',
      slug: 'phu-my',
      name_with_type: 'Huyện Phù Mỹ',
      path: 'Phù Mỹ, Bình Định',
      path_with_type:
        'Huyện Phù Mỹ, Tỉnh Bình Định',
      code: '545',
      parent_code: '52',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354f8',
      name: 'Vĩnh Thạnh',
      type: 'huyen',
      slug: 'vinh-thanh',
      name_with_type: 'Huyện Vĩnh Thạnh',
      path: 'Vĩnh Thạnh, Bình Định',
      path_with_type:
        'Huyện Vĩnh Thạnh, Tỉnh Bình Định',
      code: '546',
      parent_code: '52',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354f9',
      name: 'Tây Sơn',
      type: 'huyen',
      slug: 'tay-son',
      name_with_type: 'Huyện Tây Sơn',
      path: 'Tây Sơn, Bình Định',
      path_with_type:
        'Huyện Tây Sơn, Tỉnh Bình Định',
      code: '547',
      parent_code: '52',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354fa',
      name: 'Phù Cát',
      type: 'huyen',
      slug: 'phu-cat',
      name_with_type: 'Huyện Phù Cát',
      path: 'Phù Cát, Bình Định',
      path_with_type:
        'Huyện Phù Cát, Tỉnh Bình Định',
      code: '548',
      parent_code: '52',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354fb',
      name: 'An Nhơn',
      type: 'thi-xa',
      slug: 'an-nhon',
      name_with_type: 'Thị xã An Nhơn',
      path: 'An Nhơn, Bình Định',
      path_with_type:
        'Thị xã An Nhơn, Tỉnh Bình Định',
      code: '549',
      parent_code: '52',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354fc',
      name: 'Tuy Phước',
      type: 'huyen',
      slug: 'tuy-phuoc',
      name_with_type: 'Huyện Tuy Phước',
      path: 'Tuy Phước, Bình Định',
      path_with_type:
        'Huyện Tuy Phước, Tỉnh Bình Định',
      code: '550',
      parent_code: '52',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354fd',
      name: 'Vân Canh',
      type: 'huyen',
      slug: 'van-canh',
      name_with_type: 'Huyện Vân Canh',
      path: 'Vân Canh, Bình Định',
      path_with_type:
        'Huyện Vân Canh, Tỉnh Bình Định',
      code: '551',
      parent_code: '52',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354fe',
      name: 'Tuy Hoà',
      type: 'thanh-pho',
      slug: 'tuy-hoa',
      name_with_type: 'Thành phố Tuy Hoà',
      path: 'Tuy Hoà, Phú Yên',
      path_with_type:
        'Thành phố Tuy Hoà, Tỉnh Phú Yên',
      code: '555',
      parent_code: '54',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c354ff',
      name: 'Sông Cầu',
      type: 'thi-xa',
      slug: 'song-cau',
      name_with_type: 'Thị xã Sông Cầu',
      path: 'Sông Cầu, Phú Yên',
      path_with_type:
        'Thị xã Sông Cầu, Tỉnh Phú Yên',
      code: '557',
      parent_code: '54',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35500',
      name: 'Đồng Xuân',
      type: 'huyen',
      slug: 'dong-xuan',
      name_with_type: 'Huyện Đồng Xuân',
      path: 'Đồng Xuân, Phú Yên',
      path_with_type:
        'Huyện Đồng Xuân, Tỉnh Phú Yên',
      code: '558',
      parent_code: '54',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35501',
      name: 'Tuy An',
      type: 'huyen',
      slug: 'tuy-an',
      name_with_type: 'Huyện Tuy An',
      path: 'Tuy An, Phú Yên',
      path_with_type:
        'Huyện Tuy An, Tỉnh Phú Yên',
      code: '559',
      parent_code: '54',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35502',
      name: 'Sơn Hòa',
      type: 'huyen',
      slug: 'son-hoa',
      name_with_type: 'Huyện Sơn Hòa',
      path: 'Sơn Hòa, Phú Yên',
      path_with_type:
        'Huyện Sơn Hòa, Tỉnh Phú Yên',
      code: '560',
      parent_code: '54',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35503',
      name: 'Sông Hinh',
      type: 'huyen',
      slug: 'song-hinh',
      name_with_type: 'Huyện Sông Hinh',
      path: 'Sông Hinh, Phú Yên',
      path_with_type:
        'Huyện Sông Hinh, Tỉnh Phú Yên',
      code: '561',
      parent_code: '54',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35504',
      name: 'Tây Hoà',
      type: 'huyen',
      slug: 'tay-hoa',
      name_with_type: 'Huyện Tây Hoà',
      path: 'Tây Hoà, Phú Yên',
      path_with_type:
        'Huyện Tây Hoà, Tỉnh Phú Yên',
      code: '562',
      parent_code: '54',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35505',
      name: 'Phú Hoà',
      type: 'huyen',
      slug: 'phu-hoa',
      name_with_type: 'Huyện Phú Hoà',
      path: 'Phú Hoà, Phú Yên',
      path_with_type:
        'Huyện Phú Hoà, Tỉnh Phú Yên',
      code: '563',
      parent_code: '54',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35506',
      name: 'Đông Hòa',
      type: 'thi-xa',
      slug: 'dong-hoa',
      name_with_type: 'Thị xã Đông Hòa',
      path: 'Đông Hòa, Phú Yên',
      path_with_type:
        'Thị xã Đông Hòa, Tỉnh Phú Yên',
      code: '564',
      parent_code: '54',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35507',
      name: 'Nha Trang',
      type: 'thanh-pho',
      slug: 'nha-trang',
      name_with_type: 'Thành phố Nha Trang',
      path: 'Nha Trang, Khánh Hòa',
      path_with_type:
        'Thành phố Nha Trang, Tỉnh Khánh Hòa',
      code: '568',
      parent_code: '56',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35508',
      name: 'Cam Ranh',
      type: 'thanh-pho',
      slug: 'cam-ranh',
      name_with_type: 'Thành phố Cam Ranh',
      path: 'Cam Ranh, Khánh Hòa',
      path_with_type:
        'Thành phố Cam Ranh, Tỉnh Khánh Hòa',
      code: '569',
      parent_code: '56',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35509',
      name: 'Cam Lâm',
      type: 'huyen',
      slug: 'cam-lam',
      name_with_type: 'Huyện Cam Lâm',
      path: 'Cam Lâm, Khánh Hòa',
      path_with_type:
        'Huyện Cam Lâm, Tỉnh Khánh Hòa',
      code: '570',
      parent_code: '56',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3550a',
      name: 'Vạn Ninh',
      type: 'huyen',
      slug: 'van-ninh',
      name_with_type: 'Huyện Vạn Ninh',
      path: 'Vạn Ninh, Khánh Hòa',
      path_with_type:
        'Huyện Vạn Ninh, Tỉnh Khánh Hòa',
      code: '571',
      parent_code: '56',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3550b',
      name: 'Ninh Hòa',
      type: 'thi-xa',
      slug: 'ninh-hoa',
      name_with_type: 'Thị xã Ninh Hòa',
      path: 'Ninh Hòa, Khánh Hòa',
      path_with_type:
        'Thị xã Ninh Hòa, Tỉnh Khánh Hòa',
      code: '572',
      parent_code: '56',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3550c',
      name: 'Khánh Vĩnh',
      type: 'huyen',
      slug: 'khanh-vinh',
      name_with_type: 'Huyện Khánh Vĩnh',
      path: 'Khánh Vĩnh, Khánh Hòa',
      path_with_type:
        'Huyện Khánh Vĩnh, Tỉnh Khánh Hòa',
      code: '573',
      parent_code: '56',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3550d',
      name: 'Diên Khánh',
      type: 'huyen',
      slug: 'dien-khanh',
      name_with_type: 'Huyện Diên Khánh',
      path: 'Diên Khánh, Khánh Hòa',
      path_with_type:
        'Huyện Diên Khánh, Tỉnh Khánh Hòa',
      code: '574',
      parent_code: '56',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3550e',
      name: 'Khánh Sơn',
      type: 'huyen',
      slug: 'khanh-son',
      name_with_type: 'Huyện Khánh Sơn',
      path: 'Khánh Sơn, Khánh Hòa',
      path_with_type:
        'Huyện Khánh Sơn, Tỉnh Khánh Hòa',
      code: '575',
      parent_code: '56',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3550f',
      name: 'Trường Sa',
      type: 'huyen',
      slug: 'truong-sa',
      name_with_type: 'Huyện Trường Sa',
      path: 'Trường Sa, Khánh Hòa',
      path_with_type:
        'Huyện Trường Sa, Tỉnh Khánh Hòa',
      code: '576',
      parent_code: '56',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35510',
      name: 'Phan Rang-Tháp Chàm',
      type: 'thanh-pho',
      slug: 'phan-rang-thap-cham',
      name_with_type:
        'Thành phố Phan Rang-Tháp Chàm',
      path: 'Phan Rang-Tháp Chàm, Ninh Thuận',
      path_with_type:
        'Thành phố Phan Rang-Tháp Chàm, Tỉnh Ninh Thuận',
      code: '582',
      parent_code: '58',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35511',
      name: 'Bác Ái',
      type: 'huyen',
      slug: 'bac-ai',
      name_with_type: 'Huyện Bác Ái',
      path: 'Bác Ái, Ninh Thuận',
      path_with_type:
        'Huyện Bác Ái, Tỉnh Ninh Thuận',
      code: '584',
      parent_code: '58',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35512',
      name: 'Ninh Sơn',
      type: 'huyen',
      slug: 'ninh-son',
      name_with_type: 'Huyện Ninh Sơn',
      path: 'Ninh Sơn, Ninh Thuận',
      path_with_type:
        'Huyện Ninh Sơn, Tỉnh Ninh Thuận',
      code: '585',
      parent_code: '58',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35513',
      name: 'Ninh Hải',
      type: 'huyen',
      slug: 'ninh-hai',
      name_with_type: 'Huyện Ninh Hải',
      path: 'Ninh Hải, Ninh Thuận',
      path_with_type:
        'Huyện Ninh Hải, Tỉnh Ninh Thuận',
      code: '586',
      parent_code: '58',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35514',
      name: 'Ninh Phước',
      type: 'huyen',
      slug: 'ninh-phuoc',
      name_with_type: 'Huyện Ninh Phước',
      path: 'Ninh Phước, Ninh Thuận',
      path_with_type:
        'Huyện Ninh Phước, Tỉnh Ninh Thuận',
      code: '587',
      parent_code: '58',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35515',
      name: 'Thuận Bắc',
      type: 'huyen',
      slug: 'thuan-bac',
      name_with_type: 'Huyện Thuận Bắc',
      path: 'Thuận Bắc, Ninh Thuận',
      path_with_type:
        'Huyện Thuận Bắc, Tỉnh Ninh Thuận',
      code: '588',
      parent_code: '58',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35516',
      name: 'Thuận Nam',
      type: 'huyen',
      slug: 'thuan-nam',
      name_with_type: 'Huyện Thuận Nam',
      path: 'Thuận Nam, Ninh Thuận',
      path_with_type:
        'Huyện Thuận Nam, Tỉnh Ninh Thuận',
      code: '589',
      parent_code: '58',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35517',
      name: 'Phan Thiết',
      type: 'thanh-pho',
      slug: 'phan-thiet',
      name_with_type: 'Thành phố Phan Thiết',
      path: 'Phan Thiết, Bình Thuận',
      path_with_type:
        'Thành phố Phan Thiết, Tỉnh Bình Thuận',
      code: '593',
      parent_code: '60',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35518',
      name: 'La Gi',
      type: 'thi-xa',
      slug: 'la-gi',
      name_with_type: 'Thị xã La Gi',
      path: 'La Gi, Bình Thuận',
      path_with_type:
        'Thị xã La Gi, Tỉnh Bình Thuận',
      code: '594',
      parent_code: '60',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35519',
      name: 'Tuy Phong',
      type: 'huyen',
      slug: 'tuy-phong',
      name_with_type: 'Huyện Tuy Phong',
      path: 'Tuy Phong, Bình Thuận',
      path_with_type:
        'Huyện Tuy Phong, Tỉnh Bình Thuận',
      code: '595',
      parent_code: '60',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3551a',
      name: 'Bắc Bình',
      type: 'huyen',
      slug: 'bac-binh',
      name_with_type: 'Huyện Bắc Bình',
      path: 'Bắc Bình, Bình Thuận',
      path_with_type:
        'Huyện Bắc Bình, Tỉnh Bình Thuận',
      code: '596',
      parent_code: '60',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3551b',
      name: 'Hàm Thuận Bắc',
      type: 'huyen',
      slug: 'ham-thuan-bac',
      name_with_type: 'Huyện Hàm Thuận Bắc',
      path: 'Hàm Thuận Bắc, Bình Thuận',
      path_with_type:
        'Huyện Hàm Thuận Bắc, Tỉnh Bình Thuận',
      code: '597',
      parent_code: '60',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3551c',
      name: 'Hàm Thuận Nam',
      type: 'huyen',
      slug: 'ham-thuan-nam',
      name_with_type: 'Huyện Hàm Thuận Nam',
      path: 'Hàm Thuận Nam, Bình Thuận',
      path_with_type:
        'Huyện Hàm Thuận Nam, Tỉnh Bình Thuận',
      code: '598',
      parent_code: '60',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3551d',
      name: 'Tánh Linh',
      type: 'huyen',
      slug: 'tanh-linh',
      name_with_type: 'Huyện Tánh Linh',
      path: 'Tánh Linh, Bình Thuận',
      path_with_type:
        'Huyện Tánh Linh, Tỉnh Bình Thuận',
      code: '599',
      parent_code: '60',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3551e',
      name: 'Đức Linh',
      type: 'huyen',
      slug: 'duc-linh',
      name_with_type: 'Huyện Đức Linh',
      path: 'Đức Linh, Bình Thuận',
      path_with_type:
        'Huyện Đức Linh, Tỉnh Bình Thuận',
      code: '600',
      parent_code: '60',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3551f',
      name: 'Hàm Tân',
      type: 'huyen',
      slug: 'ham-tan',
      name_with_type: 'Huyện Hàm Tân',
      path: 'Hàm Tân, Bình Thuận',
      path_with_type:
        'Huyện Hàm Tân, Tỉnh Bình Thuận',
      code: '601',
      parent_code: '60',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35520',
      name: 'Phú Quí',
      type: 'huyen',
      slug: 'phu-qui',
      name_with_type: 'Huyện Phú Quí',
      path: 'Phú Quí, Bình Thuận',
      path_with_type:
        'Huyện Phú Quí, Tỉnh Bình Thuận',
      code: '602',
      parent_code: '60',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35521',
      name: 'Kon Tum',
      type: 'thanh-pho',
      slug: 'kon-tum',
      name_with_type: 'Thành phố Kon Tum',
      path: 'Kon Tum, Kon Tum',
      path_with_type:
        'Thành phố Kon Tum, Tỉnh Kon Tum',
      code: '608',
      parent_code: '62',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35522',
      name: 'Đắk Glei',
      type: 'huyen',
      slug: 'dak-glei',
      name_with_type: 'Huyện Đắk Glei',
      path: 'Đắk Glei, Kon Tum',
      path_with_type:
        'Huyện Đắk Glei, Tỉnh Kon Tum',
      code: '610',
      parent_code: '62',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35523',
      name: 'Ngọc Hồi',
      type: 'huyen',
      slug: 'ngoc-hoi',
      name_with_type: 'Huyện Ngọc Hồi',
      path: 'Ngọc Hồi, Kon Tum',
      path_with_type:
        'Huyện Ngọc Hồi, Tỉnh Kon Tum',
      code: '611',
      parent_code: '62',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35524',
      name: 'Đắk Tô',
      type: 'huyen',
      slug: 'dak-to',
      name_with_type: 'Huyện Đắk Tô',
      path: 'Đắk Tô, Kon Tum',
      path_with_type:
        'Huyện Đắk Tô, Tỉnh Kon Tum',
      code: '612',
      parent_code: '62',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35525',
      name: 'Kon Plông',
      type: 'huyen',
      slug: 'kon-plong',
      name_with_type: 'Huyện Kon Plông',
      path: 'Kon Plông, Kon Tum',
      path_with_type:
        'Huyện Kon Plông, Tỉnh Kon Tum',
      code: '613',
      parent_code: '62',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35526',
      name: 'Kon Rẫy',
      type: 'huyen',
      slug: 'kon-ray',
      name_with_type: 'Huyện Kon Rẫy',
      path: 'Kon Rẫy, Kon Tum',
      path_with_type:
        'Huyện Kon Rẫy, Tỉnh Kon Tum',
      code: '614',
      parent_code: '62',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35527',
      name: 'Đắk Hà',
      type: 'huyen',
      slug: 'dak-ha',
      name_with_type: 'Huyện Đắk Hà',
      path: 'Đắk Hà, Kon Tum',
      path_with_type:
        'Huyện Đắk Hà, Tỉnh Kon Tum',
      code: '615',
      parent_code: '62',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35528',
      name: 'Sa Thầy',
      type: 'huyen',
      slug: 'sa-thay',
      name_with_type: 'Huyện Sa Thầy',
      path: 'Sa Thầy, Kon Tum',
      path_with_type:
        'Huyện Sa Thầy, Tỉnh Kon Tum',
      code: '616',
      parent_code: '62',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35529',
      name: 'Tu Mơ Rông',
      type: 'huyen',
      slug: 'tu-mo-rong',
      name_with_type: 'Huyện Tu Mơ Rông',
      path: 'Tu Mơ Rông, Kon Tum',
      path_with_type:
        'Huyện Tu Mơ Rông, Tỉnh Kon Tum',
      code: '617',
      parent_code: '62',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3552a',
      name: "Ia H' Drai",
      type: 'huyen',
      slug: "ia-h'-drai",
      name_with_type: "Huyện Ia H' Drai",
      path: "Ia H' Drai, Kon Tum",
      path_with_type:
        "Huyện Ia H' Drai, Tỉnh Kon Tum",
      code: '618',
      parent_code: '62',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3552b',
      name: 'Pleiku',
      type: 'thanh-pho',
      slug: 'pleiku',
      name_with_type: 'Thành phố Pleiku',
      path: 'Pleiku, Gia Lai',
      path_with_type:
        'Thành phố Pleiku, Tỉnh Gia Lai',
      code: '622',
      parent_code: '64',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3552c',
      name: 'An Khê',
      type: 'thi-xa',
      slug: 'an-khe',
      name_with_type: 'Thị xã An Khê',
      path: 'An Khê, Gia Lai',
      path_with_type:
        'Thị xã An Khê, Tỉnh Gia Lai',
      code: '623',
      parent_code: '64',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3552d',
      name: 'Ayun Pa',
      type: 'thi-xa',
      slug: 'ayun-pa',
      name_with_type: 'Thị xã Ayun Pa',
      path: 'Ayun Pa, Gia Lai',
      path_with_type:
        'Thị xã Ayun Pa, Tỉnh Gia Lai',
      code: '624',
      parent_code: '64',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3552e',
      name: 'KBang',
      type: 'huyen',
      slug: 'kbang',
      name_with_type: 'Huyện KBang',
      path: 'KBang, Gia Lai',
      path_with_type: 'Huyện KBang, Tỉnh Gia Lai',
      code: '625',
      parent_code: '64',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3552f',
      name: 'Đăk Đoa',
      type: 'huyen',
      slug: 'dak-doa',
      name_with_type: 'Huyện Đăk Đoa',
      path: 'Đăk Đoa, Gia Lai',
      path_with_type:
        'Huyện Đăk Đoa, Tỉnh Gia Lai',
      code: '626',
      parent_code: '64',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35530',
      name: 'Chư Păh',
      type: 'huyen',
      slug: 'chu-pah',
      name_with_type: 'Huyện Chư Păh',
      path: 'Chư Păh, Gia Lai',
      path_with_type:
        'Huyện Chư Păh, Tỉnh Gia Lai',
      code: '627',
      parent_code: '64',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35531',
      name: 'Ia Grai',
      type: 'huyen',
      slug: 'ia-grai',
      name_with_type: 'Huyện Ia Grai',
      path: 'Ia Grai, Gia Lai',
      path_with_type:
        'Huyện Ia Grai, Tỉnh Gia Lai',
      code: '628',
      parent_code: '64',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35532',
      name: 'Mang Yang',
      type: 'huyen',
      slug: 'mang-yang',
      name_with_type: 'Huyện Mang Yang',
      path: 'Mang Yang, Gia Lai',
      path_with_type:
        'Huyện Mang Yang, Tỉnh Gia Lai',
      code: '629',
      parent_code: '64',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35533',
      name: 'Kông Chro',
      type: 'huyen',
      slug: 'kong-chro',
      name_with_type: 'Huyện Kông Chro',
      path: 'Kông Chro, Gia Lai',
      path_with_type:
        'Huyện Kông Chro, Tỉnh Gia Lai',
      code: '630',
      parent_code: '64',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35534',
      name: 'Đức Cơ',
      type: 'huyen',
      slug: 'duc-co',
      name_with_type: 'Huyện Đức Cơ',
      path: 'Đức Cơ, Gia Lai',
      path_with_type:
        'Huyện Đức Cơ, Tỉnh Gia Lai',
      code: '631',
      parent_code: '64',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35535',
      name: 'Chư Prông',
      type: 'huyen',
      slug: 'chu-prong',
      name_with_type: 'Huyện Chư Prông',
      path: 'Chư Prông, Gia Lai',
      path_with_type:
        'Huyện Chư Prông, Tỉnh Gia Lai',
      code: '632',
      parent_code: '64',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35536',
      name: 'Chư Sê',
      type: 'huyen',
      slug: 'chu-se',
      name_with_type: 'Huyện Chư Sê',
      path: 'Chư Sê, Gia Lai',
      path_with_type:
        'Huyện Chư Sê, Tỉnh Gia Lai',
      code: '633',
      parent_code: '64',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35537',
      name: 'Đăk Pơ',
      type: 'huyen',
      slug: 'dak-po',
      name_with_type: 'Huyện Đăk Pơ',
      path: 'Đăk Pơ, Gia Lai',
      path_with_type:
        'Huyện Đăk Pơ, Tỉnh Gia Lai',
      code: '634',
      parent_code: '64',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35538',
      name: 'Ia Pa',
      type: 'huyen',
      slug: 'ia-pa',
      name_with_type: 'Huyện Ia Pa',
      path: 'Ia Pa, Gia Lai',
      path_with_type: 'Huyện Ia Pa, Tỉnh Gia Lai',
      code: '635',
      parent_code: '64',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35539',
      name: 'Krông Pa',
      type: 'huyen',
      slug: 'krong-pa',
      name_with_type: 'Huyện Krông Pa',
      path: 'Krông Pa, Gia Lai',
      path_with_type:
        'Huyện Krông Pa, Tỉnh Gia Lai',
      code: '637',
      parent_code: '64',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3553a',
      name: 'Phú Thiện',
      type: 'huyen',
      slug: 'phu-thien',
      name_with_type: 'Huyện Phú Thiện',
      path: 'Phú Thiện, Gia Lai',
      path_with_type:
        'Huyện Phú Thiện, Tỉnh Gia Lai',
      code: '638',
      parent_code: '64',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3553b',
      name: 'Chư Pưh',
      type: 'huyen',
      slug: 'chu-puh',
      name_with_type: 'Huyện Chư Pưh',
      path: 'Chư Pưh, Gia Lai',
      path_with_type:
        'Huyện Chư Pưh, Tỉnh Gia Lai',
      code: '639',
      parent_code: '64',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3553c',
      name: 'Buôn Ma Thuột',
      type: 'thanh-pho',
      slug: 'buon-ma-thuot',
      name_with_type: 'Thành phố Buôn Ma Thuột',
      path: 'Buôn Ma Thuột, Đắk Lắk',
      path_with_type:
        'Thành phố Buôn Ma Thuột, Tỉnh Đắk Lắk',
      code: '643',
      parent_code: '66',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3553d',
      name: 'Buôn Hồ',
      type: 'thi-xa',
      slug: 'buon-ho',
      name_with_type: 'Thị Xã Buôn Hồ',
      path: 'Buôn Hồ, Đắk Lắk',
      path_with_type:
        'Thị Xã Buôn Hồ, Tỉnh Đắk Lắk',
      code: '644',
      parent_code: '66',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3553e',
      name: "Ea H'leo",
      type: 'huyen',
      slug: "ea-h'leo",
      name_with_type: "Huyện Ea H'leo",
      path: "Ea H'leo, Đắk Lắk",
      path_with_type:
        "Huyện Ea H'leo, Tỉnh Đắk Lắk",
      code: '645',
      parent_code: '66',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3553f',
      name: 'Ea Súp',
      type: 'huyen',
      slug: 'ea-sup',
      name_with_type: 'Huyện Ea Súp',
      path: 'Ea Súp, Đắk Lắk',
      path_with_type:
        'Huyện Ea Súp, Tỉnh Đắk Lắk',
      code: '646',
      parent_code: '66',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35540',
      name: 'Buôn Đôn',
      type: 'huyen',
      slug: 'buon-don',
      name_with_type: 'Huyện Buôn Đôn',
      path: 'Buôn Đôn, Đắk Lắk',
      path_with_type:
        'Huyện Buôn Đôn, Tỉnh Đắk Lắk',
      code: '647',
      parent_code: '66',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35541',
      name: "Cư M'gar",
      type: 'huyen',
      slug: "cu-m'gar",
      name_with_type: "Huyện Cư M'gar",
      path: "Cư M'gar, Đắk Lắk",
      path_with_type:
        "Huyện Cư M'gar, Tỉnh Đắk Lắk",
      code: '648',
      parent_code: '66',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35542',
      name: 'Krông Búk',
      type: 'huyen',
      slug: 'krong-buk',
      name_with_type: 'Huyện Krông Búk',
      path: 'Krông Búk, Đắk Lắk',
      path_with_type:
        'Huyện Krông Búk, Tỉnh Đắk Lắk',
      code: '649',
      parent_code: '66',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35543',
      name: 'Krông Năng',
      type: 'huyen',
      slug: 'krong-nang',
      name_with_type: 'Huyện Krông Năng',
      path: 'Krông Năng, Đắk Lắk',
      path_with_type:
        'Huyện Krông Năng, Tỉnh Đắk Lắk',
      code: '650',
      parent_code: '66',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35544',
      name: 'Ea Kar',
      type: 'huyen',
      slug: 'ea-kar',
      name_with_type: 'Huyện Ea Kar',
      path: 'Ea Kar, Đắk Lắk',
      path_with_type:
        'Huyện Ea Kar, Tỉnh Đắk Lắk',
      code: '651',
      parent_code: '66',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35545',
      name: "M'Đrắk",
      type: 'huyen',
      slug: "m'drak",
      name_with_type: "Huyện M'Đrắk",
      path: "M'Đrắk, Đắk Lắk",
      path_with_type:
        "Huyện M'Đrắk, Tỉnh Đắk Lắk",
      code: '652',
      parent_code: '66',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35546',
      name: 'Krông Bông',
      type: 'huyen',
      slug: 'krong-bong',
      name_with_type: 'Huyện Krông Bông',
      path: 'Krông Bông, Đắk Lắk',
      path_with_type:
        'Huyện Krông Bông, Tỉnh Đắk Lắk',
      code: '653',
      parent_code: '66',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35547',
      name: 'Krông Pắc',
      type: 'huyen',
      slug: 'krong-pac',
      name_with_type: 'Huyện Krông Pắc',
      path: 'Krông Pắc, Đắk Lắk',
      path_with_type:
        'Huyện Krông Pắc, Tỉnh Đắk Lắk',
      code: '654',
      parent_code: '66',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35548',
      name: 'Krông A Na',
      type: 'huyen',
      slug: 'krong-a-na',
      name_with_type: 'Huyện Krông A Na',
      path: 'Krông A Na, Đắk Lắk',
      path_with_type:
        'Huyện Krông A Na, Tỉnh Đắk Lắk',
      code: '655',
      parent_code: '66',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35549',
      name: 'Lắk',
      type: 'huyen',
      slug: 'lak',
      name_with_type: 'Huyện Lắk',
      path: 'Lắk, Đắk Lắk',
      path_with_type: 'Huyện Lắk, Tỉnh Đắk Lắk',
      code: '656',
      parent_code: '66',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3554a',
      name: 'Cư Kuin',
      type: 'huyen',
      slug: 'cu-kuin',
      name_with_type: 'Huyện Cư Kuin',
      path: 'Cư Kuin, Đắk Lắk',
      path_with_type:
        'Huyện Cư Kuin, Tỉnh Đắk Lắk',
      code: '657',
      parent_code: '66',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3554b',
      name: 'Gia Nghĩa',
      type: 'thanh-pho',
      slug: 'gia-nghia',
      name_with_type: 'Thành phố Gia Nghĩa',
      path: 'Gia Nghĩa, Đắk Nông',
      path_with_type:
        'Thành phố Gia Nghĩa, Tỉnh Đắk Nông',
      code: '660',
      parent_code: '67',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3554c',
      name: 'Đăk Glong',
      type: 'huyen',
      slug: 'dak-glong',
      name_with_type: 'Huyện Đăk Glong',
      path: 'Đăk Glong, Đắk Nông',
      path_with_type:
        'Huyện Đăk Glong, Tỉnh Đắk Nông',
      code: '661',
      parent_code: '67',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3554d',
      name: 'Cư Jút',
      type: 'huyen',
      slug: 'cu-jut',
      name_with_type: 'Huyện Cư Jút',
      path: 'Cư Jút, Đắk Nông',
      path_with_type:
        'Huyện Cư Jút, Tỉnh Đắk Nông',
      code: '662',
      parent_code: '67',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3554e',
      name: 'Đắk Mil',
      type: 'huyen',
      slug: 'dak-mil',
      name_with_type: 'Huyện Đắk Mil',
      path: 'Đắk Mil, Đắk Nông',
      path_with_type:
        'Huyện Đắk Mil, Tỉnh Đắk Nông',
      code: '663',
      parent_code: '67',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3554f',
      name: 'Krông Nô',
      type: 'huyen',
      slug: 'krong-no',
      name_with_type: 'Huyện Krông Nô',
      path: 'Krông Nô, Đắk Nông',
      path_with_type:
        'Huyện Krông Nô, Tỉnh Đắk Nông',
      code: '664',
      parent_code: '67',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35550',
      name: 'Đắk Song',
      type: 'huyen',
      slug: 'dak-song',
      name_with_type: 'Huyện Đắk Song',
      path: 'Đắk Song, Đắk Nông',
      path_with_type:
        'Huyện Đắk Song, Tỉnh Đắk Nông',
      code: '665',
      parent_code: '67',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35551',
      name: "Đắk R'Lấp",
      type: 'huyen',
      slug: "dak-r'lap",
      name_with_type: "Huyện Đắk R'Lấp",
      path: "Đắk R'Lấp, Đắk Nông",
      path_with_type:
        "Huyện Đắk R'Lấp, Tỉnh Đắk Nông",
      code: '666',
      parent_code: '67',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35552',
      name: 'Tuy Đức',
      type: 'huyen',
      slug: 'tuy-duc',
      name_with_type: 'Huyện Tuy Đức',
      path: 'Tuy Đức, Đắk Nông',
      path_with_type:
        'Huyện Tuy Đức, Tỉnh Đắk Nông',
      code: '667',
      parent_code: '67',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35553',
      name: 'Đà Lạt',
      type: 'thanh-pho',
      slug: 'da-lat',
      name_with_type: 'Thành phố Đà Lạt',
      path: 'Đà Lạt, Lâm Đồng',
      path_with_type:
        'Thành phố Đà Lạt, Tỉnh Lâm Đồng',
      code: '672',
      parent_code: '68',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35554',
      name: 'Bảo Lộc',
      type: 'thanh-pho',
      slug: 'bao-loc',
      name_with_type: 'Thành phố Bảo Lộc',
      path: 'Bảo Lộc, Lâm Đồng',
      path_with_type:
        'Thành phố Bảo Lộc, Tỉnh Lâm Đồng',
      code: '673',
      parent_code: '68',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35555',
      name: 'Đam Rông',
      type: 'huyen',
      slug: 'dam-rong',
      name_with_type: 'Huyện Đam Rông',
      path: 'Đam Rông, Lâm Đồng',
      path_with_type:
        'Huyện Đam Rông, Tỉnh Lâm Đồng',
      code: '674',
      parent_code: '68',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35556',
      name: 'Lạc Dương',
      type: 'huyen',
      slug: 'lac-duong',
      name_with_type: 'Huyện Lạc Dương',
      path: 'Lạc Dương, Lâm Đồng',
      path_with_type:
        'Huyện Lạc Dương, Tỉnh Lâm Đồng',
      code: '675',
      parent_code: '68',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35557',
      name: 'Lâm Hà',
      type: 'huyen',
      slug: 'lam-ha',
      name_with_type: 'Huyện Lâm Hà',
      path: 'Lâm Hà, Lâm Đồng',
      path_with_type:
        'Huyện Lâm Hà, Tỉnh Lâm Đồng',
      code: '676',
      parent_code: '68',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35558',
      name: 'Đơn Dương',
      type: 'huyen',
      slug: 'don-duong',
      name_with_type: 'Huyện Đơn Dương',
      path: 'Đơn Dương, Lâm Đồng',
      path_with_type:
        'Huyện Đơn Dương, Tỉnh Lâm Đồng',
      code: '677',
      parent_code: '68',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35559',
      name: 'Đức Trọng',
      type: 'huyen',
      slug: 'duc-trong',
      name_with_type: 'Huyện Đức Trọng',
      path: 'Đức Trọng, Lâm Đồng',
      path_with_type:
        'Huyện Đức Trọng, Tỉnh Lâm Đồng',
      code: '678',
      parent_code: '68',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3555a',
      name: 'Di Linh',
      type: 'huyen',
      slug: 'di-linh',
      name_with_type: 'Huyện Di Linh',
      path: 'Di Linh, Lâm Đồng',
      path_with_type:
        'Huyện Di Linh, Tỉnh Lâm Đồng',
      code: '679',
      parent_code: '68',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3555b',
      name: 'Bảo Lâm',
      type: 'huyen',
      slug: 'bao-lam',
      name_with_type: 'Huyện Bảo Lâm',
      path: 'Bảo Lâm, Lâm Đồng',
      path_with_type:
        'Huyện Bảo Lâm, Tỉnh Lâm Đồng',
      code: '680',
      parent_code: '68',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3555c',
      name: 'Đạ Huoai',
      type: 'huyen',
      slug: 'da-huoai',
      name_with_type: 'Huyện Đạ Huoai',
      path: 'Đạ Huoai, Lâm Đồng',
      path_with_type:
        'Huyện Đạ Huoai, Tỉnh Lâm Đồng',
      code: '681',
      parent_code: '68',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3555d',
      name: 'Đạ Tẻh',
      type: 'huyen',
      slug: 'da-teh',
      name_with_type: 'Huyện Đạ Tẻh',
      path: 'Đạ Tẻh, Lâm Đồng',
      path_with_type:
        'Huyện Đạ Tẻh, Tỉnh Lâm Đồng',
      code: '682',
      parent_code: '68',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3555e',
      name: 'Cát Tiên',
      type: 'huyen',
      slug: 'cat-tien',
      name_with_type: 'Huyện Cát Tiên',
      path: 'Cát Tiên, Lâm Đồng',
      path_with_type:
        'Huyện Cát Tiên, Tỉnh Lâm Đồng',
      code: '683',
      parent_code: '68',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3555f',
      name: 'Phước Long',
      type: 'thi-xa',
      slug: 'phuoc-long',
      name_with_type: 'Thị xã Phước Long',
      path: 'Phước Long, Bình Phước',
      path_with_type:
        'Thị xã Phước Long, Tỉnh Bình Phước',
      code: '688',
      parent_code: '70',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35560',
      name: 'Đồng Xoài',
      type: 'thanh-pho',
      slug: 'dong-xoai',
      name_with_type: 'Thành phố Đồng Xoài',
      path: 'Đồng Xoài, Bình Phước',
      path_with_type:
        'Thành phố Đồng Xoài, Tỉnh Bình Phước',
      code: '689',
      parent_code: '70',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35561',
      name: 'Bình Long',
      type: 'thi-xa',
      slug: 'binh-long',
      name_with_type: 'Thị xã Bình Long',
      path: 'Bình Long, Bình Phước',
      path_with_type:
        'Thị xã Bình Long, Tỉnh Bình Phước',
      code: '690',
      parent_code: '70',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35562',
      name: 'Bù Gia Mập',
      type: 'huyen',
      slug: 'bu-gia-map',
      name_with_type: 'Huyện Bù Gia Mập',
      path: 'Bù Gia Mập, Bình Phước',
      path_with_type:
        'Huyện Bù Gia Mập, Tỉnh Bình Phước',
      code: '691',
      parent_code: '70',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35563',
      name: 'Lộc Ninh',
      type: 'huyen',
      slug: 'loc-ninh',
      name_with_type: 'Huyện Lộc Ninh',
      path: 'Lộc Ninh, Bình Phước',
      path_with_type:
        'Huyện Lộc Ninh, Tỉnh Bình Phước',
      code: '692',
      parent_code: '70',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35564',
      name: 'Bù Đốp',
      type: 'huyen',
      slug: 'bu-dop',
      name_with_type: 'Huyện Bù Đốp',
      path: 'Bù Đốp, Bình Phước',
      path_with_type:
        'Huyện Bù Đốp, Tỉnh Bình Phước',
      code: '693',
      parent_code: '70',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35565',
      name: 'Hớn Quản',
      type: 'huyen',
      slug: 'hon-quan',
      name_with_type: 'Huyện Hớn Quản',
      path: 'Hớn Quản, Bình Phước',
      path_with_type:
        'Huyện Hớn Quản, Tỉnh Bình Phước',
      code: '694',
      parent_code: '70',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35566',
      name: 'Đồng Phú',
      type: 'huyen',
      slug: 'dong-phu',
      name_with_type: 'Huyện Đồng Phú',
      path: 'Đồng Phú, Bình Phước',
      path_with_type:
        'Huyện Đồng Phú, Tỉnh Bình Phước',
      code: '695',
      parent_code: '70',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35567',
      name: 'Bù Đăng',
      type: 'huyen',
      slug: 'bu-dang',
      name_with_type: 'Huyện Bù Đăng',
      path: 'Bù Đăng, Bình Phước',
      path_with_type:
        'Huyện Bù Đăng, Tỉnh Bình Phước',
      code: '696',
      parent_code: '70',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35568',
      name: 'Chơn Thành',
      type: 'huyen',
      slug: 'chon-thanh',
      name_with_type: 'Huyện Chơn Thành',
      path: 'Chơn Thành, Bình Phước',
      path_with_type:
        'Huyện Chơn Thành, Tỉnh Bình Phước',
      code: '697',
      parent_code: '70',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35569',
      name: 'Phú Riềng',
      type: 'huyen',
      slug: 'phu-rieng',
      name_with_type: 'Huyện Phú Riềng',
      path: 'Phú Riềng, Bình Phước',
      path_with_type:
        'Huyện Phú Riềng, Tỉnh Bình Phước',
      code: '698',
      parent_code: '70',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3556a',
      name: 'Tây Ninh',
      type: 'thanh-pho',
      slug: 'tay-ninh',
      name_with_type: 'Thành phố Tây Ninh',
      path: 'Tây Ninh, Tây Ninh',
      path_with_type:
        'Thành phố Tây Ninh, Tỉnh Tây Ninh',
      code: '703',
      parent_code: '72',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3556b',
      name: 'Tân Biên',
      type: 'huyen',
      slug: 'tan-bien',
      name_with_type: 'Huyện Tân Biên',
      path: 'Tân Biên, Tây Ninh',
      path_with_type:
        'Huyện Tân Biên, Tỉnh Tây Ninh',
      code: '705',
      parent_code: '72',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3556c',
      name: 'Tân Châu',
      type: 'huyen',
      slug: 'tan-chau',
      name_with_type: 'Huyện Tân Châu',
      path: 'Tân Châu, Tây Ninh',
      path_with_type:
        'Huyện Tân Châu, Tỉnh Tây Ninh',
      code: '706',
      parent_code: '72',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3556d',
      name: 'Dương Minh Châu',
      type: 'huyen',
      slug: 'duong-minh-chau',
      name_with_type: 'Huyện Dương Minh Châu',
      path: 'Dương Minh Châu, Tây Ninh',
      path_with_type:
        'Huyện Dương Minh Châu, Tỉnh Tây Ninh',
      code: '707',
      parent_code: '72',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3556e',
      name: 'Châu Thành',
      type: 'huyen',
      slug: 'chau-thanh',
      name_with_type: 'Huyện Châu Thành',
      path: 'Châu Thành, Tây Ninh',
      path_with_type:
        'Huyện Châu Thành, Tỉnh Tây Ninh',
      code: '708',
      parent_code: '72',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3556f',
      name: 'Hòa Thành',
      type: 'thi-xa',
      slug: 'hoa-thanh',
      name_with_type: 'Thị xã Hòa Thành',
      path: 'Hòa Thành, Tây Ninh',
      path_with_type:
        'Thị xã Hòa Thành, Tỉnh Tây Ninh',
      code: '709',
      parent_code: '72',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35570',
      name: 'Gò Dầu',
      type: 'huyen',
      slug: 'go-dau',
      name_with_type: 'Huyện Gò Dầu',
      path: 'Gò Dầu, Tây Ninh',
      path_with_type:
        'Huyện Gò Dầu, Tỉnh Tây Ninh',
      code: '710',
      parent_code: '72',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35571',
      name: 'Bến Cầu',
      type: 'huyen',
      slug: 'ben-cau',
      name_with_type: 'Huyện Bến Cầu',
      path: 'Bến Cầu, Tây Ninh',
      path_with_type:
        'Huyện Bến Cầu, Tỉnh Tây Ninh',
      code: '711',
      parent_code: '72',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35572',
      name: 'Trảng Bàng',
      type: 'thi-xa',
      slug: 'trang-bang',
      name_with_type: 'Thị xã Trảng Bàng',
      path: 'Trảng Bàng, Tây Ninh',
      path_with_type:
        'Thị xã Trảng Bàng, Tỉnh Tây Ninh',
      code: '712',
      parent_code: '72',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35573',
      name: 'Thủ Dầu Một',
      type: 'thanh-pho',
      slug: 'thu-dau-mot',
      name_with_type: 'Thành phố Thủ Dầu Một',
      path: 'Thủ Dầu Một, Bình Dương',
      path_with_type:
        'Thành phố Thủ Dầu Một, Tỉnh Bình Dương',
      code: '718',
      parent_code: '74',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35574',
      name: 'Bàu Bàng',
      type: 'huyen',
      slug: 'bau-bang',
      name_with_type: 'Huyện Bàu Bàng',
      path: 'Bàu Bàng, Bình Dương',
      path_with_type:
        'Huyện Bàu Bàng, Tỉnh Bình Dương',
      code: '719',
      parent_code: '74',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35575',
      name: 'Dầu Tiếng',
      type: 'huyen',
      slug: 'dau-tieng',
      name_with_type: 'Huyện Dầu Tiếng',
      path: 'Dầu Tiếng, Bình Dương',
      path_with_type:
        'Huyện Dầu Tiếng, Tỉnh Bình Dương',
      code: '720',
      parent_code: '74',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35576',
      name: 'Bến Cát',
      type: 'thi-xa',
      slug: 'ben-cat',
      name_with_type: 'Thị xã Bến Cát',
      path: 'Bến Cát, Bình Dương',
      path_with_type:
        'Thị xã Bến Cát, Tỉnh Bình Dương',
      code: '721',
      parent_code: '74',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35577',
      name: 'Phú Giáo',
      type: 'huyen',
      slug: 'phu-giao',
      name_with_type: 'Huyện Phú Giáo',
      path: 'Phú Giáo, Bình Dương',
      path_with_type:
        'Huyện Phú Giáo, Tỉnh Bình Dương',
      code: '722',
      parent_code: '74',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35578',
      name: 'Tân Uyên',
      type: 'thi-xa',
      slug: 'tan-uyen',
      name_with_type: 'Thị xã Tân Uyên',
      path: 'Tân Uyên, Bình Dương',
      path_with_type:
        'Thị xã Tân Uyên, Tỉnh Bình Dương',
      code: '723',
      parent_code: '74',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35579',
      name: 'Dĩ An',
      type: 'thanh-pho',
      slug: 'di-an',
      name_with_type: 'Thành phố Dĩ An',
      path: 'Dĩ An, Bình Dương',
      path_with_type:
        'Thành phố Dĩ An, Tỉnh Bình Dương',
      code: '724',
      parent_code: '74',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3557a',
      name: 'Thuận An',
      type: 'thanh-pho',
      slug: 'thuan-an',
      name_with_type: 'Thành phố Thuận An',
      path: 'Thuận An, Bình Dương',
      path_with_type:
        'Thành phố Thuận An, Tỉnh Bình Dương',
      code: '725',
      parent_code: '74',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3557b',
      name: 'Bắc Tân Uyên',
      type: 'huyen',
      slug: 'bac-tan-uyen',
      name_with_type: 'Huyện Bắc Tân Uyên',
      path: 'Bắc Tân Uyên, Bình Dương',
      path_with_type:
        'Huyện Bắc Tân Uyên, Tỉnh Bình Dương',
      code: '726',
      parent_code: '74',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3557c',
      name: 'Biên Hòa',
      type: 'thanh-pho',
      slug: 'bien-hoa',
      name_with_type: 'Thành phố Biên Hòa',
      path: 'Biên Hòa, Đồng Nai',
      path_with_type:
        'Thành phố Biên Hòa, Tỉnh Đồng Nai',
      code: '731',
      parent_code: '75',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3557d',
      name: 'Long Khánh',
      type: 'thanh-pho',
      slug: 'long-khanh',
      name_with_type: 'Thành phố Long Khánh',
      path: 'Long Khánh, Đồng Nai',
      path_with_type:
        'Thành phố Long Khánh, Tỉnh Đồng Nai',
      code: '732',
      parent_code: '75',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3557e',
      name: 'Tân Phú',
      type: 'huyen',
      slug: 'tan-phu',
      name_with_type: 'Huyện Tân Phú',
      path: 'Tân Phú, Đồng Nai',
      path_with_type:
        'Huyện Tân Phú, Tỉnh Đồng Nai',
      code: '734',
      parent_code: '75',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3557f',
      name: 'Vĩnh Cửu',
      type: 'huyen',
      slug: 'vinh-cuu',
      name_with_type: 'Huyện Vĩnh Cửu',
      path: 'Vĩnh Cửu, Đồng Nai',
      path_with_type:
        'Huyện Vĩnh Cửu, Tỉnh Đồng Nai',
      code: '735',
      parent_code: '75',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35580',
      name: 'Định Quán',
      type: 'huyen',
      slug: 'dinh-quan',
      name_with_type: 'Huyện Định Quán',
      path: 'Định Quán, Đồng Nai',
      path_with_type:
        'Huyện Định Quán, Tỉnh Đồng Nai',
      code: '736',
      parent_code: '75',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35581',
      name: 'Trảng Bom',
      type: 'huyen',
      slug: 'trang-bom',
      name_with_type: 'Huyện Trảng Bom',
      path: 'Trảng Bom, Đồng Nai',
      path_with_type:
        'Huyện Trảng Bom, Tỉnh Đồng Nai',
      code: '737',
      parent_code: '75',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35582',
      name: 'Thống Nhất',
      type: 'huyen',
      slug: 'thong-nhat',
      name_with_type: 'Huyện Thống Nhất',
      path: 'Thống Nhất, Đồng Nai',
      path_with_type:
        'Huyện Thống Nhất, Tỉnh Đồng Nai',
      code: '738',
      parent_code: '75',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35583',
      name: 'Cẩm Mỹ',
      type: 'huyen',
      slug: 'cam-my',
      name_with_type: 'Huyện Cẩm Mỹ',
      path: 'Cẩm Mỹ, Đồng Nai',
      path_with_type:
        'Huyện Cẩm Mỹ, Tỉnh Đồng Nai',
      code: '739',
      parent_code: '75',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35584',
      name: 'Long Thành',
      type: 'huyen',
      slug: 'long-thanh',
      name_with_type: 'Huyện Long Thành',
      path: 'Long Thành, Đồng Nai',
      path_with_type:
        'Huyện Long Thành, Tỉnh Đồng Nai',
      code: '740',
      parent_code: '75',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35585',
      name: 'Xuân Lộc',
      type: 'huyen',
      slug: 'xuan-loc',
      name_with_type: 'Huyện Xuân Lộc',
      path: 'Xuân Lộc, Đồng Nai',
      path_with_type:
        'Huyện Xuân Lộc, Tỉnh Đồng Nai',
      code: '741',
      parent_code: '75',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35586',
      name: 'Nhơn Trạch',
      type: 'huyen',
      slug: 'nhon-trach',
      name_with_type: 'Huyện Nhơn Trạch',
      path: 'Nhơn Trạch, Đồng Nai',
      path_with_type:
        'Huyện Nhơn Trạch, Tỉnh Đồng Nai',
      code: '742',
      parent_code: '75',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35587',
      name: 'Vũng Tàu',
      type: 'thanh-pho',
      slug: 'vung-tau',
      name_with_type: 'Thành phố Vũng Tàu',
      path: 'Vũng Tàu, Bà Rịa - Vũng Tàu',
      path_with_type:
        'Thành phố Vũng Tàu, Tỉnh Bà Rịa - Vũng Tàu',
      code: '747',
      parent_code: '77',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35588',
      name: 'Bà Rịa',
      type: 'thanh-pho',
      slug: 'ba-ria',
      name_with_type: 'Thành phố Bà Rịa',
      path: 'Bà Rịa, Bà Rịa - Vũng Tàu',
      path_with_type:
        'Thành phố Bà Rịa, Tỉnh Bà Rịa - Vũng Tàu',
      code: '748',
      parent_code: '77',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35589',
      name: 'Châu Đức',
      type: 'huyen',
      slug: 'chau-duc',
      name_with_type: 'Huyện Châu Đức',
      path: 'Châu Đức, Bà Rịa - Vũng Tàu',
      path_with_type:
        'Huyện Châu Đức, Tỉnh Bà Rịa - Vũng Tàu',
      code: '750',
      parent_code: '77',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3558a',
      name: 'Xuyên Mộc',
      type: 'huyen',
      slug: 'xuyen-moc',
      name_with_type: 'Huyện Xuyên Mộc',
      path: 'Xuyên Mộc, Bà Rịa - Vũng Tàu',
      path_with_type:
        'Huyện Xuyên Mộc, Tỉnh Bà Rịa - Vũng Tàu',
      code: '751',
      parent_code: '77',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3558b',
      name: 'Long Điền',
      type: 'huyen',
      slug: 'long-dien',
      name_with_type: 'Huyện Long Điền',
      path: 'Long Điền, Bà Rịa - Vũng Tàu',
      path_with_type:
        'Huyện Long Điền, Tỉnh Bà Rịa - Vũng Tàu',
      code: '752',
      parent_code: '77',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3558c',
      name: 'Đất Đỏ',
      type: 'huyen',
      slug: 'dat-do',
      name_with_type: 'Huyện Đất Đỏ',
      path: 'Đất Đỏ, Bà Rịa - Vũng Tàu',
      path_with_type:
        'Huyện Đất Đỏ, Tỉnh Bà Rịa - Vũng Tàu',
      code: '753',
      parent_code: '77',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3558d',
      name: 'Phú Mỹ',
      type: 'thi-xa',
      slug: 'phu-my',
      name_with_type: 'Thị xã Phú Mỹ',
      path: 'Phú Mỹ, Bà Rịa - Vũng Tàu',
      path_with_type:
        'Thị xã Phú Mỹ, Tỉnh Bà Rịa - Vũng Tàu',
      code: '754',
      parent_code: '77',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3558e',
      name: 'Côn Đảo',
      type: 'huyen',
      slug: 'con-dao',
      name_with_type: 'Huyện Côn Đảo',
      path: 'Côn Đảo, Bà Rịa - Vũng Tàu',
      path_with_type:
        'Huyện Côn Đảo, Tỉnh Bà Rịa - Vũng Tàu',
      code: '755',
      parent_code: '77',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3558f',
      name: '1',
      type: 'quan',
      slug: '1',
      name_with_type: 'Quận 1',
      path: '1, Hồ Chí Minh',
      path_with_type:
        'Quận 1, Thành phố Hồ Chí Minh',
      code: '760',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35590',
      name: '12',
      type: 'quan',
      slug: '12',
      name_with_type: 'Quận 12',
      path: '12, Hồ Chí Minh',
      path_with_type:
        'Quận 12, Thành phố Hồ Chí Minh',
      code: '761',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35591',
      name: 'Gò Vấp',
      type: 'quan',
      slug: 'go-vap',
      name_with_type: 'Quận Gò Vấp',
      path: 'Gò Vấp, Hồ Chí Minh',
      path_with_type:
        'Quận Gò Vấp, Thành phố Hồ Chí Minh',
      code: '764',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35592',
      name: 'Bình Thạnh',
      type: 'quan',
      slug: 'binh-thanh',
      name_with_type: 'Quận Bình Thạnh',
      path: 'Bình Thạnh, Hồ Chí Minh',
      path_with_type:
        'Quận Bình Thạnh, Thành phố Hồ Chí Minh',
      code: '765',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35593',
      name: 'Tân Bình',
      type: 'quan',
      slug: 'tan-binh',
      name_with_type: 'Quận Tân Bình',
      path: 'Tân Bình, Hồ Chí Minh',
      path_with_type:
        'Quận Tân Bình, Thành phố Hồ Chí Minh',
      code: '766',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35594',
      name: 'Tân Phú',
      type: 'quan',
      slug: 'tan-phu',
      name_with_type: 'Quận Tân Phú',
      path: 'Tân Phú, Hồ Chí Minh',
      path_with_type:
        'Quận Tân Phú, Thành phố Hồ Chí Minh',
      code: '767',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35595',
      name: 'Phú Nhuận',
      type: 'quan',
      slug: 'phu-nhuan',
      name_with_type: 'Quận Phú Nhuận',
      path: 'Phú Nhuận, Hồ Chí Minh',
      path_with_type:
        'Quận Phú Nhuận, Thành phố Hồ Chí Minh',
      code: '768',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35596',
      name: 'Thủ Đức',
      type: 'thanh-pho',
      slug: 'thu-duc',
      name_with_type: 'Thành phố Thủ Đức',
      path: 'Thủ Đức, Hồ Chí Minh',
      path_with_type:
        'Thành phố Thủ Đức, Thành phố Hồ Chí Minh',
      code: '769',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35597',
      name: '3',
      type: 'quan',
      slug: '3',
      name_with_type: 'Quận 3',
      path: '3, Hồ Chí Minh',
      path_with_type:
        'Quận 3, Thành phố Hồ Chí Minh',
      code: '770',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35598',
      name: '10',
      type: 'quan',
      slug: '10',
      name_with_type: 'Quận 10',
      path: '10, Hồ Chí Minh',
      path_with_type:
        'Quận 10, Thành phố Hồ Chí Minh',
      code: '771',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35599',
      name: '11',
      type: 'quan',
      slug: '11',
      name_with_type: 'Quận 11',
      path: '11, Hồ Chí Minh',
      path_with_type:
        'Quận 11, Thành phố Hồ Chí Minh',
      code: '772',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3559a',
      name: '4',
      type: 'quan',
      slug: '4',
      name_with_type: 'Quận 4',
      path: '4, Hồ Chí Minh',
      path_with_type:
        'Quận 4, Thành phố Hồ Chí Minh',
      code: '773',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3559b',
      name: '5',
      type: 'quan',
      slug: '5',
      name_with_type: 'Quận 5',
      path: '5, Hồ Chí Minh',
      path_with_type:
        'Quận 5, Thành phố Hồ Chí Minh',
      code: '774',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3559c',
      name: '6',
      type: 'quan',
      slug: '6',
      name_with_type: 'Quận 6',
      path: '6, Hồ Chí Minh',
      path_with_type:
        'Quận 6, Thành phố Hồ Chí Minh',
      code: '775',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3559d',
      name: '8',
      type: 'quan',
      slug: '8',
      name_with_type: 'Quận 8',
      path: '8, Hồ Chí Minh',
      path_with_type:
        'Quận 8, Thành phố Hồ Chí Minh',
      code: '776',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3559e',
      name: 'Bình Tân',
      type: 'quan',
      slug: 'binh-tan',
      name_with_type: 'Quận Bình Tân',
      path: 'Bình Tân, Hồ Chí Minh',
      path_with_type:
        'Quận Bình Tân, Thành phố Hồ Chí Minh',
      code: '777',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3559f',
      name: '7',
      type: 'quan',
      slug: '7',
      name_with_type: 'Quận 7',
      path: '7, Hồ Chí Minh',
      path_with_type:
        'Quận 7, Thành phố Hồ Chí Minh',
      code: '778',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355a0',
      name: 'Củ Chi',
      type: 'huyen',
      slug: 'cu-chi',
      name_with_type: 'Huyện Củ Chi',
      path: 'Củ Chi, Hồ Chí Minh',
      path_with_type:
        'Huyện Củ Chi, Thành phố Hồ Chí Minh',
      code: '783',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355a1',
      name: 'Hóc Môn',
      type: 'huyen',
      slug: 'hoc-mon',
      name_with_type: 'Huyện Hóc Môn',
      path: 'Hóc Môn, Hồ Chí Minh',
      path_with_type:
        'Huyện Hóc Môn, Thành phố Hồ Chí Minh',
      code: '784',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355a2',
      name: 'Bình Chánh',
      type: 'huyen',
      slug: 'binh-chanh',
      name_with_type: 'Huyện Bình Chánh',
      path: 'Bình Chánh, Hồ Chí Minh',
      path_with_type:
        'Huyện Bình Chánh, Thành phố Hồ Chí Minh',
      code: '785',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355a3',
      name: 'Nhà Bè',
      type: 'huyen',
      slug: 'nha-be',
      name_with_type: 'Huyện Nhà Bè',
      path: 'Nhà Bè, Hồ Chí Minh',
      path_with_type:
        'Huyện Nhà Bè, Thành phố Hồ Chí Minh',
      code: '786',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355a4',
      name: 'Cần Giờ',
      type: 'huyen',
      slug: 'can-gio',
      name_with_type: 'Huyện Cần Giờ',
      path: 'Cần Giờ, Hồ Chí Minh',
      path_with_type:
        'Huyện Cần Giờ, Thành phố Hồ Chí Minh',
      code: '787',
      parent_code: '79',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355a5',
      name: 'Tân An',
      type: 'thanh-pho',
      slug: 'tan-an',
      name_with_type: 'Thành phố Tân An',
      path: 'Tân An, Long An',
      path_with_type:
        'Thành phố Tân An, Tỉnh Long An',
      code: '794',
      parent_code: '80',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355a6',
      name: 'Kiến Tường',
      type: 'thi-xa',
      slug: 'kien-tuong',
      name_with_type: 'Thị xã Kiến Tường',
      path: 'Kiến Tường, Long An',
      path_with_type:
        'Thị xã Kiến Tường, Tỉnh Long An',
      code: '795',
      parent_code: '80',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355a7',
      name: 'Tân Hưng',
      type: 'huyen',
      slug: 'tan-hung',
      name_with_type: 'Huyện Tân Hưng',
      path: 'Tân Hưng, Long An',
      path_with_type:
        'Huyện Tân Hưng, Tỉnh Long An',
      code: '796',
      parent_code: '80',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355a8',
      name: 'Vĩnh Hưng',
      type: 'huyen',
      slug: 'vinh-hung',
      name_with_type: 'Huyện Vĩnh Hưng',
      path: 'Vĩnh Hưng, Long An',
      path_with_type:
        'Huyện Vĩnh Hưng, Tỉnh Long An',
      code: '797',
      parent_code: '80',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355a9',
      name: 'Mộc Hóa',
      type: 'huyen',
      slug: 'moc-hoa',
      name_with_type: 'Huyện Mộc Hóa',
      path: 'Mộc Hóa, Long An',
      path_with_type:
        'Huyện Mộc Hóa, Tỉnh Long An',
      code: '798',
      parent_code: '80',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355aa',
      name: 'Tân Thạnh',
      type: 'huyen',
      slug: 'tan-thanh',
      name_with_type: 'Huyện Tân Thạnh',
      path: 'Tân Thạnh, Long An',
      path_with_type:
        'Huyện Tân Thạnh, Tỉnh Long An',
      code: '799',
      parent_code: '80',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355ab',
      name: 'Thạnh Hóa',
      type: 'huyen',
      slug: 'thanh-hoa',
      name_with_type: 'Huyện Thạnh Hóa',
      path: 'Thạnh Hóa, Long An',
      path_with_type:
        'Huyện Thạnh Hóa, Tỉnh Long An',
      code: '800',
      parent_code: '80',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355ac',
      name: 'Đức Huệ',
      type: 'huyen',
      slug: 'duc-hue',
      name_with_type: 'Huyện Đức Huệ',
      path: 'Đức Huệ, Long An',
      path_with_type:
        'Huyện Đức Huệ, Tỉnh Long An',
      code: '801',
      parent_code: '80',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355ad',
      name: 'Đức Hòa',
      type: 'huyen',
      slug: 'duc-hoa',
      name_with_type: 'Huyện Đức Hòa',
      path: 'Đức Hòa, Long An',
      path_with_type:
        'Huyện Đức Hòa, Tỉnh Long An',
      code: '802',
      parent_code: '80',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355ae',
      name: 'Bến Lức',
      type: 'huyen',
      slug: 'ben-luc',
      name_with_type: 'Huyện Bến Lức',
      path: 'Bến Lức, Long An',
      path_with_type:
        'Huyện Bến Lức, Tỉnh Long An',
      code: '803',
      parent_code: '80',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355af',
      name: 'Thủ Thừa',
      type: 'huyen',
      slug: 'thu-thua',
      name_with_type: 'Huyện Thủ Thừa',
      path: 'Thủ Thừa, Long An',
      path_with_type:
        'Huyện Thủ Thừa, Tỉnh Long An',
      code: '804',
      parent_code: '80',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355b0',
      name: 'Tân Trụ',
      type: 'huyen',
      slug: 'tan-tru',
      name_with_type: 'Huyện Tân Trụ',
      path: 'Tân Trụ, Long An',
      path_with_type:
        'Huyện Tân Trụ, Tỉnh Long An',
      code: '805',
      parent_code: '80',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355b1',
      name: 'Cần Đước',
      type: 'huyen',
      slug: 'can-duoc',
      name_with_type: 'Huyện Cần Đước',
      path: 'Cần Đước, Long An',
      path_with_type:
        'Huyện Cần Đước, Tỉnh Long An',
      code: '806',
      parent_code: '80',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355b2',
      name: 'Cần Giuộc',
      type: 'huyen',
      slug: 'can-giuoc',
      name_with_type: 'Huyện Cần Giuộc',
      path: 'Cần Giuộc, Long An',
      path_with_type:
        'Huyện Cần Giuộc, Tỉnh Long An',
      code: '807',
      parent_code: '80',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355b3',
      name: 'Châu Thành',
      type: 'huyen',
      slug: 'chau-thanh',
      name_with_type: 'Huyện Châu Thành',
      path: 'Châu Thành, Long An',
      path_with_type:
        'Huyện Châu Thành, Tỉnh Long An',
      code: '808',
      parent_code: '80',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355b4',
      name: 'Mỹ Tho',
      type: 'thanh-pho',
      slug: 'my-tho',
      name_with_type: 'Thành phố Mỹ Tho',
      path: 'Mỹ Tho, Tiền Giang',
      path_with_type:
        'Thành phố Mỹ Tho, Tỉnh Tiền Giang',
      code: '815',
      parent_code: '82',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355b5',
      name: 'Gò Công',
      type: 'thi-xa',
      slug: 'go-cong',
      name_with_type: 'Thị xã Gò Công',
      path: 'Gò Công, Tiền Giang',
      path_with_type:
        'Thị xã Gò Công, Tỉnh Tiền Giang',
      code: '816',
      parent_code: '82',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355b6',
      name: 'Cai Lậy',
      type: 'thi-xa',
      slug: 'cai-lay',
      name_with_type: 'Thị xã Cai Lậy',
      path: 'Cai Lậy, Tiền Giang',
      path_with_type:
        'Thị xã Cai Lậy, Tỉnh Tiền Giang',
      code: '817',
      parent_code: '82',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355b7',
      name: 'Tân Phước',
      type: 'huyen',
      slug: 'tan-phuoc',
      name_with_type: 'Huyện Tân Phước',
      path: 'Tân Phước, Tiền Giang',
      path_with_type:
        'Huyện Tân Phước, Tỉnh Tiền Giang',
      code: '818',
      parent_code: '82',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355b8',
      name: 'Cái Bè',
      type: 'huyen',
      slug: 'cai-be',
      name_with_type: 'Huyện Cái Bè',
      path: 'Cái Bè, Tiền Giang',
      path_with_type:
        'Huyện Cái Bè, Tỉnh Tiền Giang',
      code: '819',
      parent_code: '82',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355b9',
      name: 'Cai Lậy',
      type: 'huyen',
      slug: 'cai-lay',
      name_with_type: 'Huyện Cai Lậy',
      path: 'Cai Lậy, Tiền Giang',
      path_with_type:
        'Huyện Cai Lậy, Tỉnh Tiền Giang',
      code: '820',
      parent_code: '82',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355ba',
      name: 'Châu Thành',
      type: 'huyen',
      slug: 'chau-thanh',
      name_with_type: 'Huyện Châu Thành',
      path: 'Châu Thành, Tiền Giang',
      path_with_type:
        'Huyện Châu Thành, Tỉnh Tiền Giang',
      code: '821',
      parent_code: '82',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355bb',
      name: 'Chợ Gạo',
      type: 'huyen',
      slug: 'cho-gao',
      name_with_type: 'Huyện Chợ Gạo',
      path: 'Chợ Gạo, Tiền Giang',
      path_with_type:
        'Huyện Chợ Gạo, Tỉnh Tiền Giang',
      code: '822',
      parent_code: '82',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355bc',
      name: 'Gò Công Tây',
      type: 'huyen',
      slug: 'go-cong-tay',
      name_with_type: 'Huyện Gò Công Tây',
      path: 'Gò Công Tây, Tiền Giang',
      path_with_type:
        'Huyện Gò Công Tây, Tỉnh Tiền Giang',
      code: '823',
      parent_code: '82',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355bd',
      name: 'Gò Công Đông',
      type: 'huyen',
      slug: 'go-cong-dong',
      name_with_type: 'Huyện Gò Công Đông',
      path: 'Gò Công Đông, Tiền Giang',
      path_with_type:
        'Huyện Gò Công Đông, Tỉnh Tiền Giang',
      code: '824',
      parent_code: '82',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355be',
      name: 'Tân Phú Đông',
      type: 'huyen',
      slug: 'tan-phu-dong',
      name_with_type: 'Huyện Tân Phú Đông',
      path: 'Tân Phú Đông, Tiền Giang',
      path_with_type:
        'Huyện Tân Phú Đông, Tỉnh Tiền Giang',
      code: '825',
      parent_code: '82',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355bf',
      name: 'Bến Tre',
      type: 'thanh-pho',
      slug: 'ben-tre',
      name_with_type: 'Thành phố Bến Tre',
      path: 'Bến Tre, Bến Tre',
      path_with_type:
        'Thành phố Bến Tre, Tỉnh Bến Tre',
      code: '829',
      parent_code: '83',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355c0',
      name: 'Châu Thành',
      type: 'huyen',
      slug: 'chau-thanh',
      name_with_type: 'Huyện Châu Thành',
      path: 'Châu Thành, Bến Tre',
      path_with_type:
        'Huyện Châu Thành, Tỉnh Bến Tre',
      code: '831',
      parent_code: '83',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355c1',
      name: 'Chợ Lách',
      type: 'huyen',
      slug: 'cho-lach',
      name_with_type: 'Huyện Chợ Lách',
      path: 'Chợ Lách, Bến Tre',
      path_with_type:
        'Huyện Chợ Lách, Tỉnh Bến Tre',
      code: '832',
      parent_code: '83',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355c2',
      name: 'Mỏ Cày Nam',
      type: 'huyen',
      slug: 'mo-cay-nam',
      name_with_type: 'Huyện Mỏ Cày Nam',
      path: 'Mỏ Cày Nam, Bến Tre',
      path_with_type:
        'Huyện Mỏ Cày Nam, Tỉnh Bến Tre',
      code: '833',
      parent_code: '83',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355c3',
      name: 'Giồng Trôm',
      type: 'huyen',
      slug: 'giong-trom',
      name_with_type: 'Huyện Giồng Trôm',
      path: 'Giồng Trôm, Bến Tre',
      path_with_type:
        'Huyện Giồng Trôm, Tỉnh Bến Tre',
      code: '834',
      parent_code: '83',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355c4',
      name: 'Bình Đại',
      type: 'huyen',
      slug: 'binh-dai',
      name_with_type: 'Huyện Bình Đại',
      path: 'Bình Đại, Bến Tre',
      path_with_type:
        'Huyện Bình Đại, Tỉnh Bến Tre',
      code: '835',
      parent_code: '83',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355c5',
      name: 'Ba Tri',
      type: 'huyen',
      slug: 'ba-tri',
      name_with_type: 'Huyện Ba Tri',
      path: 'Ba Tri, Bến Tre',
      path_with_type:
        'Huyện Ba Tri, Tỉnh Bến Tre',
      code: '836',
      parent_code: '83',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355c6',
      name: 'Thạnh Phú',
      type: 'huyen',
      slug: 'thanh-phu',
      name_with_type: 'Huyện Thạnh Phú',
      path: 'Thạnh Phú, Bến Tre',
      path_with_type:
        'Huyện Thạnh Phú, Tỉnh Bến Tre',
      code: '837',
      parent_code: '83',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355c7',
      name: 'Mỏ Cày Bắc',
      type: 'huyen',
      slug: 'mo-cay-bac',
      name_with_type: 'Huyện Mỏ Cày Bắc',
      path: 'Mỏ Cày Bắc, Bến Tre',
      path_with_type:
        'Huyện Mỏ Cày Bắc, Tỉnh Bến Tre',
      code: '838',
      parent_code: '83',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355c8',
      name: 'Trà Vinh',
      type: 'thanh-pho',
      slug: 'tra-vinh',
      name_with_type: 'Thành phố Trà Vinh',
      path: 'Trà Vinh, Trà Vinh',
      path_with_type:
        'Thành phố Trà Vinh, Tỉnh Trà Vinh',
      code: '842',
      parent_code: '84',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355c9',
      name: 'Càng Long',
      type: 'huyen',
      slug: 'cang-long',
      name_with_type: 'Huyện Càng Long',
      path: 'Càng Long, Trà Vinh',
      path_with_type:
        'Huyện Càng Long, Tỉnh Trà Vinh',
      code: '844',
      parent_code: '84',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355ca',
      name: 'Cầu Kè',
      type: 'huyen',
      slug: 'cau-ke',
      name_with_type: 'Huyện Cầu Kè',
      path: 'Cầu Kè, Trà Vinh',
      path_with_type:
        'Huyện Cầu Kè, Tỉnh Trà Vinh',
      code: '845',
      parent_code: '84',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355cb',
      name: 'Tiểu Cần',
      type: 'huyen',
      slug: 'tieu-can',
      name_with_type: 'Huyện Tiểu Cần',
      path: 'Tiểu Cần, Trà Vinh',
      path_with_type:
        'Huyện Tiểu Cần, Tỉnh Trà Vinh',
      code: '846',
      parent_code: '84',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355cc',
      name: 'Châu Thành',
      type: 'huyen',
      slug: 'chau-thanh',
      name_with_type: 'Huyện Châu Thành',
      path: 'Châu Thành, Trà Vinh',
      path_with_type:
        'Huyện Châu Thành, Tỉnh Trà Vinh',
      code: '847',
      parent_code: '84',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355cd',
      name: 'Cầu Ngang',
      type: 'huyen',
      slug: 'cau-ngang',
      name_with_type: 'Huyện Cầu Ngang',
      path: 'Cầu Ngang, Trà Vinh',
      path_with_type:
        'Huyện Cầu Ngang, Tỉnh Trà Vinh',
      code: '848',
      parent_code: '84',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355ce',
      name: 'Trà Cú',
      type: 'huyen',
      slug: 'tra-cu',
      name_with_type: 'Huyện Trà Cú',
      path: 'Trà Cú, Trà Vinh',
      path_with_type:
        'Huyện Trà Cú, Tỉnh Trà Vinh',
      code: '849',
      parent_code: '84',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355cf',
      name: 'Duyên Hải',
      type: 'huyen',
      slug: 'duyen-hai',
      name_with_type: 'Huyện Duyên Hải',
      path: 'Duyên Hải, Trà Vinh',
      path_with_type:
        'Huyện Duyên Hải, Tỉnh Trà Vinh',
      code: '850',
      parent_code: '84',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355d0',
      name: 'Duyên Hải',
      type: 'thi-xa',
      slug: 'duyen-hai',
      name_with_type: 'Thị xã Duyên Hải',
      path: 'Duyên Hải, Trà Vinh',
      path_with_type:
        'Thị xã Duyên Hải, Tỉnh Trà Vinh',
      code: '851',
      parent_code: '84',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355d1',
      name: 'Vĩnh Long',
      type: 'thanh-pho',
      slug: 'vinh-long',
      name_with_type: 'Thành phố Vĩnh Long',
      path: 'Vĩnh Long, Vĩnh Long',
      path_with_type:
        'Thành phố Vĩnh Long, Tỉnh Vĩnh Long',
      code: '855',
      parent_code: '86',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355d2',
      name: 'Long Hồ',
      type: 'huyen',
      slug: 'long-ho',
      name_with_type: 'Huyện Long Hồ',
      path: 'Long Hồ, Vĩnh Long',
      path_with_type:
        'Huyện Long Hồ, Tỉnh Vĩnh Long',
      code: '857',
      parent_code: '86',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355d3',
      name: 'Mang Thít',
      type: 'huyen',
      slug: 'mang-thit',
      name_with_type: 'Huyện Mang Thít',
      path: 'Mang Thít, Vĩnh Long',
      path_with_type:
        'Huyện Mang Thít, Tỉnh Vĩnh Long',
      code: '858',
      parent_code: '86',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355d4',
      name: ' Vũng Liêm',
      type: 'huyen',
      slug: 'vung-liem',
      name_with_type: 'Huyện  Vũng Liêm',
      path: ' Vũng Liêm, Vĩnh Long',
      path_with_type:
        'Huyện  Vũng Liêm, Tỉnh Vĩnh Long',
      code: '859',
      parent_code: '86',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355d5',
      name: 'Tam Bình',
      type: 'huyen',
      slug: 'tam-binh',
      name_with_type: 'Huyện Tam Bình',
      path: 'Tam Bình, Vĩnh Long',
      path_with_type:
        'Huyện Tam Bình, Tỉnh Vĩnh Long',
      code: '860',
      parent_code: '86',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355d6',
      name: 'Bình Minh',
      type: 'thi-xa',
      slug: 'binh-minh',
      name_with_type: 'Thị xã Bình Minh',
      path: 'Bình Minh, Vĩnh Long',
      path_with_type:
        'Thị xã Bình Minh, Tỉnh Vĩnh Long',
      code: '861',
      parent_code: '86',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355d7',
      name: 'Trà Ôn',
      type: 'huyen',
      slug: 'tra-on',
      name_with_type: 'Huyện Trà Ôn',
      path: 'Trà Ôn, Vĩnh Long',
      path_with_type:
        'Huyện Trà Ôn, Tỉnh Vĩnh Long',
      code: '862',
      parent_code: '86',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355d8',
      name: 'Bình Tân',
      type: 'huyen',
      slug: 'binh-tan',
      name_with_type: 'Huyện Bình Tân',
      path: 'Bình Tân, Vĩnh Long',
      path_with_type:
        'Huyện Bình Tân, Tỉnh Vĩnh Long',
      code: '863',
      parent_code: '86',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355d9',
      name: 'Cao Lãnh',
      type: 'thanh-pho',
      slug: 'cao-lanh',
      name_with_type: 'Thành phố Cao Lãnh',
      path: 'Cao Lãnh, Đồng Tháp',
      path_with_type:
        'Thành phố Cao Lãnh, Tỉnh Đồng Tháp',
      code: '866',
      parent_code: '87',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355da',
      name: 'Sa Đéc',
      type: 'thanh-pho',
      slug: 'sa-dec',
      name_with_type: 'Thành phố Sa Đéc',
      path: 'Sa Đéc, Đồng Tháp',
      path_with_type:
        'Thành phố Sa Đéc, Tỉnh Đồng Tháp',
      code: '867',
      parent_code: '87',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355db',
      name: 'Hồng Ngự',
      type: 'thanh-pho',
      slug: 'hong-ngu',
      name_with_type: 'Thành phố Hồng Ngự',
      path: 'Hồng Ngự, Đồng Tháp',
      path_with_type:
        'Thành phố Hồng Ngự, Tỉnh Đồng Tháp',
      code: '868',
      parent_code: '87',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355dc',
      name: 'Tân Hồng',
      type: 'huyen',
      slug: 'tan-hong',
      name_with_type: 'Huyện Tân Hồng',
      path: 'Tân Hồng, Đồng Tháp',
      path_with_type:
        'Huyện Tân Hồng, Tỉnh Đồng Tháp',
      code: '869',
      parent_code: '87',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355dd',
      name: 'Hồng Ngự',
      type: 'huyen',
      slug: 'hong-ngu',
      name_with_type: 'Huyện Hồng Ngự',
      path: 'Hồng Ngự, Đồng Tháp',
      path_with_type:
        'Huyện Hồng Ngự, Tỉnh Đồng Tháp',
      code: '870',
      parent_code: '87',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355de',
      name: 'Tam Nông',
      type: 'huyen',
      slug: 'tam-nong',
      name_with_type: 'Huyện Tam Nông',
      path: 'Tam Nông, Đồng Tháp',
      path_with_type:
        'Huyện Tam Nông, Tỉnh Đồng Tháp',
      code: '871',
      parent_code: '87',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355df',
      name: 'Tháp Mười',
      type: 'huyen',
      slug: 'thap-muoi',
      name_with_type: 'Huyện Tháp Mười',
      path: 'Tháp Mười, Đồng Tháp',
      path_with_type:
        'Huyện Tháp Mười, Tỉnh Đồng Tháp',
      code: '872',
      parent_code: '87',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355e0',
      name: 'Cao Lãnh',
      type: 'huyen',
      slug: 'cao-lanh',
      name_with_type: 'Huyện Cao Lãnh',
      path: 'Cao Lãnh, Đồng Tháp',
      path_with_type:
        'Huyện Cao Lãnh, Tỉnh Đồng Tháp',
      code: '873',
      parent_code: '87',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355e1',
      name: 'Thanh Bình',
      type: 'huyen',
      slug: 'thanh-binh',
      name_with_type: 'Huyện Thanh Bình',
      path: 'Thanh Bình, Đồng Tháp',
      path_with_type:
        'Huyện Thanh Bình, Tỉnh Đồng Tháp',
      code: '874',
      parent_code: '87',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355e2',
      name: 'Lấp Vò',
      type: 'huyen',
      slug: 'lap-vo',
      name_with_type: 'Huyện Lấp Vò',
      path: 'Lấp Vò, Đồng Tháp',
      path_with_type:
        'Huyện Lấp Vò, Tỉnh Đồng Tháp',
      code: '875',
      parent_code: '87',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355e3',
      name: 'Lai Vung',
      type: 'huyen',
      slug: 'lai-vung',
      name_with_type: 'Huyện Lai Vung',
      path: 'Lai Vung, Đồng Tháp',
      path_with_type:
        'Huyện Lai Vung, Tỉnh Đồng Tháp',
      code: '876',
      parent_code: '87',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355e4',
      name: 'Châu Thành',
      type: 'huyen',
      slug: 'chau-thanh',
      name_with_type: 'Huyện Châu Thành',
      path: 'Châu Thành, Đồng Tháp',
      path_with_type:
        'Huyện Châu Thành, Tỉnh Đồng Tháp',
      code: '877',
      parent_code: '87',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355e5',
      name: 'Long Xuyên',
      type: 'thanh-pho',
      slug: 'long-xuyen',
      name_with_type: 'Thành phố Long Xuyên',
      path: 'Long Xuyên, An Giang',
      path_with_type:
        'Thành phố Long Xuyên, Tỉnh An Giang',
      code: '883',
      parent_code: '89',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355e6',
      name: 'Châu Đốc',
      type: 'thanh-pho',
      slug: 'chau-doc',
      name_with_type: 'Thành phố Châu Đốc',
      path: 'Châu Đốc, An Giang',
      path_with_type:
        'Thành phố Châu Đốc, Tỉnh An Giang',
      code: '884',
      parent_code: '89',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355e7',
      name: 'An Phú',
      type: 'huyen',
      slug: 'an-phu',
      name_with_type: 'Huyện An Phú',
      path: 'An Phú, An Giang',
      path_with_type:
        'Huyện An Phú, Tỉnh An Giang',
      code: '886',
      parent_code: '89',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355e8',
      name: 'Tân Châu',
      type: 'thi-xa',
      slug: 'tan-chau',
      name_with_type: 'Thị xã Tân Châu',
      path: 'Tân Châu, An Giang',
      path_with_type:
        'Thị xã Tân Châu, Tỉnh An Giang',
      code: '887',
      parent_code: '89',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355e9',
      name: 'Phú Tân',
      type: 'huyen',
      slug: 'phu-tan',
      name_with_type: 'Huyện Phú Tân',
      path: 'Phú Tân, An Giang',
      path_with_type:
        'Huyện Phú Tân, Tỉnh An Giang',
      code: '888',
      parent_code: '89',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355ea',
      name: 'Châu Phú',
      type: 'huyen',
      slug: 'chau-phu',
      name_with_type: 'Huyện Châu Phú',
      path: 'Châu Phú, An Giang',
      path_with_type:
        'Huyện Châu Phú, Tỉnh An Giang',
      code: '889',
      parent_code: '89',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355eb',
      name: 'Tịnh Biên',
      type: 'huyen',
      slug: 'tinh-bien',
      name_with_type: 'Huyện Tịnh Biên',
      path: 'Tịnh Biên, An Giang',
      path_with_type:
        'Huyện Tịnh Biên, Tỉnh An Giang',
      code: '890',
      parent_code: '89',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355ec',
      name: 'Tri Tôn',
      type: 'huyen',
      slug: 'tri-ton',
      name_with_type: 'Huyện Tri Tôn',
      path: 'Tri Tôn, An Giang',
      path_with_type:
        'Huyện Tri Tôn, Tỉnh An Giang',
      code: '891',
      parent_code: '89',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355ed',
      name: 'Châu Thành',
      type: 'huyen',
      slug: 'chau-thanh',
      name_with_type: 'Huyện Châu Thành',
      path: 'Châu Thành, An Giang',
      path_with_type:
        'Huyện Châu Thành, Tỉnh An Giang',
      code: '892',
      parent_code: '89',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355ee',
      name: 'Chợ Mới',
      type: 'huyen',
      slug: 'cho-moi',
      name_with_type: 'Huyện Chợ Mới',
      path: 'Chợ Mới, An Giang',
      path_with_type:
        'Huyện Chợ Mới, Tỉnh An Giang',
      code: '893',
      parent_code: '89',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355ef',
      name: 'Thoại Sơn',
      type: 'huyen',
      slug: 'thoai-son',
      name_with_type: 'Huyện Thoại Sơn',
      path: 'Thoại Sơn, An Giang',
      path_with_type:
        'Huyện Thoại Sơn, Tỉnh An Giang',
      code: '894',
      parent_code: '89',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355f0',
      name: 'Rạch Giá',
      type: 'thanh-pho',
      slug: 'rach-gia',
      name_with_type: 'Thành phố Rạch Giá',
      path: 'Rạch Giá, Kiên Giang',
      path_with_type:
        'Thành phố Rạch Giá, Tỉnh Kiên Giang',
      code: '899',
      parent_code: '91',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355f1',
      name: 'Hà Tiên',
      type: 'thanh-pho',
      slug: 'ha-tien',
      name_with_type: 'Thành phố Hà Tiên',
      path: 'Hà Tiên, Kiên Giang',
      path_with_type:
        'Thành phố Hà Tiên, Tỉnh Kiên Giang',
      code: '900',
      parent_code: '91',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355f2',
      name: 'Kiên Lương',
      type: 'huyen',
      slug: 'kien-luong',
      name_with_type: 'Huyện Kiên Lương',
      path: 'Kiên Lương, Kiên Giang',
      path_with_type:
        'Huyện Kiên Lương, Tỉnh Kiên Giang',
      code: '902',
      parent_code: '91',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355f3',
      name: 'Hòn Đất',
      type: 'huyen',
      slug: 'hon-dat',
      name_with_type: 'Huyện Hòn Đất',
      path: 'Hòn Đất, Kiên Giang',
      path_with_type:
        'Huyện Hòn Đất, Tỉnh Kiên Giang',
      code: '903',
      parent_code: '91',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355f4',
      name: 'Tân Hiệp',
      type: 'huyen',
      slug: 'tan-hiep',
      name_with_type: 'Huyện Tân Hiệp',
      path: 'Tân Hiệp, Kiên Giang',
      path_with_type:
        'Huyện Tân Hiệp, Tỉnh Kiên Giang',
      code: '904',
      parent_code: '91',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355f5',
      name: 'Châu Thành',
      type: 'huyen',
      slug: 'chau-thanh',
      name_with_type: 'Huyện Châu Thành',
      path: 'Châu Thành, Kiên Giang',
      path_with_type:
        'Huyện Châu Thành, Tỉnh Kiên Giang',
      code: '905',
      parent_code: '91',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355f6',
      name: 'Giồng Riềng',
      type: 'huyen',
      slug: 'giong-rieng',
      name_with_type: 'Huyện Giồng Riềng',
      path: 'Giồng Riềng, Kiên Giang',
      path_with_type:
        'Huyện Giồng Riềng, Tỉnh Kiên Giang',
      code: '906',
      parent_code: '91',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355f7',
      name: 'Gò Quao',
      type: 'huyen',
      slug: 'go-quao',
      name_with_type: 'Huyện Gò Quao',
      path: 'Gò Quao, Kiên Giang',
      path_with_type:
        'Huyện Gò Quao, Tỉnh Kiên Giang',
      code: '907',
      parent_code: '91',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355f8',
      name: 'An Biên',
      type: 'huyen',
      slug: 'an-bien',
      name_with_type: 'Huyện An Biên',
      path: 'An Biên, Kiên Giang',
      path_with_type:
        'Huyện An Biên, Tỉnh Kiên Giang',
      code: '908',
      parent_code: '91',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355f9',
      name: 'An Minh',
      type: 'huyen',
      slug: 'an-minh',
      name_with_type: 'Huyện An Minh',
      path: 'An Minh, Kiên Giang',
      path_with_type:
        'Huyện An Minh, Tỉnh Kiên Giang',
      code: '909',
      parent_code: '91',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355fa',
      name: 'Vĩnh Thuận',
      type: 'huyen',
      slug: 'vinh-thuan',
      name_with_type: 'Huyện Vĩnh Thuận',
      path: 'Vĩnh Thuận, Kiên Giang',
      path_with_type:
        'Huyện Vĩnh Thuận, Tỉnh Kiên Giang',
      code: '910',
      parent_code: '91',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355fb',
      name: 'Phú Quốc',
      type: 'thanh-pho',
      slug: 'phu-quoc',
      name_with_type: 'Thành phố Phú Quốc',
      path: 'Phú Quốc, Kiên Giang',
      path_with_type:
        'Thành phố Phú Quốc, Tỉnh Kiên Giang',
      code: '911',
      parent_code: '91',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355fc',
      name: 'Kiên Hải',
      type: 'huyen',
      slug: 'kien-hai',
      name_with_type: 'Huyện Kiên Hải',
      path: 'Kiên Hải, Kiên Giang',
      path_with_type:
        'Huyện Kiên Hải, Tỉnh Kiên Giang',
      code: '912',
      parent_code: '91',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355fd',
      name: 'U Minh Thượng',
      type: 'huyen',
      slug: 'u-minh-thuong',
      name_with_type: 'Huyện U Minh Thượng',
      path: 'U Minh Thượng, Kiên Giang',
      path_with_type:
        'Huyện U Minh Thượng, Tỉnh Kiên Giang',
      code: '913',
      parent_code: '91',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355fe',
      name: 'Giang Thành',
      type: 'huyen',
      slug: 'giang-thanh',
      name_with_type: 'Huyện Giang Thành',
      path: 'Giang Thành, Kiên Giang',
      path_with_type:
        'Huyện Giang Thành, Tỉnh Kiên Giang',
      code: '914',
      parent_code: '91',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c355ff',
      name: 'Ninh Kiều',
      type: 'quan',
      slug: 'ninh-kieu',
      name_with_type: 'Quận Ninh Kiều',
      path: 'Ninh Kiều, Cần Thơ',
      path_with_type:
        'Quận Ninh Kiều, Thành phố Cần Thơ',
      code: '916',
      parent_code: '92',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35600',
      name: 'Ô Môn',
      type: 'quan',
      slug: 'o-mon',
      name_with_type: 'Quận Ô Môn',
      path: 'Ô Môn, Cần Thơ',
      path_with_type:
        'Quận Ô Môn, Thành phố Cần Thơ',
      code: '917',
      parent_code: '92',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35601',
      name: 'Bình Thuỷ',
      type: 'quan',
      slug: 'binh-thuy',
      name_with_type: 'Quận Bình Thuỷ',
      path: 'Bình Thuỷ, Cần Thơ',
      path_with_type:
        'Quận Bình Thuỷ, Thành phố Cần Thơ',
      code: '918',
      parent_code: '92',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35602',
      name: 'Cái Răng',
      type: 'quan',
      slug: 'cai-rang',
      name_with_type: 'Quận Cái Răng',
      path: 'Cái Răng, Cần Thơ',
      path_with_type:
        'Quận Cái Răng, Thành phố Cần Thơ',
      code: '919',
      parent_code: '92',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35603',
      name: 'Thốt Nốt',
      type: 'quan',
      slug: 'thot-not',
      name_with_type: 'Quận Thốt Nốt',
      path: 'Thốt Nốt, Cần Thơ',
      path_with_type:
        'Quận Thốt Nốt, Thành phố Cần Thơ',
      code: '923',
      parent_code: '92',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35604',
      name: 'Vĩnh Thạnh',
      type: 'huyen',
      slug: 'vinh-thanh',
      name_with_type: 'Huyện Vĩnh Thạnh',
      path: 'Vĩnh Thạnh, Cần Thơ',
      path_with_type:
        'Huyện Vĩnh Thạnh, Thành phố Cần Thơ',
      code: '924',
      parent_code: '92',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35605',
      name: 'Cờ Đỏ',
      type: 'huyen',
      slug: 'co-do',
      name_with_type: 'Huyện Cờ Đỏ',
      path: 'Cờ Đỏ, Cần Thơ',
      path_with_type:
        'Huyện Cờ Đỏ, Thành phố Cần Thơ',
      code: '925',
      parent_code: '92',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35606',
      name: 'Phong Điền',
      type: 'huyen',
      slug: 'phong-dien',
      name_with_type: 'Huyện Phong Điền',
      path: 'Phong Điền, Cần Thơ',
      path_with_type:
        'Huyện Phong Điền, Thành phố Cần Thơ',
      code: '926',
      parent_code: '92',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35607',
      name: 'Thới Lai',
      type: 'huyen',
      slug: 'thoi-lai',
      name_with_type: 'Huyện Thới Lai',
      path: 'Thới Lai, Cần Thơ',
      path_with_type:
        'Huyện Thới Lai, Thành phố Cần Thơ',
      code: '927',
      parent_code: '92',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35608',
      name: 'Vị Thanh',
      type: 'thanh-pho',
      slug: 'vi-thanh',
      name_with_type: 'Thành phố Vị Thanh',
      path: 'Vị Thanh, Hậu Giang',
      path_with_type:
        'Thành phố Vị Thanh, Tỉnh Hậu Giang',
      code: '930',
      parent_code: '93',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35609',
      name: 'Ngã Bảy',
      type: 'thanh-pho',
      slug: 'nga-bay',
      name_with_type: 'Thành phố Ngã Bảy',
      path: 'Ngã Bảy, Hậu Giang',
      path_with_type:
        'Thành phố Ngã Bảy, Tỉnh Hậu Giang',
      code: '931',
      parent_code: '93',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3560a',
      name: 'Châu Thành A',
      type: 'huyen',
      slug: 'chau-thanh-a',
      name_with_type: 'Huyện Châu Thành A',
      path: 'Châu Thành A, Hậu Giang',
      path_with_type:
        'Huyện Châu Thành A, Tỉnh Hậu Giang',
      code: '932',
      parent_code: '93',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3560b',
      name: 'Châu Thành',
      type: 'huyen',
      slug: 'chau-thanh',
      name_with_type: 'Huyện Châu Thành',
      path: 'Châu Thành, Hậu Giang',
      path_with_type:
        'Huyện Châu Thành, Tỉnh Hậu Giang',
      code: '933',
      parent_code: '93',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3560c',
      name: 'Phụng Hiệp',
      type: 'huyen',
      slug: 'phung-hiep',
      name_with_type: 'Huyện Phụng Hiệp',
      path: 'Phụng Hiệp, Hậu Giang',
      path_with_type:
        'Huyện Phụng Hiệp, Tỉnh Hậu Giang',
      code: '934',
      parent_code: '93',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3560d',
      name: 'Vị Thuỷ',
      type: 'huyen',
      slug: 'vi-thuy',
      name_with_type: 'Huyện Vị Thuỷ',
      path: 'Vị Thuỷ, Hậu Giang',
      path_with_type:
        'Huyện Vị Thuỷ, Tỉnh Hậu Giang',
      code: '935',
      parent_code: '93',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3560e',
      name: 'Long Mỹ',
      type: 'huyen',
      slug: 'long-my',
      name_with_type: 'Huyện Long Mỹ',
      path: 'Long Mỹ, Hậu Giang',
      path_with_type:
        'Huyện Long Mỹ, Tỉnh Hậu Giang',
      code: '936',
      parent_code: '93',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3560f',
      name: 'Long Mỹ',
      type: 'thi-xa',
      slug: 'long-my',
      name_with_type: 'Thị xã Long Mỹ',
      path: 'Long Mỹ, Hậu Giang',
      path_with_type:
        'Thị xã Long Mỹ, Tỉnh Hậu Giang',
      code: '937',
      parent_code: '93',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35610',
      name: 'Sóc Trăng',
      type: 'thanh-pho',
      slug: 'soc-trang',
      name_with_type: 'Thành phố Sóc Trăng',
      path: 'Sóc Trăng, Sóc Trăng',
      path_with_type:
        'Thành phố Sóc Trăng, Tỉnh Sóc Trăng',
      code: '941',
      parent_code: '94',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35611',
      name: 'Châu Thành',
      type: 'huyen',
      slug: 'chau-thanh',
      name_with_type: 'Huyện Châu Thành',
      path: 'Châu Thành, Sóc Trăng',
      path_with_type:
        'Huyện Châu Thành, Tỉnh Sóc Trăng',
      code: '942',
      parent_code: '94',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35612',
      name: 'Kế Sách',
      type: 'huyen',
      slug: 'ke-sach',
      name_with_type: 'Huyện Kế Sách',
      path: 'Kế Sách, Sóc Trăng',
      path_with_type:
        'Huyện Kế Sách, Tỉnh Sóc Trăng',
      code: '943',
      parent_code: '94',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35613',
      name: 'Mỹ Tú',
      type: 'huyen',
      slug: 'my-tu',
      name_with_type: 'Huyện Mỹ Tú',
      path: 'Mỹ Tú, Sóc Trăng',
      path_with_type:
        'Huyện Mỹ Tú, Tỉnh Sóc Trăng',
      code: '944',
      parent_code: '94',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35614',
      name: 'Cù Lao Dung',
      type: 'huyen',
      slug: 'cu-lao-dung',
      name_with_type: 'Huyện Cù Lao Dung',
      path: 'Cù Lao Dung, Sóc Trăng',
      path_with_type:
        'Huyện Cù Lao Dung, Tỉnh Sóc Trăng',
      code: '945',
      parent_code: '94',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35615',
      name: 'Long Phú',
      type: 'huyen',
      slug: 'long-phu',
      name_with_type: 'Huyện Long Phú',
      path: 'Long Phú, Sóc Trăng',
      path_with_type:
        'Huyện Long Phú, Tỉnh Sóc Trăng',
      code: '946',
      parent_code: '94',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35616',
      name: 'Mỹ Xuyên',
      type: 'huyen',
      slug: 'my-xuyen',
      name_with_type: 'Huyện Mỹ Xuyên',
      path: 'Mỹ Xuyên, Sóc Trăng',
      path_with_type:
        'Huyện Mỹ Xuyên, Tỉnh Sóc Trăng',
      code: '947',
      parent_code: '94',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35617',
      name: 'Ngã Năm',
      type: 'thi-xa',
      slug: 'nga-nam',
      name_with_type: 'Thị xã Ngã Năm',
      path: 'Ngã Năm, Sóc Trăng',
      path_with_type:
        'Thị xã Ngã Năm, Tỉnh Sóc Trăng',
      code: '948',
      parent_code: '94',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35618',
      name: 'Thạnh Trị',
      type: 'huyen',
      slug: 'thanh-tri',
      name_with_type: 'Huyện Thạnh Trị',
      path: 'Thạnh Trị, Sóc Trăng',
      path_with_type:
        'Huyện Thạnh Trị, Tỉnh Sóc Trăng',
      code: '949',
      parent_code: '94',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35619',
      name: 'Vĩnh Châu',
      type: 'thi-xa',
      slug: 'vinh-chau',
      name_with_type: 'Thị xã Vĩnh Châu',
      path: 'Vĩnh Châu, Sóc Trăng',
      path_with_type:
        'Thị xã Vĩnh Châu, Tỉnh Sóc Trăng',
      code: '950',
      parent_code: '94',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3561a',
      name: 'Trần Đề',
      type: 'huyen',
      slug: 'tran-de',
      name_with_type: 'Huyện Trần Đề',
      path: 'Trần Đề, Sóc Trăng',
      path_with_type:
        'Huyện Trần Đề, Tỉnh Sóc Trăng',
      code: '951',
      parent_code: '94',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3561b',
      name: 'Bạc Liêu',
      type: 'thanh-pho',
      slug: 'bac-lieu',
      name_with_type: 'Thành phố Bạc Liêu',
      path: 'Bạc Liêu, Bạc Liêu',
      path_with_type:
        'Thành phố Bạc Liêu, Tỉnh Bạc Liêu',
      code: '954',
      parent_code: '95',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3561c',
      name: 'Hồng Dân',
      type: 'huyen',
      slug: 'hong-dan',
      name_with_type: 'Huyện Hồng Dân',
      path: 'Hồng Dân, Bạc Liêu',
      path_with_type:
        'Huyện Hồng Dân, Tỉnh Bạc Liêu',
      code: '956',
      parent_code: '95',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3561d',
      name: 'Phước Long',
      type: 'huyen',
      slug: 'phuoc-long',
      name_with_type: 'Huyện Phước Long',
      path: 'Phước Long, Bạc Liêu',
      path_with_type:
        'Huyện Phước Long, Tỉnh Bạc Liêu',
      code: '957',
      parent_code: '95',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3561e',
      name: 'Vĩnh Lợi',
      type: 'huyen',
      slug: 'vinh-loi',
      name_with_type: 'Huyện Vĩnh Lợi',
      path: 'Vĩnh Lợi, Bạc Liêu',
      path_with_type:
        'Huyện Vĩnh Lợi, Tỉnh Bạc Liêu',
      code: '958',
      parent_code: '95',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3561f',
      name: 'Giá Rai',
      type: 'thi-xa',
      slug: 'gia-rai',
      name_with_type: 'Thị xã Giá Rai',
      path: 'Giá Rai, Bạc Liêu',
      path_with_type:
        'Thị xã Giá Rai, Tỉnh Bạc Liêu',
      code: '959',
      parent_code: '95',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35620',
      name: 'Đông Hải',
      type: 'huyen',
      slug: 'dong-hai',
      name_with_type: 'Huyện Đông Hải',
      path: 'Đông Hải, Bạc Liêu',
      path_with_type:
        'Huyện Đông Hải, Tỉnh Bạc Liêu',
      code: '960',
      parent_code: '95',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35621',
      name: 'Hoà Bình',
      type: 'huyen',
      slug: 'hoa-binh',
      name_with_type: 'Huyện Hoà Bình',
      path: 'Hoà Bình, Bạc Liêu',
      path_with_type:
        'Huyện Hoà Bình, Tỉnh Bạc Liêu',
      code: '961',
      parent_code: '95',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35622',
      name: 'Cà Mau',
      type: 'thanh-pho',
      slug: 'ca-mau',
      name_with_type: 'Thành phố Cà Mau',
      path: 'Cà Mau, Cà Mau',
      path_with_type:
        'Thành phố Cà Mau, Tỉnh Cà Mau',
      code: '964',
      parent_code: '96',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35623',
      name: 'U Minh',
      type: 'huyen',
      slug: 'u-minh',
      name_with_type: 'Huyện U Minh',
      path: 'U Minh, Cà Mau',
      path_with_type: 'Huyện U Minh, Tỉnh Cà Mau',
      code: '966',
      parent_code: '96',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35624',
      name: 'Thới Bình',
      type: 'huyen',
      slug: 'thoi-binh',
      name_with_type: 'Huyện Thới Bình',
      path: 'Thới Bình, Cà Mau',
      path_with_type:
        'Huyện Thới Bình, Tỉnh Cà Mau',
      code: '967',
      parent_code: '96',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35625',
      name: 'Trần Văn Thời',
      type: 'huyen',
      slug: 'tran-van-thoi',
      name_with_type: 'Huyện Trần Văn Thời',
      path: 'Trần Văn Thời, Cà Mau',
      path_with_type:
        'Huyện Trần Văn Thời, Tỉnh Cà Mau',
      code: '968',
      parent_code: '96',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35626',
      name: 'Cái Nước',
      type: 'huyen',
      slug: 'cai-nuoc',
      name_with_type: 'Huyện Cái Nước',
      path: 'Cái Nước, Cà Mau',
      path_with_type:
        'Huyện Cái Nước, Tỉnh Cà Mau',
      code: '969',
      parent_code: '96',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35627',
      name: 'Đầm Dơi',
      type: 'huyen',
      slug: 'dam-doi',
      name_with_type: 'Huyện Đầm Dơi',
      path: 'Đầm Dơi, Cà Mau',
      path_with_type:
        'Huyện Đầm Dơi, Tỉnh Cà Mau',
      code: '970',
      parent_code: '96',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35628',
      name: 'Năm Căn',
      type: 'huyen',
      slug: 'nam-can',
      name_with_type: 'Huyện Năm Căn',
      path: 'Năm Căn, Cà Mau',
      path_with_type:
        'Huyện Năm Căn, Tỉnh Cà Mau',
      code: '971',
      parent_code: '96',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c35629',
      name: 'Phú Tân',
      type: 'huyen',
      slug: 'phu-tan',
      name_with_type: 'Huyện Phú Tân',
      path: 'Phú Tân, Cà Mau',
      path_with_type:
        'Huyện Phú Tân, Tỉnh Cà Mau',
      code: '972',
      parent_code: '96',
      isDeleted: false,
    },
    {
      _id: '60eaaa6f1173335842c3562a',
      name: 'Ngọc Hiển',
      type: 'huyen',
      slug: 'ngoc-hien',
      name_with_type: 'Huyện Ngọc Hiển',
      path: 'Ngọc Hiển, Cà Mau',
      path_with_type:
        'Huyện Ngọc Hiển, Tỉnh Cà Mau',
      code: '973',
      parent_code: '96',
      isDeleted: false,
    },
  ];
